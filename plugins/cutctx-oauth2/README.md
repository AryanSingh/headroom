...
It plugs into Cutctx's public `cutctx.proxy_extension` entry-point seam, so it
is fully out-of-tree and opt-in.

## Install & enable
```bash
pip install cutctx-oauth2
cutctx proxy --backend litellm-openai --proxy-extension oauth2
```

## Configure (env; no-op unless CUTCTX_OAUTH2_TOKEN_URL is set)
| Env | Meaning |
|-----|---------|
| `CUTCTX_OAUTH2_TOKEN_URL` | token endpoint (client_credentials grant) |
| `CUTCTX_OAUTH2_CLIENT_ID` / `_CLIENT_SECRET` | credentials (secrets) |
| `CUTCTX_OAUTH2_SCOPES` | space/comma-separated scopes |
| `CUTCTX_OAUTH2_AUDIENCE` | optional audience |
| `CUTCTX_OAUTH2_GRANT_TYPE` | default `client_credentials` |
| `CUTCTX_OAUTH2_AUTH_STYLE` | `post` (form creds) or `basic` (HTTP Basic) |
| `CUTCTX_OAUTH2_HEADERS` | static upstream headers, `K=V,K2=V2` |
...
`CUTCTX_OAUTH2_ALLOW_INSECURE=1` to override). Tokens are minted with the standard library
