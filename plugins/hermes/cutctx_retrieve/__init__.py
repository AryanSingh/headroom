...
CUTCTX_RETRIEVE_SCHEMA = {
    "name": "cutctx_retrieve",
    "description": (
        "Retrieve the original uncompressed content behind a cutctx "
...
def _handle_cutctx_retrieve(args: dict, **kw) -> str:
...
def register(ctx) -> None:
    """Register the cutctx_retrieve tool. Called by the plugin loader."""
    ctx.register_tool(
        name="cutctx_retrieve",
        toolset="cutctx",
        schema=CUTCTX_RETRIEVE_SCHEMA,
        handler=_handle_cutctx_retrieve,
        emoji="🗜️",
    )
