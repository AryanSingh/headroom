...
BEGIN
  ...
  -- Version breakdown
  SELECT COALESCE(jsonb_object_agg(COALESCE(cutctx_version, '?'), cnt), '{}'::jsonb)
  INTO _versions
  FROM (
    SELECT cutctx_version, COUNT(*) AS cnt
    FROM proxy_telemetry_v2
    GROUP BY cutctx_version
  ) sub;
