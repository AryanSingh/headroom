...
| **Docker Hub (`ghcr.io/cutctx/cutctx`)** | NO-GO | ENTRYPOINT `cutctx proxy` is broken (binary doesn't exist); image registry still `ghcr.io/aryansingh/cutctx` |
| **Helm chart (`helm/cutctx/`)** | NO-GO | Port 8080 ≠ image 8787; chart name + image registry on old brand |
...
| **Private EE PyPI (`cutctx-ee`)** | NO-GO | `cutctx_ee/__init__.py` says "Cutctx Labs" but `cutctx_ee/LICENSE` says "Cutctx Labs / Payzli Inc." — direct legal conflict; publish-ee.yml has stale comment |
...
|---|---|---|---|---|
| 1 | `cutctx/proxy/handlers/anthropic.py:875` | `AttributeError: '_Entry' object has no attribute 'tokens_saved_per_hit'` | Commit `2d9316ea9` references field that doesn't exist on `_Entry` | NEW (post-Round-2) |
| 2 | `cutctx/proxy/handlers/batch.py:768` | `NameError: name 'request_savings_metadata' is not defined` | Local var never assigned in `handle_google_batch_results` | NEW (post-Round-2) |
| 3 | `cutctx/proxy/server.py:605` (uncommitted) | `TypeError: ...got an unexpected keyword argument 'default_ttl'` | New code calls `get_compression_store(default_ttl=...)` but tests expect 3-arg signature | NEW (uncommitted, in rebrand set) |
...
Same root cause: the Round 3 rebrand renamed symbols (e.g. `CutctxHookProvider` → `CutctxHookProvider`) and the `7795ffb6` fix only patched 15 of 58 test files. Two distinct failure modes:

- **`ImportError: cannot import name 'CutctxHookProvider'`** (30 tests in `tests/integrations/test_strands/test_hooks_unit.py`). The strands integration is dead; needs `CutctxHookProvider = CutctxHookProvider` alias added to `cutctx/integrations/strands/__init__.py`. **Mirror the `CutctxProxy = CutctxProxy` fix from `7795ffb6`.**
- **`NameError: name 'CutctxX' is not defined`** (28 tests across agno, langchain, observability, MCP server). Test files import the old `Cutctx*` name. Either rename tests to `Cutctx*` or add aliases in the integration `__init__.py`s.

**Recommended fix pattern:** Add `CutctxX = CutctxX` aliases in the four public `__init__.py` files (cutctx, cutctx/observability, cutctx/integrations/__init__.py, cutctx/integrations/strands/__init__.py) for every class/function that doesn't have one yet. The aliases already exist for 13 of 19 — only 6 are missing.

### Test gaps worth filling

- `test_hooks_unit.py` import resolution test (catches missing aliases)
- Naming-convention conformance test (walks public API, asserts no symbol begins with `Cutctx*` — only `Cutctx*` is canonical)
...
**🚨 P0: Docker ENTRYPOINT broken.** `Dockerfile:74,98,121` references `/usr/local/bin/cutctx` and `ENTRYPOINT ["cutctx", "proxy"]`, but no `cutctx` script exists in `pyproject.toml` — only `cutctx`. The distroless stage works (`["python3", "-m", "cutctx.cli", "proxy"]`) but the `runtime-slim-base` stage (the default published image per `Dockerfile:125`) is broken. Any `docker pull` + `docker run` will fail with "cutctx: command not found" at container start.

**P1: Version drift.** `pyproject.toml [project].version` says `0.26.0`. `cutctx._version.__version__` at runtime says `0.27.0` (auto-computed from git commits since last tag). `git tag -l` is empty. The CHANGELOG says v0.26.0 was released on 2026-06-19 but **no tag was ever pushed** — see Release Health § below.
...
**P1: Helm port mismatch.** `helm/cutctx/values.yaml:31` has `port: 8080`; Dockerfile `EXPOSE 8787`; compose `8787:8787`. The chart would deploy a service on 8080 that the image doesn't bind to.

**P1: Helm image registry on old brand + personal namespace.** `helm/cutctx/values.yaml:8` says `repository: ghcr.io/aryansingh/cutctx`. Should be the new `cutctx/cutctx` org.
...
**P1: `packaging/cutctx-ee/setup.py` is untracked but checked-in-shape.** Git won't preserve it. Either track it or delete it.
...
| Blocker 3a | Air-gap a no-op | **STILL OPEN — false close** | `EgressEnforcer` not wired into outbound HTTP; `CUTCTX_OFFLINE_MODE=1` is theatrical |
...
- License DB at `~/.cutctx/licenses.db` with no permissions (readable by any local user).
...
3. **`verify-versions.py` fails on HEAD.** `plugins/openclaw/package.json:32` has `cutctx-ai^file:../../../../../../../../tmp/cutctx-release-audit/cutctx-ai-0.26.0.tgz` — a hard-coded absolute `file:` URL from a developer's machine. Shipping a release with this would break `npm install` for any fresh consumer of the openclaw plugin.

4. **README and install scripts have 4+ instances of typo `AryanSingh/cutcxt`.** The dirty diff only fixes 1 line in pyproject. The 4 instances in `README.md` lines 14, 15, 16, 17 are still there. (`cutcxt`, not `cutctx` — `c-x-t` instead of `c-t-x`.) Plus more in `scripts/install.sh`, `scripts/install.ps1`, `helm/cutctx/Chart.yaml:15`.
...
6. **K8s image registry drift.** `k8s/deployment.yaml:42` says `ghcr.io/aryansingh/cutctx:v0.26.0`. `scripts/install.sh:5` and `install.ps1:3` say `ghcr.io/chopratejas/cutctx:latest`. `docker.yml` is undefined. Three different image registries in three different files. Should be `ghcr.io/cutctx/cutctx:0.26.0` (or similar — pick one).
...
8. **EE LICENSE brand name conflict (legal risk).** `cutctx_ee/LICENSE:3` says "Copyright (c) 2025-2026 Cutctx Labs". `cutctx_ee/__init__.py:2` says "Copyright (c) 2025-2026 Cutctx Labs". `LICENSE-COMMERCIAL:4` and `cutctx_ee/LICENSE:5` both say "Legal entity: Payzli Inc. (operating as Cutctx Labs)". The legal entity is Payzli Inc. (parent), operating as Cutctx Labs (sub-brand) — but the codebase has rewritten `__init__.py` to claim Cutctx Labs. This is a corporate-identity mismatch, not just branding.

### The 10 P1 release issues

1. `version-sync.py` and `verify-versions.py` are incomplete — don't cover `helm/Chart.yaml`, `k8s/deployment.yaml`, `packaging/cutctx-ee/pyproject.toml`, or `.release-please-manifest.json`. A version bump that misses any of these is a P1 release bug.
2. `packaging/cutctx-ee/setup.py` is untracked (git won't preserve it).
3. No SBOM (no CycloneDX, no SPDX). `add_spdx.py` is a 152-byte stub.
4. Two license-token formats coexist (HMAC-SHA256 from `generate_license.py` + Ed25519 from `license_keygen.py` + `license_token.py`). Verify the Rust proxy (`crates/cutctx-core/src/licensing.rs` — file is untracked) actually validates the Ed25519 path.
...
9. `crates/cutctx-core/src/licensing.rs` is untracked — Rust-side license enforcement code not committed.
...
| Go + Java SDK rebrand | DEFERRED | ❌ **Still deferred — Java SDK still on `com.cutctx.CutctxClient`** |
...
The rebrand from `cutctx` to `cutctx` is **partially complete**. Python package name and CLI binary are done. Supporting surfaces (install scripts, Docker, Helm, K8s, legal entity, GitHub URLs) are still on the old brand or worse.

### What's done ✓

- `pyproject.toml [project].name` = `cutctx-ai`
- CLI entry point: `cutctx = "cutctx.cli:main"`
- `cutctx --version` works; `cutctx --version` is gone
- `import cutctx` and `import cutctx` both work (shim)
...
- 13 backward-compat aliases: `CutctxConfig`, `CutctxClient`, `CutctxMode`, `CutctxProxy`, `CutctxTracer`, `CutctxOtelMetrics`, `CutctxHookProvider`, `CutctxStrandsModel`, `CutctxBundle`, `get_cutctx_provider`, `get_cutctx_tracer`, `reset_cutctx_tracing`, `CutctxConfig`, `CutctxMode`, `CutctxClient`, `CutctxError`, `CutctxTracer`, `CutctxOtelMetrics`
- `sdk/{python,typescript,go}/` (no `cutctx` suffix)
...
|---|---|---|
| `cutctx_ee/__init__.py` vs `cutctx_ee/LICENSE` | **P0 — legal conflict** | `__init__.py` says "Cutctx Labs"; `LICENSE` says "Cutctx Labs / Payzli Inc." |
| `LICENSE-COMMERCIAL` and `NOTICE` | **P0** | Still on old brand |
| `plugins/cutctx-agent-hooks/hooks/hooks.json` | **P0 — runtime broken** | Invokes gone `cutctx` binary |
| `plugins/cutctx-oauth2/pyproject.toml` | **P0** | Package name `cutctx-oauth2` but URLs `chopratejas/cutctx` |
| `Dockerfile` lines 74, 98, 121 | **P0 — runtime broken** | `cutctx proxy` ENTRYPOINT, no such binary |
| `docker-compose.yml:2` | **P0** | Service `cutctx-proxy` |
| `docker-bake.hcl` | **P0** | `CUTCTX_EXTRAS` build args (8 of 8) |
| `k8s/deployment.yaml:42` | **P0** | `ghcr.io/aryansingh/cutctx:v0.26.0` |
| `helm/cutctx/values.yaml:8` | **P0** | Image registry old brand + personal ns |
| `helm/cutctx/Chart.yaml:2,13,15,18` | **P0** | `name: cutctx`, `home: cutctx.dev`, `email: hello@cutctx.dev`, sources typo `AryanSingh/cutcxt` |
| `scripts/install.sh:5,54,55,65,568,944` | **P0** | Image `ghcr.io/chopratejas/cutctx`; wrapper `cutctx`; URLs typo `AryanSingh/cutcxt` |
| `scripts/install.ps1:3,67-70` | **P0** | Same pattern |
| `.release-please-config.json:14` | **P0** | `package-name: cutctx-ai` |
| `Cargo.toml:24` | **P0** | `repository: github.com/chopratejas/cutctx` |
...
| `sdks/{go-cutctx,java-cutctx}/` | **P0** | Java SDK still on `com.cutctx.CutctxClient` |
| `helm/cutctx/templates/*` | **P0** | All template helpers `{{ include "cutctx.fullname" . }}` — ~30 occurrences |

### What's not started ✗

- `helm/cutctx/` chart does not exist
- `docs/legal/PRIVACY_POLICY_DRAFT.md` and `docs/legal/TERMS_OF_SERVICE_DRAFT.md` do not exist
- No rebrand of `cutctx_ee/` directory itself (only docstring updated)
- `cutctx-oauth2` URL fix (3 occurrences of `chopratejas/cutctx`)
...
- **Python source**: `cutctx` modules not renamed; only `cutctx/` shim added
...
- `crates/cutctx-core/src/licensing.rs` is untracked (new file)
- `cutctx_ee/__init__.py` was modified to say "Cutctx Labs" but the sibling `LICENSE` still says "Cutctx Labs"
- `Cargo.lock` has 17 uncommitted modifications (added `hex`, `hmac 0.12.1` deps to `cutctx-core`)
...
Add all 23 modified + 18 untracked files in a single commit titled `chore(rebrand): in-tree rebrand shell — cutctx consolidation`. Resolve the `cutctx_ee/LICENSE` vs `__init__.py` brand conflict first (the `__init__.py` "Cutctx Labs" claim is the new intent; rewrite `LICENSE` to match, or revert `__init__.py` to match `LICENSE`). **Unblocks:** clears the dirty tree; unblocks every subsequent step.
...
Mirror the `7795ffb6` pattern. Add to `cutctx/integrations/strands/__init__.py`, `cutctx/integrations/agno/__init__.py` (if it exists), and the four other public `__init__.py` files: `CutctxHookProvider = CutctxHookProvider`, `CutctxChatModel = CutctxChatModel`, `CutctxConfig = CutctxConfig` (already done), `CutctxRetriever = CutctxRetriever`, etc. **Unblocks:** test suite goes to 92 → 64 → 5 (Playwright) + 1 (isolation) = 6 fails.

### Fix 4: Fix DSR + air-gap + Stripe tier + residency (~4 hours)

- `cutctx/proxy/routes/dsr.py:181,263,276` — replace the missing-function calls with the correct API surface from `cutctx_ee.ledger.query.LedgerQuery` and `cutctx_ee.audit.AuditLogger`. Add a regression test that fails on the broken version.
- `cutctx/proxy/routing/failover.py` — call `get_egress_enforcer().check(host)` before opening any outbound HTTP connection. Wire the same into `handlers/streaming.py` and any other outbound call site.
- `cutctx_ee/billing/stripe_webhook.py:86` — look up the Price ID from the line items and derive the tier from the canonical mapping; reject any webhook where the metadata tier doesn't match the Price-derived tier.
- `cutctx/proxy/routes/residency.py:26-87` — add `Depends(require_admin_auth) + Depends(require_rbac_permission("residency.read"))`. Add `residency.read` to `PERMISSION_MAP` in `cutctx_ee/rbac.py`.
...
- `Dockerfile:74,98` — replace `cutctx proxy` with `cutctx proxy` (or use the distroless form `[python3, -m, cutctx.cli, proxy]`).
- `Dockerfile:93,116` + `helm/cutctx/values.yaml:31` — pick one port (recommend 8787) and use it everywhere.
- `helm/cutctx/values.yaml:8` + `k8s/deployment.yaml:42` + `scripts/install.{sh,ps1}` — use `ghcr.io/cutctx/cutctx:0.26.0`.
- `helm/cutctx/Chart.yaml:2,13,15,18` — `name: cutctx`, `home: cutctx.dev`, `email: hello@cutctx.dev`, sources: `github.com/cutctx/cutctx`.
- `helm/cutctx/templates/*` — s/cutctx/cutctx/ in all template helpers.
- `docker-compose.yml:2` — `service: cutctx-proxy`.
- `Makefile` — s/cutctx_ai/cutctx_ai/, s/cutctx-proxy/cutctx-proxy/.
...
- `plugins/openclaw/package.json` — replace `file:../../../../../../../../tmp/cutctx-release-audit/cutctx-ai-0.26.0.tgz` with `^0.26.0`.
- `scripts/version-sync.py` — extend to cover `helm/cutctx/Chart.yaml`, `k8s/deployment.yaml`, `packaging/cutctx-ee/pyproject.toml`, `.release-please-manifest.json`.
...
- `cutctx/proxy/webhooks.py:216` — refuse to start without `CUTCTX_WEBHOOK_SECRET` (no `cutctx-dev-secret` default).
- `cutctx/proxy/webhook_stores.py` — encrypt the `secret` column with the same Fernet key as `secrets_store.py`.
- `cutctx/proxy/routes/secrets.py:67` — remove the `strict=False` default; require strict mode.
- `pyproject.toml` — tighten `cryptography>=41.0.0` → `~=46.0`; `PyJWT[crypto]>=2.8.0` → `~=2.11`; `fastapi>=0.100.0` → `>=0.115.0`.
- Resolve the `cutctx_ee/LICENSE` vs `__init__.py` legal entity conflict (see Fix 1).
...
- Java SDK rebrand (`com.cutctx.CutctxClient` → `com.cutctx.CutctxClient`)
