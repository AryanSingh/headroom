...
**Repository:** `/Users/aryansingh/Documents/Claude/Projects/cutctx`
...
|---|---|---|
| LLM request proxy (Anthropic, OpenAI Chat, OpenAI Responses, OpenAI Codex, Gemini, Bedrock, Vertex) | **Implemented** | `cutctx/providers/proxy_routes.py:316-754`; `cutctx/proxy/handlers/anthropic.py`; `cutctx/proxy/handlers/openai/{chat,responses,passthrough,compress}.py`; `cutctx/proxy/handlers/gemini.py` |
| Compression pipeline (CCR, SmartCrusher, LiveZone, Markdown-KV, etc.) | **Implemented** | `cutctx/transforms/pipeline.py:36-49,89-97`; `cutctx/compress/` |
| Per-provider cache compatibility (Anthropic 5m/1h, OpenAI auto-cache, Gemini) | **Implemented** | `cutctx/proxy/handlers/anthropic.py:2516-2520`; `cutctx/proxy/savings_tracker.py:660-668` |
| Response cache (semantic cache, in-memory) | **Implemented** | `cutctx/proxy/semantic_cache.py:24-147`; wired in `anthropic.py:790-840`, `openai/chat.py:275-325` |
| LLM Firewall (regex-based prompt injection + PII scanner) | **Implemented** | `cutctx/security/firewall.py:1-534`; `server.py:2050-2128` (off by default) |
| Streaming PII redactor | **Defined but unwired** | `cutctx/security/firewall.py:409-534`; `wrap_stream` defined but zero callsites |
| Upstream circuit breaker | **Implemented** | `cutctx/proxy/routing/failover.py:3-155` |
| Pipeline circuit breaker | **Implemented** | `cutctx/transforms/pipeline.py:36-49`; `CUTCTX_PIPELINE_BREAKER_*` env vars |
| Health endpoints (`/livez`, `/readyz`, `/health`) | **Implemented** | `cutctx/proxy/server.py:2617-2641`; used in `Dockerfile:95-96,118-119`, `docker-compose.yml:12-16` |
| Prometheus `/metrics` | **Implemented** | `cutctx/proxy/prometheus_metrics.py`; `routes/admin.py:1366-1375` (admin-gated) |
| Spend ledger (per-org/workspace/project) | **Implemented** | `cutctx_ee/ledger/`; `cutctx_ee/ledger/api.py:46-139` |
| Multi-tenancy (per-project memory, per-org spend) | **Implemented (data) / Partial (enforcement)** | `cutctx/memory/storage_router.py:60-240`; `cutctx_ee/org.py:66-465` |
| CLI (28 top-level commands) | **Implemented** | `cutctx/cli/` (541 click decorators across 30 files) |
| Native binary (Rust) | **Built (33.5 MB, 2026-06-20)** | `cutctx/_core.abi3.so`; `crates/cutctx-{core,parity,proxy,py}/` |
...
### 1.2 Enterprise layer (`cutctx_ee/`, commercial)

| Feature | Status | Evidence |
|---|---|---|
| RBAC (4 roles, 25+ permissions) | **Implemented** | `cutctx_ee/rbac.py:31-84`; ~40 admin routes enforce |
| Org/Workspace/Project hierarchy | **Implemented** | `cutctx_ee/org.py:66-465` |
| Admin auth (API key + SSO) | **Implemented (but SSO broken)** | `server.py:2280-2343`; `cutctx_ee/sso.py:263-558` |
| Audit log (SQLite) | **Implemented** | `cutctx_ee/audit.py:88-360` |
| Audit log (HMAC hash chain) | **Implemented but separate, dev-default secret** | `cutctx_ee/audit/store.py:17-156` (default `"dev-secret-key"` at line 24) |
| SCIM 2.0 | **Implemented** | `routes/admin.py:844-1086` |
| Fleet management | **Implemented** | `routes/admin.py:707-758` |
| Residency proof (Ed25519-signed attestation) | **Implemented** | `cutctx/security/residency_proof.py:1-338`; `routes/residency.py:27-88` |
| License validation (Ed25519) | **Implemented** | `cutctx_ee/license_validation.py` |
| Spend ledger ingest/query/export | **Implemented** | `cutctx_ee/ledger/api.py:46-139` |
| Billing / Stripe webhooks (inbound) | **Implemented** | `cutctx_ee/billing/stripe_webhook.py:1-161` |
| Abuse detection | **Implemented but unwired** | `cutctx_ee/abuse.py:135-289`; alerts not delivered to any channel |
...
| `prefix_cache_self_hosted` | Yes (`parse_vllm_apc`) | Yes (header-driven) | Yes | **No** — all live handler sites hardcode `0`; only fires from `x-cutctx-prefix-cache-hits` request header opt-in |
| `model_routing` | Yes (`parse_model_routing_metadata`) | Yes (header-driven) | Yes | **No** — all live handler sites hardcode `0`; only fires from `x-cutctx-model-routing-*` request header opt-in |
...
|---|---|---|---|
| 1 | **Live detection of vLLM APC prefix-cache hits** | **High** | `cutctx/proxy/handlers/{anthropic,openai/chat}.py:830-832,1983-1985,2532-2534,2950-2952,3086-3088,3276-3278` all hardcode `self_hosted_prefix_cache_hits=0` |
...
| 3 | **GDPR/CCPA right-to-delete endpoint** | **Critical** | 0 matches for `gdpr\|ccpa\|DSR\|DSAR\|right_to_delete\|right_to_export` in `cutctx/` or `cutctx_ee/` |
...
| 6 | **SAML SSO** | **High** | `cutctx_ee/sso.py` only handles JWT/JWKS/OIDC. No SAML library. |
| 7 | **End-user API key issuance/revocation** | **High** | 0 matches for `issue_key\|create_api_key\|end_user_key` |
| 8 | **Automated retention/cleanup background loop** | **High** | `cutctx_ee/retention.py:107-122` has `start()` method but no caller in `server.py` |
| 9 | **Per-identity (org/user/API-key) rate limiting** | **High** | `cutctx/proxy/rate_limiter.py:65-79` is per-IP only |
| 10 | **Outbound event subscription model (webhooks with retry/signing/types)** | **High** | `cutctx/proxy/webhooks.py:1-28` is 28 lines, single env var, no signing, no types |
| 11 | **Spend ledger automated backup** | **High** | `k8s/backup-cronjob.yaml:1-30` only backs up `cutctx_memory.db` |
| 12 | **Corruption recovery for new savings store** | **Medium** | `cutctx/savings/` has no `integrity_check` / corrupt-state handling |
| 13 | **Retry on upstream 5xx in core proxy** | **Medium** | `cutctx/memory/adapters/embedders.py:572-597,795-830` has retry for memory; proxy path has none |
| 14 | **Dashboard search, filter, sort, pagination, bulk actions, loading states, error states** | **Medium** | `cutctx/dashboard/templates/dashboard.html` has 0 `<input>` elements, 0 spinners, 0 toasts |
| 15 | **Native binary Prometheus exporter** | **Low** | `cutctx/_core.abi3.so` exists but not exposed for runtime metrics |
| 16 | **Tests for `cutctx.savings/` module** | **High** | 0 test imports in `tests/` (the entire new module on this branch) |
| 17 | **Tests for `cutctx/proxy/routes/{airgap,rate_limit,rbac,secrets,sso}.py`** | **High** | 5 new route modules, 0 test coverage |
| 18 | **`/admin` EE dashboard** | **High** | `routes/admin.py:174-188` returns 404 (references missing `cutctx/dashboard/dist/index.html`) |
...
|---|---|---|---|---|
| 1 | **SSO OIDC** | JWKS + introspection path | PyJWT missing branch silently bypasses signature verification (`sso.py:466-470`); JWKS-dict passed to `pyjwt.decode` raises on real keys (`sso.py:458-465`) | `cutctx_ee/sso.py:263-558` |
| 2 | **Audit logging** | Two parallel stores: simple SQLite + hash chain | 8+ enum events never emitted (`auth.login`, `auth.failed`, `auth.key_rotated`, `license.validated`, etc.); default HMAC secret is `"dev-secret-key"`; only the simple store is wired into admin endpoints | `cutctx_ee/audit.py:46-85`; `cutctx_ee/audit/store.py:24`; `routes/admin.py:237-247` |
| 3 | **Multi-tenancy isolation** | Per-project SQLite (memory) + per-org spend | `/v1/memory/sync` accepts `org_id` from request body and trusts it; review endpoint has explicit TODOs admitting no auth/audit | `cutctx_ee/memory_service/api.py:36-85` |
| 4 | **RBAC persistence** | In-memory dict with ~40 admin routes enforcing | Roles lost on restart; no multi-role; no team membership; no per-project override | `cutctx_ee/rbac.py:102, 174-176` |
...
| 6 | **Encryption at rest** | Fernet (AES-128-CBC + HMAC-SHA256) | SOC2 roadmap claims AES-256 (`gtm/soc2-roadmap.md:60`) — false | `cutctx/security/state_crypto.py:67-75`; `gtm/soc2-roadmap.md:60` |
| 7 | **Dashboard** | Read-only session stats with 5-source cards | No admin workflows (spend management, policy editing, RBAC assignment, SSO config, API-key rotation) in UI; CLI/API-only | `cutctx/dashboard/templates/dashboard.html` |
| 8 | **Spending attribution** | Per-org/workspace/project at storage layer | `/v1/spend/*` is unauthenticated; auth in admin layer doesn't reach EE spend router | `cutctx/proxy/routes/spend.py:1-14` |
| 9 | **Team-memory sync** | Sync endpoint + review endpoint | No auth, no RBAC, no audit, no tenant binding | `cutctx_ee/memory_service/api.py:36-85` |
| 10 | **Provider failover** | Per-provider circuit breaker | `/v1/providers/{name}/disable\|enable` has no auth | `cutctx/proxy/routes/failover.py:59, 74` |
| 11 | **Streaming PII redactor** | `wrap_stream` defined | Zero callsites; PII scanning of streamed responses never happens | `cutctx/security/firewall.py:510-528` |
| 12 | **ML-based prompt injection classifier** | ONNX classifier defined | `security/models/injection_classifier.onnx` does not exist on disk; silently returns 0.0 | `cutctx/security/firewall_ml.py:43-89` |
| 13 | **Audit "actor" attribution** | Some routes read `X-Cutctx-User-Id` header | `policy/api.py:55` hardcodes `actor="admin"`; actor is client-controllable everywhere | `cutctx_ee/policy/api.py:55`; `routes/admin.py:45-47` |
| 14 | **CSP / XSS hardening on dashboard** | Alpine.js HTML-escape helpers used in some places | Live feed string-literal contains user content from disk (`dashboard.html:1724-1726`) — pattern is inconsistent | `cutctx/dashboard/templates/dashboard.html:1724` |
...
|---|---|---|---|
| 1 | **`/admin` returns 404 with raw HTML** (EE dashboard route references non-existent file) | **Critical (UX)** | `cutctx/proxy/routes/admin.py:174-188` reads `cutctx/dashboard/dist/index.html` which does not exist |
| 2 | **SSO signature verification silently bypassed when PyJWT missing** | **Critical (Security)** | `cutctx_ee/sso.py:466-470` |
| 3 | **Dashboard "Documentation" footer link points to `cutctx.dev/docs` which serves the old Cutctx brand** | **High (UX)** | `dashboard.html:1528` |
| 4 | **All EE routes mounted without auth**: `/v1/spend/*`, `/v1/policies/*`, `/v1/audit/*`, `/v1/memory/*`, `/v1/license/*`, `/webhooks/stripe` | **Critical (Security)** | `cutctx/proxy/routes/{spend,policy,audit,memory,license,license_validation}.py:1-14` |
| 5 | **Default admin key auto-generated and logged in plaintext when `CUTCTX_ADMIN_API_KEY` is unset** | **High (Security)** | `cutctx/proxy/server.py:2252-2260` |
| 6 | **`/v1/providers/{name}/disable` has no auth** | **High (Security)** | `cutctx/proxy/routes/failover.py:59, 74` |
| 7 | **`RequestOutcome.from_stream` cannot carry `self_hosted_prefix_cache_hits` or `model_routing_*` (no parameters in signature)** | **High (Functional)** | `cutctx/proxy/outcome.py:278-307` |
| 8 | **Duplicate `_build_savings_breakdown` in outcome.py:36-92 (dead code) and outcome.py:405-540 (active)** | **Medium (Maintainability)** | `cutctx/proxy/outcome.py:36, 405` |
| 9 | **cutctx_compression is computed redundantly in two places; funnel's value overwritten by savings_tracker fallback** | **Medium (Correctness)** | `cutctx/proxy/savings_tracker.py:644-656` vs `cutctx/proxy/outcome.py:464-474` |
| 10 | **Hub label inconsistency: `chopratejas` in `docker-compose.native.yml` vs `aryansingh` in Helm chart** | **Low** | `docker/docker-compose.native.yml:4` vs `helm/cutctx/values.yaml:6` |
| 11 | **Helm chart `tag: "latest"` despite audit claim it was pinned to v0.26.0** | **Medium (Ops)** | `helm/cutctx/values.yaml:6` |
| 12 | **`cutctx learn_share` not registered in `_register_commands()` (file exists, command unreachable)** | **Low** | `cutctx/cli/learn_share.py`; `cutctx/cli/main.py:41-66` |
| 13 | **Dashboard "Per-source USD cards" guard has subtle null-truthy bug in `x-if`** | **Low** | `cutctx/dashboard/templates/dashboard.html:254` |
| 14 | **Hardcoded `version: '0.3.0'` in dashboard HTML; real version only after `/health` succeeds** | **Low** | `cutctx/dashboard/templates/dashboard.html:1539` |
| 15 | **Retirement manager `start()` is never called at server boot** | **High (Compliance)** | `cutctx_ee/retention.py:107-122` (no caller in `server.py`) |
| 16 | **Audit hash-chain store uses default `"dev-secret-key"`** | **High (Security)** | `cutctx_ee/audit/store.py:24` |
...
| 18 | **Dashboard footer "Documentation" link → `cutctx.dev/docs` (old brand)** | **Medium (UX)** | `cutctx/dashboard/templates/dashboard.html:1528` |
...
| SDKs (Python, TS, Go, Java) | Python + TS rebranded; Go + Java still "cutctx" | Yes | No | Python only | Yes | Yes |
...
**Fix:** Add `Depends(_require_admin_auth)` + `Depends(_require_rbac_permission(...))` to all EE router mounts in `cutctx/proxy/routes/spend.py`, `policy.py`, `audit.py`, `memory.py`, `license.py`, `license_validation.py`, and `failover.py`.
...
- `gtm/soc2-roadmap.md:78` "Automated backups — Implemented" — **partially false** (only `cutctx_memory.db` backed up; spend ledger has no backup)
...
**Fix:** Add either (a) response-header-based vLLM APC detection + cost-based model routing policy, or (b) a local prefix-cache store + a routing policy resolver. The escape hatch `savings_metadata` only fires from `x-cutctx-*` request headers that the client must set.

### 6.6 Blocker 6 — UX: `/admin` EE dashboard returns 404
**Severity: High (enterprise admin surface is invisible)**

`cutctx/proxy/routes/admin.py:174-188` returns 404 because it references `cutctx/dashboard/dist/index.html` which doesn't exist. The real dashboard is at `cutctx/dashboard/templates/dashboard.html`.
...
`cutctx_ee/sso.py:466-470` silently bypasses signature verification if PyJWT is missing. `sso.py:458-465` passes a JWKS dict to `pyjwt.decode` which expects a PEM/cryptography key object — this will raise on real keys. Either install PyJWT in production (required) AND fix the JWKS handling, or document that the operator must write a custom key-resolver.
...
`cutctx/proxy/server.py:2258-2259` logs the auto-generated admin key at WARNING level when no `CUTCTX_ADMIN_API_KEY` is set. Any log aggregator (Datadog, Splunk, CloudWatch) will capture the key.

**Fix:** Either (a) refuse to start without `CUTCTX_ADMIN_API_KEY` set, or (b) log the key only at startup on stdout (not via Python logging), and only if no env var is set.
...
9. **Set `CUTCTX_AUDIT_SECRET_KEY` as a required env var** (refuse to start with default `"dev-secret-key"`).
...
20. **Add tests for `cutctx.savings/` module** (the new moat-b1 code).
21. **Add tests for `cutctx/proxy/routes/{airgap,rate_limit,rbac,secrets,sso}.py`** (5 new route modules with 0 coverage).
...
24. **Stop the uncommitted rebrand** — commit the 51-test fix atomically as `chore: complete rebrand Cutctx→Cutctx`.
...
33. **Replace `X-Cutctx-User-Id` as audit-actor source** with the actual authenticated identity (SSO claims or admin key fingerprint).
...
37. **Rebrand Go + Java SDKs** (still use old "cutctx" name).
...
| Test coverage | 10% | 7/10 | 7,338 tests, 66.7% module coverage — 0 tests for new `cutctx.savings/`, 5 new route modules with 0 coverage |
...
- 0 tests for new `cutctx.savings/` module + 5 new route modules with 0 coverage (-4)
...
| Incident response + DR + backups | 5% | 0/5 | No IR plan in code; no DR; spend ledger not backed up; `k8s/backup-cronjob.yaml` only covers `cutctx_memory.db` |
...
- No tests for new `cutctx.savings/` module (-3)
...
- `cutctx/proxy/server.py:2241-2343` — admin auth + SSO validator wiring
- `cutctx/proxy/server.py:2252-2260` — default admin key logged in plaintext
- `cutctx/proxy/server.py:2278-2299` — HMAC compare_digest for admin key
- `cutctx_ee/sso.py:263-558` — full OIDC/JWKS validator (broken PyJWT path at 466-470)
- `cutctx/proxy/routes/spend.py:1-14` — `/v1/spend/*` unauthenticated
- `cutctx/proxy/routes/policy.py`, `audit.py`, `memory.py`, `license.py` — same pattern
- `cutctx/proxy/routes/failover.py:59, 74` — `/v1/providers/{name}/disable` unauthenticated
- `cutctx_ee/memory_service/api.py:36-85` — team-memory sync with explicit TODOs admitting no auth/audit
- `cutctx_ee/audit.py:46-85` — AuditAction enum with 8+ events never emitted
- `cutctx_ee/audit/store.py:24` — default `"dev-secret-key"` HMAC
- `cutctx/proxy/security.py:4` + `state_crypto.py` — Fernet (AES-128-CBC)
- `cutctx_ee/retention.py:107-122` — retention manager `start()` never called from `server.py`

### Core proxy + savings

- `cutctx/proxy/handlers/anthropic.py:807, 1961, 2496, 2922, 3051, 3238` — 6 RequestOutcome sites
- `cutctx/proxy/handlers/openai/chat.py:287, 998, 1321` — 3 RequestOutcome sites
- `cutctx/proxy/outcome.py:36-92, 278-307, 405-540, 543-712` — duplicate `_build_savings_breakdown`, `from_stream` lacking fields, `emit_request_outcome` funnel
- `cutctx/proxy/savings_tracker.py:254-364, 572-903` — `_normalize_history_entry`, `record_request`
- `cutctx/savings/integrations.py:48-119` — parsers for vLLM APC, gptcache, model routing, litellm
- `cutctx/proxy/handlers/anthropic.py:790-840` — semantic cache hit branch
- `cutctx/proxy/handlers/openai/chat.py:275-325` — semantic cache hit branch

### Dashboard

- `cutctx/dashboard/templates/dashboard.html:2351` (file size)
- `cutctx/dashboard/templates/dashboard.html:254-313` — per-source USD cards (the moat-b1 commit db7f7a4)
- `cutctx/dashboard/templates/dashboard.html:1014, 1125-1132` — Recent Requests table + export buttons
- `cutctx/dashboard/templates/dashboard.html:1100-1106, 1469-1476, 804-808` — empty states
- `cutctx/dashboard/templates/dashboard.html:1528, 1539` — dead docs link + hardcoded version
- `cutctx/proxy/server.py:2829-2891` — `_build_stats_payload` (savings_by_source added in commit db7f7a4)
- `cutctx/proxy/routes/admin.py:174-188` — `/admin` 404

### Enterprise

- `cutctx_ee/rbac.py:31-204` — RBAC roles, permissions, in-memory persistence
- `cutctx_ee/org.py:66-465` — Org/Workspace/Project hierarchy
- `cutctx_ee/audit.py:88-360` — audit logger
- `cutctx_ee/audit/store.py:17-156` — hash-chain store
- `cutctx_ee/abuse.py:135-289` — abuse detection (alerts not delivered)
- `cutctx/security/residency_proof.py:1-338` — Ed25519-signed attestation

### Reliability

- `cutctx/proxy/server.py:2617-2641` — health endpoints
- `cutctx/proxy/server.py:1545-1714` — lifespan context manager
- `cutctx/proxy/routing/failover.py:3-155` — per-provider circuit breaker
- `cutctx/transforms/pipeline.py:36-49, 89-97` — pipeline circuit breaker
- `k8s/backup-cronjob.yaml:1-30` — daily backup of `cutctx_memory.db` only

### Tests

- 7,338 tests collected in 11.48s (post-rebrand)
- 0 tests for `cutctx.savings/` module (the new moat-b1 code)
- 0 tests for `cutctx/proxy/routes/{airgap,rate_limit,rbac,secrets,sso}.py` (5 new route modules)

### Packaging

- `pyproject.toml:3-4` — `name = "cutctx-ai"`, `version = "0.26.0"`
- `cutctx/_core.abi3.so` — 33.5 MB, built 2026-06-20 21:21
- `helm/cutctx/values.yaml:6` — `tag: "latest"` (audit claim of pinned tag is false)
- `docker/docker-compose.native.yml:4` — `ghcr.io/chopratejas/cutctx` (old owner)
- `helm/cutctx/values.yaml:6` — `ghcr.io/aryansingh/cutctx` (new owner) — inconsistent

### Plugins

- `plugins/cutctx-oauth2/pyproject.toml:1-30` — functional OAuth2 plugin
- `plugins/cutctx-agent-hooks/hooks/hooks.json` — functional Claude Code plugin
- `plugins/hermes/cutctx_retrieve/__init__.py` — stub (no real code)
...
2. **3 of the 5 savings sources are opt-in only.** Provider cache and semantic cache fire from real traffic. Self-hosted prefix cache, model routing require either header-based opt-in (`x-cutctx-prefix-cache-hits`, `x-cutctx-model-routing-tokens`, `x-cutctx-model-routing-usd`) or a future code change to enable live detection. **The buyer report and dashboard will show 0 for these three sources until either of those is in place.**
