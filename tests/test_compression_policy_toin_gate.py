...
from cutctx.telemetry.toin import TOINConfig, get_toin, reset_toin
from cutctx.tokenizer import Tokenizer
from cutctx.tokenizers import EstimatingTokenCounter
from cutctx.transforms.compression_policy import policy_for_mode


def _has_core() -> bool:
    """Match the pattern in ``test_smart_crusher_rust_parity.py``.

    SmartCrusher's __init__ hard-imports ``cutctx._core`` (the Rust
