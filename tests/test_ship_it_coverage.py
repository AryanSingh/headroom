...
class TestCompressionProfile:
    ...
    def test_load_save_roundtrip(self, tmp_path):
        ...
        with patch("cutctx.profiles._get_profile_path", return_value=tmp_path / "test.json"):
            ...
            p.record_session(
                "session-1", [{"content_type": "json", "original_count": 10, "compressed_count": 5}]
            )
            p.save()
...
class TestAbuseDetector:
    ...
    def test_impossible_travel(self):
        ...
        d.process_event(
            ActivationRecord(
                lic_id="L1", fingerprint="fp1", ip_address="1.2.3.4", geo="US", timestamp=t
            )
        )
        # Second event 60s later from EU — impossible to travel 7000km in 60s
        alerts = d.process_event(
            ActivationRecord(
                lic_id="L1", fingerprint="fp1", ip_address="5.6.7.8", geo="EU", timestamp=t + 60
            )
        )
    ...
    def test_too_many_fingerprints(self):
        ...
        for i in range(5):
            d.process_event(
                ActivationRecord(
                    lic_id="L1", fingerprint=f"fp{i}", ip_address=f"1.2.3.{i}", timestamp=t + i
                )
            )
        alerts = d.process_event(
            ActivationRecord(
                lic_id="L1", fingerprint="fp99", ip_address="1.2.3.99", timestamp=t + 10
            )
        )
