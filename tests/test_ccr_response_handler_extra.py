...
from cutctx.ccr.tool_injection import CCR_TOOL_NAME
...
def test_execute_retrieval_error_paths(monkeypatch: pytest.MonkeyPatch) -> None:
    handler = CCRResponseHandler()
    monkeypatch.setattr(
        "cutctx.ccr.response_handler.get_compression_store",
...
def test_create_tool_result_message_google_and_generic_formats() -> None:
    ...
    assert google_message == {
        "role": "user",
        "parts": [{"functionResponse": {"name": "cutctx_retrieve", "response": {"count": 1}}}],
    }
    ...
    invalid_google = handler._create_tool_result_message(
        [CCRToolResult(tool_call_id="cutctx_retrieve", content="not-json", success=True)],
        "google",
    )
...
def test_streaming_buffer_and_parse_sse_helpers() -> None:
    ...
    anthropic_data = b"\n\n".join(
        [
            ...
            b'data: {"type":"content_block_start","content_block":{"type":"tool_use","id":"tool_1","name":"cutctx_retrieve"}}',
    ...
    assert parsed["content"][1]["name"] == "cutctx_retrieve"
...
async def test_streaming_handler_process_stream_pass_through_and_ccr(
    ...
    ccr_chunks = [
        b'{"type":"tool_use","name":"cutctx_retrieve"',
...
async def test_streaming_handler_falls_back_to_buffer_on_processing_error(
    ...
    chunks = [b'{"type":"tool_use","name":"cutctx_retrieve"', b',"stop_reason":"tool_use"}']
    streamed = [
        chunk
