...
* Adds a ``PERF`` log emission from ``handle_openai_responses_ws`` so
  Codex traffic is no longer invisible to ``cutctx perf``.
...
def test_module_global_unit_semaphore_is_removed() -> None:
    ...
    assert "CUTCTX_CODEX_WS_UNIT_WORKERS" not in source, (
        "The CUTCTX_CODEX_WS_UNIT_WORKERS env knob was removed. It only "
...
#
# Codex WS traffic was invisible to ``cutctx perf`` pre-fix because
...
class _DirectLogCapture(logging.Handler):
    """Direct handler attached to ``cutctx.proxy`` so the proxy's
...
async def test_codex_ws_emits_perf_log_with_cache_keys() -> None:
    """``handle_openai_responses_ws`` must emit a PERF line so ``cutctx
    perf`` counts Codex traffic instead of reporting it as zero requests.

    Asserts on the structured-PERF kv fragment used by ``cutctx/perf/
