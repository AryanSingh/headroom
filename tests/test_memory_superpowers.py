...
async def test_subagent_bridge():
    ...
    # Create bridge
    bridge = SubAgentBridge(memory=memory, parent_session_id=test_session, user_id="test_user")
    ...
    await bridge.merge_result(
        subagent_id="refactor_agent_1", distilled_result="Extracted auth logic into domain layer."
    )

    # Verify result in memory
    results = await memory.query(MemoryFilter(user_id="test_user", session_id=test_session))
