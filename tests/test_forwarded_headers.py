"""Tests for ``cutctx.proxy.forwarded_headers`` — Phase F PR-F4.
...
(``CUTCTX_PROXY_TRUSTED_GATEWAY_CIDRS`` unset / empty) is
...
from cutctx.proxy.forwarded_headers import (
...
def _enable_forwarded_headers_log_propagation() -> None:
    ...
    ``cutctx.proxy`` or this module-specific child logger. These tests assert
...
def test_non_allowlisted_peer_ignores_forwarded_and_logs(
    ...
    with caplog.at_level(logging.WARNING, logger="cutctx.proxy.forwarded_headers"):
