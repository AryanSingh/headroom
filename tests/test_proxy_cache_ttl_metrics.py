...
from cutctx.observability import reset_cutctx_tracing, reset_otel_metrics
from cutctx.proxy.cost import CostTracker, build_prefix_cache_stats
...
def test_prometheus_metrics_export_includes_extended_fields() -> None:
    ...
    assert "cutctx_latency_ms_count 1" in exported
    assert 'cutctx_transform_timing_ms_sum{transform="router"} 4.5' in exported
    assert 'cutctx_waste_signal_tokens_total{signal="json_bloat"} 7' in exported
    assert 'cutctx_cache_write_ttl_tokens_total{provider="anthropic",ttl="5m"} 10' in exported
    assert 'cutctx_provider_cache_hit_requests_total{provider="anthropic"} 1' in exported
...
def test_stats_endpoint_reports_otel_configuration(monkeypatch: pytest.MonkeyPatch) -> None:
    pytest.importorskip(
        ...
        reason="requires the optional otel extra",
    )
    pytest.importorskip("fastapi")
    from fastapi.testclient import TestClient
    ...
    reset_otel_metrics()
    monkeypatch.setenv("CUTCTX_OTEL_METRICS_ENABLED", "1")
    monkeypatch.setenv("CUTCTX_OTEL_METRICS_EXPORTER", "console")
    ...
    assert otel["service_name"] == "cutctx-proxy"
...
def test_stats_endpoint_reports_langfuse_configuration(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("CUTCTX_LANGFUSE_ENABLED", "1")
    ...
    assert langfuse["service_name"] == "cutctx-proxy"
