...
from cutctx.transforms.observability import CompressionObserver
...
def isolated_toin(tmp_path, monkeypatch):
    ...
    `~/.cutctx/toin.json`, which persists across pytest invocations.
    ...
    from cutctx.telemetry.toin import TOIN_PATH_ENV_VAR, reset_toin
...
def test_prometheus_export_does_not_leak_per_strategy_metrics():
    ...
    assert "cutctx_compressions_total" not in output
    assert "cutctx_tokens_saved_by_strategy_total" not in output
