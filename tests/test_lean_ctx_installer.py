...
from cutctx.lean_ctx import get_lean_ctx_path, installer
...
def test_get_lean_ctx_path_finds_windows_managed_binary(tmp_path: Path) -> None:
    ...
    with patch("cutctx.lean_ctx.LEAN_CTX_BIN_DIR", managed_dir):
        with patch("cutctx.lean_ctx.LEAN_CTX_BIN_PATH", managed_dir / "lean-ctx"):
            with patch("cutctx.lean_ctx.shutil.which", return_value=None):
                assert get_lean_ctx_path() == managed_path


def test_get_target_triple_uses_override(monkeypatch) -> None:
    monkeypatch.setenv("CUTCTX_LEAN_CTX_TARGET", "x86_64-pc-windows-msvc")
...
def test_get_download_url_uses_windows_zip(monkeypatch) -> None:
    monkeypatch.delenv("CUTCTX_LEAN_CTX_TARGET", raising=False)
...
def test_download_lean_ctx_skips_verify_for_non_native_target(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setenv("CUTCTX_LEAN_CTX_TARGET", "x86_64-apple-darwin")
...
def test_ensure_lean_ctx_returns_none_when_download_fails() -> None:
    with patch("cutctx.lean_ctx.get_lean_ctx_path", return_value=None):
        with patch.object(
