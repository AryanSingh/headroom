"""Tests for :class:`cutctx.proxy.outcome.RequestOutcome` and the
:meth:`CutctxProxy._record_request_outcome` funnel.
...
def test_cache_hit_pct_rounds_to_int() -> None:
    """PERF log line consumed by ``cutctx perf`` parses an integer here;
...
async def test_funnel_emits_perf_log_with_canonical_shape(
    ...
    """``cutctx perf`` parses this exact ``key=value`` format. Changing
    ...
    # records (the proxy disables ``cutctx.*`` propagation once started).
    target = logging.getLogger("cutctx.proxy")
...
async def test_funnel_appends_client_to_perf_log_when_set() -> None:
    """``cutctx perf --client X`` filtering relies on the ``client=X``
...
@pytest.mark.asyncio
async def test_funnel_finalizes_placeholder_model_routing_metadata() -> None:
    """Placeholder routing metadata should become typed request-log savings."""
    ...
    outcome = _outcome(
        ...
        attempted_input_tokens=100_000,
        output_tokens=0,
        tokens_saved=0,
        savings_metadata={
            "model_routing": {
...
@pytest.mark.asyncio
async def test_funnel_finalizes_model_routing_metadata_after_merge() -> None:
    """Routing metadata must survive merge steps before the outcome funnel."""
    from cutctx.proxy.model_router import ModelRoute, ModelRouter, ModelRouterConfig
    ...
    log_entry = h.logger.logs[0]
    assert log_entry.model_routing_saved_tokens == 50_000
    assert log_entry.total_saved_tokens == 50_000
    assert h.metrics.record_request.await_args.kwargs["model_routing_tokens_saved"] == 50_000
    assert h.metrics.record_request.await_args.kwargs["model_routing_usd_saved"] == pytest.approx(
        0.45
    )
...
async def test_funnel_preserves_precomputed_model_routing_metadata() -> None:
    ...
    log_entry = h.logger.logs[0]
    assert log_entry.model_routing_saved_tokens == 321
    assert log_entry.total_saved_tokens == 321
    assert h.metrics.record_request.await_args.kwargs["model_routing_tokens_saved"] == 321
    assert h.metrics.record_request.await_args.kwargs["model_routing_usd_saved"] == pytest.approx(
        0.123
    )
