...
Verifies that ``CutctxProxy`` gates the pre-upstream phase of
...
- env var ``CUTCTX_ANTHROPIC_PRE_UPSTREAM_CONCURRENCY`` with flag override
"""
...
from cutctx.cli.proxy import proxy as proxy_cli
from cutctx.proxy.handlers.anthropic import AnthropicHandlerMixin
from cutctx.proxy.models import ProxyConfig
from cutctx.proxy.server import CutctxProxy, create_app
...
class _DummyAnthropicHandler(AnthropicHandlerMixin):
    ...
    def __init__(
        ...
        *,
        ...
        # ``CutctxProxy._run_compression_in_executor`` for bounded thread
    ...
    async def _run_compression_in_executor(self, fn, *, timeout):  # noqa: ANN001
        # Mirror of ``CutctxProxy._run_compression_in_executor`` for the
...
def _run_cli_capture(args: list[str], env: dict | None = None) -> ProxyConfig:
    ...
    via ``from cutctx.proxy.server import ... run_server``) short-
    circuits it and lets us inspect the ``ProxyConfig`` that was built.
    """
    import cutctx.proxy.server as server_mod
...
def test_env_var_sets_pre_upstream_concurrency():
    ...
    env = {"CUTCTX_ANTHROPIC_PRE_UPSTREAM_CONCURRENCY": "4"}
...
def test_cli_env_sets_pre_upstream_timeouts():
    env = {
        "CUTCTX_ANTHROPIC_PRE_UPSTREAM_ACQUIRE_TIMEOUT_SECONDS": "9.5",
        "CUTCTX_ANTHROPIC_PRE_UPSTREAM_MEMORY_CONTEXT_TIMEOUT_SECONDS": "3.25",
    }
    config = _run_cli_capture([], env=env)
