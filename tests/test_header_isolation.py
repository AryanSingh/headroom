...
`x-cutctx-*` request headers are internal control flags consumed by the
...
for bypass gating, `_extract_tags` reading `x-cutctx-*`) keep working
because they never depended on the local outbound-bound dict.

Operator opt-in `CUTCTX_STRIP_INTERNAL_HEADERS=disabled` keeps the
...
def test_strip_returns_new_dict_does_not_mutate_caller() -> None:
    ...
    assert "x-cutctx-bypass" in original
    assert "x-cutctx-bypass" not in out
    assert out["authorization"] == "Bearer x"


def test_strip_x_cutctx_bypass_removed() -> None:
    out = _strip_internal_headers({"x-cutctx-bypass": "true", "k": "v"})
    assert "x-cutctx-bypass" not in out
    assert out["k"] == "v"


def test_strip_x_cutctx_mode_removed() -> None:
    out = _strip_internal_headers({"x-cutctx-mode": "passthrough", "k": "v"})
    assert "x-cutctx-mode" not in out
    assert out["k"] == "v"


def test_strip_x_cutctx_user_id_removed() -> None:
    out = _strip_internal_headers({"x-cutctx-user-id": "u1", "k": "v"})
    assert "x-cutctx-user-id" not in out
    assert out["k"] == "v"


def test_strip_x_cutctx_stack_removed() -> None:
    out = _strip_internal_headers({"x-cutctx-stack": "engineer", "k": "v"})
    assert "x-cutctx-stack" not in out
    assert out["k"] == "v"


def test_strip_x_cutctx_base_url_removed() -> None:
    out = _strip_internal_headers({"x-cutctx-base-url": "http://x", "k": "v"})
    assert "x-cutctx-base-url" not in out
    assert out["k"] == "v"


def test_strip_case_insensitive_prefix_match() -> None:
    """Mixed-case `X-Cutctx-Foo`, `x-Cutctx-Bar`, `X-CUTCTX-BAZ` all stripped."""
    ...
    assert "X-Cutctx-Foo" not in out
    assert "x-Cutctx-Bar" not in out
    assert "X-CUTCTX-BAZ" not in out
    assert out["Authorization"] == "Bearer x"
...
def test_strip_disabled_mode_passes_internal_headers_through(
    ...
    """`CUTCTX_STRIP_INTERNAL_HEADERS=disabled` is operator opt-in for diag."""
    monkeypatch.setenv("CUTCTX_STRIP_INTERNAL_HEADERS", "disabled")
    out = _strip_internal_headers({"x-cutctx-bypass": "true", "authorization": "Bearer x"})
    assert out["x-cutctx-bypass"] == "true"
    assert out["authorization"] == "Bearer x"
...
# ---------------------------------------------------------------------------
# End-to-end: x-cutctx-* never reaches the upstream
# ---------------------------------------------------------------------------
...
def test_x_cutctx_bypass_not_forwarded() -> None:
    ...
    assert "x-cutctx-bypass" not in upstream
...
def test_x_cutctx_mode_not_forwarded() -> None:
    client, transport = _make_anthropic_app()
    resp = client.post(
        "/v1/messages",
        headers={
    ...
    assert transport.captured_headers is not None
    upstream = {k.lower(): v for k, v in transport.captured_headers.items()}
    assert "x-cutctx-mode" not in upstream


def test_x_cutctx_user_id_not_forwarded() -> None:
    client, transport = _make_anthropic_app()
    resp = client.post(
        "/v1/messages",
        headers={
    ...
    assert transport.captured_headers is not None
    upstream = {k.lower(): v for k, v in transport.captured_headers.items()}
    assert "x-cutctx-user-id" not in upstream


def test_x_cutctx_stack_not_forwarded() -> None:
    client, transport = _make_anthropic_app()
    resp = client.post(
        "/v1/messages",
        headers={
    ...
    assert transport.captured_headers is not None
    upstream = {k.lower(): v for k, v in transport.captured_headers.items()}
    assert "x-cutctx-stack" not in upstream


def test_x_cutctx_base_url_not_forwarded() -> None:
    client, transport = _make_anthropic_app()
    resp = client.post(
        "/v1/messages",
        headers={
            ...
            "anthropic-version": "2023-06-01",
            "content-type": "application/json",
            "x-cutctx-base-url": "https://override.example",
        },
        json={
            "model": "claude-sonnet-4-6",
            "max_tokens": 16,
    ...
    assert transport.captured_headers is not None
    upstream = {k.lower(): v for k, v in transport.captured_headers.items()}
    assert "x-cutctx-base-url" not in upstream
...
def test_case_insensitive_prefix_match_e2e() -> None:
    ...
    assert "x-cutctx-foo" not in upstream_lower
    assert "x-cutctx-bar" not in upstream_lower
    assert "x-cutctx-baz" not in upstream_lower
...
def test_inbound_read_path_still_reads_x_cutctx_bypass() -> None:
    """Bypass header still gates compression even though it's stripped from upstream.

    The handler reads `request.headers.get('x-cutctx-bypass')` directly.
    ...
    #   2. The upstream did NOT receive the `x-cutctx-bypass` header.
    resp = client.post(
        "/v1/messages",
        headers={
            "x-api-key": "test-key",
...
def test_disabled_mode_passes_through_e2e(
    ...
    """`CUTCTX_STRIP_INTERNAL_HEADERS=disabled` lets internal headers through."""
    monkeypatch.setenv("CUTCTX_STRIP_INTERNAL_HEADERS", "disabled")
    client, transport = _make_anthropic_app()
    resp = client.post(
        "/v1/messages",
    ...
    assert upstream.get("x-cutctx-mode") == "passthrough"
