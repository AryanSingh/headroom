...
def test_anthropic_hooks_do_not_break_extract_user_query_lookup(tmp_path, monkeypatch):
    ...
    with TestClient(app) as client:
        ...
        # `x-cutctx-tokens-after` reflects the proxy-side recount of the
        ...
        # the inflation guard.
        from cutctx.tokenizers.registry import get_tokenizer

        expected_after = get_tokenizer("claude-sonnet-4-6").count_messages(compressed_messages)
        assert response.headers["x-cutctx-tokens-after"] == str(expected_after)
        tokens_before = int(response.headers["x-cutctx-tokens-before"])
        tokens_after = int(response.headers["x-cutctx-tokens-after"])
        assert tokens_before > tokens_after, (
            f"compression should reduce tokens: before={tokens_before} after={tokens_after}"
        )
        assert int(response.headers["x-cutctx-tokens-saved"]) > 0
