...
def test_bash_native_installer_supports_persistent_docker_lifecycle(tmp_path: Path) -> None:
    ...
    env["CUTCTX_DOCKER_IMAGE"] = "cutctx:test-image"
    ...
    try:
        ...
        assert "CUTCTX_IMAGE_DEFAULT=cutctx:test-image" in wrapper.read_text(encoding="utf-8")
        ...
        manifest_path = home / ".cutctx" / "deploy" / "smoke" / "manifest.json"
        manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
        assert manifest["preset"] == "persistent-docker"
        assert manifest["port"] == port
        assert manifest["memory_enabled"] is True
        assert manifest["memory_db_path"] == "/tmp/cutctx-home/.cutctx/memory.db"
        assert manifest["telemetry_enabled"] is False

        state_path = home / ".cutctx" / "deploy" / "smoke" / "docker-native.env"
        ...
        assert "/tmp/cutctx-home/.cutctx/memory.db" in install_call
        ...
        assert "CUTCTX_WORKSPACE_DIR=/tmp/cutctx-home/.cutctx" in install_call
        assert "CUTCTX_CONFIG_DIR=/tmp/cutctx-home/.cutctx/config" in install_call
...
def test_powershell_native_installer_supports_persistent_docker_lifecycle(tmp_path: Path) -> None:
    ...
    try:
        ...
        wrapper = home / ".local" / "bin" / "cutctx.ps1"
        assert wrapper.exists()
        assert "__CUTCTX_INSTALL_IMAGE__" not in wrapper.read_text(encoding="utf-8")
        assert "cutctx:test-image" in wrapper.read_text(encoding="utf-8")
        cmd_wrapper = home / ".local" / "bin" / "cutctx.cmd"
        ...
        manifest_path = home / ".cutctx" / "deploy" / "smoke" / "manifest.json"
        state_path = home / ".cutctx" / "deploy" / "smoke" / "docker-native.json"
        ...
        assert manifest["memory_db_path"] == "/tmp/cutctx-home/.cutctx/memory.db"
        assert manifest["telemetry_enabled"] is False
        assert state["container_name"] == "cutctx-smoke"
