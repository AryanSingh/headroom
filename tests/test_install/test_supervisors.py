...
from cutctx.install.models import DeploymentManifest, SupervisorKind
...
def test_linux_service_unit_uses_user_systemd_path(monkeypatch, tmp_path: Path) -> None:
    ...
    unit_path, content = _linux_service_unit(manifest, tmp_path / "run-cutctx.sh")

    assert unit_path == tmp_path / ".config" / "systemd" / "user" / "cutctx-default.service"
    assert "ExecStart=" + str(tmp_path / "run-cutctx.sh") in content
    assert "Restart=on-failure" in content


def test_command_for_script_and_unix_runner(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setattr(
        "cutctx.install.supervisors.resolve_cutctx_command",
    ...
    record = _render_unix_runner(
        tmp_path / "scripts" / "run-cutctx.sh", ["cutctx", "run", "--flag"]
    )
    assert record.kind == "script"
    content = Path(record.path).read_text(encoding="utf-8")
    assert content.startswith("#!/usr/bin/env bash")
    assert "exec cutctx run --flag" in content
...
def test_linux_task_spec_for_user_scope_includes_crontab_markers(tmp_path: Path) -> None:
    ...
    cron_path, content = _linux_task_spec(manifest, tmp_path / "ensure-cutctx.sh")

    assert cron_path is None
    assert "# >>> cutctx smoke >>>" in content
    assert "# <<< cutctx smoke <<<" in content
...
def test_macos_launchd_plist_switches_between_keepalive_and_interval(
    ...
    service_path, service_content = _macos_launchd_plist(
        service_manifest, tmp_path / "run-cutctx.sh"
    )
    assert service_path == tmp_path / "Library" / "LaunchAgents" / "com.cutctx.default.plist"
    ...
    task_path, task_content = _macos_launchd_plist(
        task_manifest, tmp_path / "ensure-cutctx.sh", interval=300
    )
    assert task_path == tmp_path / "Library" / "LaunchAgents" / "com.cutctx.tasky.plist"
...
def test_render_windows_runner_writes_ps1_and_cmd_wrappers(tmp_path: Path) -> None:
    ps1_path = tmp_path / "run-cutctx.ps1"
    cmd_path = tmp_path / "run-cutctx.cmd"
    ...
    assert '& "C:\\Program Files\\Python\\python.exe" cutctx install agent run' in ps1_content
    assert (
        'powershell -NoProfile -ExecutionPolicy Bypass -File "%~dp0run-cutctx.ps1" %*'
        in cmd_content
    )
...
def test_render_runner_scripts_writes_windows_scripts(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        "cutctx.install.supervisors.windows_run_cmd_path",
        lambda profile: tmp_path / "run-cutctx.cmd",
    )
    monkeypatch.setattr(
        "cutctx.install.supervisors.windows_ensure_script_path",
        lambda profile: tmp_path / "ensure-cutctx.ps1",
    )
    monkeypatch.setattr(
        "cutctx.install.supervisors.windows_ensure_cmd_path",
        lambda profile: tmp_path / "ensure-cutctx.cmd",
    )
...
def test_start_and_stop_supervisor_use_linux_systemctl(monkeypatch) -> None:
    ...
    monkeypatch.setattr("cutctx.install.supervisors.sys.platform", "linux")
    monkeypatch.setattr(
        "cutctx.install.supervisors.subprocess.run",
    ...
    assert calls == [
        ["systemctl", "--user", "restart", "cutctx-default"],
        ["systemctl", "--user", "stop", "cutctx-default"],
    ]
...
def test_install_supervisor_linux_service_and_tasks(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setattr("cutctx.install.supervisors.sys.platform", "linux")
    monkeypatch.setattr("cutctx.install.supervisors.sys.platform", "linux")
    run_script = tmp_path / "run-cutctx.sh"
    ensure_script = tmp_path / "ensure-cutctx.sh"
    monkeypatch.setattr(
        "cutctx.install.supervisors.render_runner_scripts",
        lambda manifest: [
            type("Record", (), {"kind": "script", "path": run_script.as_posix()})(),
            type("Record", (), {"kind": "script", "path": ensure_script.as_posix()})(),
        ],
    )
    unit_path = tmp_path / "cutctx-default.service"
    monkeypatch.setattr(
        "cutctx.install.supervisors._linux_service_unit",
    ...
    monkeypatch.setattr("cutctx.install.supervisors.subprocess.run", fake_run)
    ...
    assert ["systemctl", "--user", "enable", "cutctx-default"] in [call[0] for call in calls]
    assert service_records[-1].kind == "service-unit"

    cron_path = tmp_path / "cutctx-system"
    monkeypatch.setattr(
        "cutctx.install.supervisors._linux_task_spec",
...
def test_install_supervisor_darwin_windows_and_unsupported(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("cutctx.install.supervisors.os.getuid", lambda: 123, raising=False)

    plist_path = tmp_path / "com.cutctx.default.plist"
    monkeypatch.setattr(
        "cutctx.install.supervisors._macos_launchd_plist",
        lambda manifest, script, interval=None: (plist_path, f"plist-{interval}"),
    )
    monkeypatch.setattr("cutctx.install.supervisors.sys.platform", "darwin")
    monkeypatch.setattr("cutctx.install.supervisors.sys.platform", "darwin")
    service_records = install_supervisor(_manifest(supervisor=SupervisorKind.SERVICE.value))
    ...
    monkeypatch.setattr(
        "cutctx.install.supervisors.windows_run_cmd_path",
        lambda profile: Path(f"C:\\tmp\\{profile}\\run-cutctx.cmd"),
    )
    monkeypatch.setattr(
        "cutctx.install.supervisors.windows_ensure_cmd_path",
        lambda profile: Path(f"C:\\tmp\\{profile}\\ensure-cutctx.cmd"),
    )
    ...
    assert [
        ...
        "create",
        "cutctx-default",
        'binPath= cmd.exe /c "C:\\tmp\\default\\run-cutctx.cmd"',
        "start= auto",
    ] in calls
    assert [
        "schtasks",
        "/Create",
        "/TN",
        "cutctx-default-health",
        "/TR",
        "C:\\tmp\\default\\ensure-cutctx.cmd",
        "/SC",
        "MINUTE",
        "/MO",
        "5",
    ...
    monkeypatch.setattr("cutctx.install.supervisors.sys.platform", "plan9")
    monkeypatch.setattr("cutctx.install.supervisors.sys.platform", "plan9")
...
def test_start_and_stop_supervisor_darwin_windows_and_none(monkeypatch) -> None:
    ...
    monkeypatch.setattr("cutctx.install.supervisors.os.getuid", lambda: 77, raising=False)
    ...
    assert calls == [
        ["launchctl", "kickstart", "-k", "gui/77/com.cutctx.default"],
        ["launchctl", "bootout", "gui/77/com.cutctx.default"],
    ]
    ...
    stop_supervisor(_manifest(supervisor=SupervisorKind.SERVICE.value))
    assert calls == [
        ["sc.exe", "start", "cutctx-default"],
        ["sc.exe", "stop", "cutctx-default"],
    ]
...
def test_remove_supervisor_linux_service_cron_path_and_missing_crontab(
    ...
    assert ["systemctl", "--user", "disable", "--now", "cutctx-default"] in calls
    assert ["systemctl", "--user", "daemon-reload"] in calls

    cron_path = tmp_path / "cutctx-task"
...
def test_remove_supervisor_darwin_and_windows(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("cutctx.install.supervisors.os.getuid", lambda: 55, raising=False)

    plist_path = tmp_path / "com.cutctx.default.plist"
    plist_path.write_text("plist", encoding="utf-8")
    monkeypatch.setattr(
        "cutctx.install.supervisors.unix_run_script_path",
        lambda profile: tmp_path / "run-cutctx.sh",
    )
    monkeypatch.setattr(
        "cutctx.install.supervisors.unix_ensure_script_path",
        lambda profile: tmp_path / "ensure-cutctx.sh",
    )
    monkeypatch.setattr(
        "cutctx.install.supervisors._macos_launchd_plist",
    ...
    assert calls[0] == ["launchctl", "bootout", "gui/55/com.cutctx.default"]
    ...
    assert calls == [
        ["sc.exe", "stop", "cutctx-default"],
        ["sc.exe", "delete", "cutctx-default"],
        ["schtasks", "/Delete", "/TN", "cutctx-default-startup", "/F"],
        ["schtasks", "/Delete", "/TN", "cutctx-default-health", "/F"],
    ]
