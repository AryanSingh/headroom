"""Tests for :class:`cutctx.proxy.compression_decision.CompressionDecision`,
...
* ``handlers/gemini.py`` had THREE compression sites that NEVER checked
  the ``x-cutctx-bypass`` header — explicit user requests to skip
...
def _config(*, optimize: bool = True) -> Any:
    """Minimal stand-in for ``CutctxConfig`` — only the fields the
    decision reads."""
...
def test_bypass_header_wins_over_every_other_gate() -> None:
    """``x-cutctx-bypass`` is the user's explicit "do not touch my
...
def test_passthrough_mode_header_also_triggers_bypass() -> None:
    """``x-cutctx-mode: passthrough`` is the alternate spelling of the
    bypass signal — both go through the same path. This mirrors the
    pre-existing ``_cutctx_bypass_enabled`` semantics in helpers.py;
    re-asserted here so a refactor of that helper can't silently
    diverge."""
    d = CompressionDecision.decide(
        headers={"x-cutctx-mode": "passthrough"},
        config=_config(optimize=True),
        usage_reporter=_usage_reporter(should_compress=True),
        messages=_msgs(),
    )
...
def test_bypass_header_is_case_insensitive() -> None:
    """Real client UAs send ``X-Cutctx-Bypass`` (title-case) or
    ``x-cutctx-bypass`` (lower-case). Both must work — the existing
    helper normalised both, so the consolidated decision must too."""
    for header_key in ("X-Cutctx-Bypass", "x-cutctx-bypass", "X-CUTCTX-BYPASS"):
...
def test_decide_accepts_fastapi_style_starlette_headers() -> None:
    ...
    h = _FakeStarletteHeaders({"X-Cutctx-Bypass": "true"})
    d = CompressionDecision.decide(
