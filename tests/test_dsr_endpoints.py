...
import cutctx.proxy.routes.dsr as dsr_module
...
def test_dsr_export_unauthenticated_rejected(client: TestClient) -> None:
    ...
    r = client.get("/v1/dsr/export?user_id=alice")
    assert r.status_code == 401, r.text


def test_dsr_delete_unauthenticated_rejected(client: TestClient) -> None:
    """No Bearer token → 401 (admin auth gate)."""
    r = client.post("/v1/dsr/delete", json={"user_id": "alice"})
    assert r.status_code == 401, r.text
...
def test_dsr_export_sso_user_id_from_state(client: TestClient) -> None:
    """The user_id resolves from request.state.cutctx_user_id when set by SSO."""
    r = client.get("/v1/dsr/export", headers=_auth_headers())
...
def test_dsr_response_shape_contract() -> None:
    ...
    request = type(
        ...
        (),
        {
            "state": type("FakeState", (), {"cutctx_user_id": "alice"})(),
            "headers": {},
        },
    )()
    target = dsr_module._resolve_target_user_id(request, body_user_id=None, query_user_id=None)
