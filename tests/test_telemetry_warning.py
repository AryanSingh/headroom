...
from cutctx.telemetry.beacon import (  # noqa: E402
...
class TestIsTelemetryWarnEnabled:
    """Tests for the CUTCTX_TELEMETRY_WARN feature flag."""

    def test_enabled_by_default(self, monkeypatch):
        monkeypatch.delenv("CUTCTX_TELEMETRY_WARN", raising=False)
    ...
    def test_disabled_by_env_var(self, monkeypatch, value):
        monkeypatch.setenv("CUTCTX_TELEMETRY_WARN", value)
...
class TestFormatTelemetryNotice:
    ...
    def test_returns_notice_when_telemetry_on(self, monkeypatch):
        monkeypatch.setenv("CUTCTX_TELEMETRY", "on")
        monkeypatch.delenv("CUTCTX_TELEMETRY_WARN", raising=False)
        notice = format_telemetry_notice()
        assert notice != ""
        assert "ENABLED" in notice
        assert "CUTCTX_TELEMETRY=off" in notice
    ...
    def test_empty_when_warn_flag_off(self, monkeypatch):
        monkeypatch.setenv("CUTCTX_TELEMETRY", "on")
        monkeypatch.setenv("CUTCTX_TELEMETRY_WARN", "off")
        assert format_telemetry_notice() == ""
...
class TestProxyCLITelemetryBanner:
    ...
    def test_banner_shows_telemetry_enabled(self, runner, monkeypatch):
        monkeypatch.delenv("CUTCTX_TELEMETRY", raising=False)

        from cutctx.cli.main import main

        with patch("cutctx.proxy.server.run_server", side_effect=SystemExit(0)):
    ...
    def test_banner_shows_opt_out_instructions_when_enabled(self, runner, monkeypatch):
        ...
        assert "CUTCTX_TELEMETRY=off" in result.output or "--no-telemetry" in result.output
...
class TestWrapCLITelemetryNotice:
    ...
    def test_print_notice_outputs_when_telemetry_on(self, monkeypatch, capsys):
        ...
        from cutctx.cli.wrap import _print_telemetry_notice

        _print_telemetry_notice()
        captured = capsys.readouterr()
        assert "Telemetry" in captured.out
        assert "CUTCTX_TELEMETRY=off" in captured.out
...
class TestTelemetryModuleExports:
    """New helpers must be exported from cutctx.telemetry."""

    def test_is_telemetry_warn_enabled_exported(self):
        from cutctx.telemetry import is_telemetry_warn_enabled as fn

        assert callable(fn)

    def test_is_telemetry_enabled_exported(self):
        from cutctx.telemetry import is_telemetry_enabled as fn

        assert callable(fn)

    def test_format_telemetry_notice_exported(self):
        from cutctx.telemetry import format_telemetry_notice as fn
