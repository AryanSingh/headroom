...
@pytest.fixture(autouse=True)
def _enable_cutctx_log_propagation() -> Iterator[None]:
    """Re-enable propagation on the cutctx loggers during tests.

    configure_proxy_logging() calls cutctx_logger.propagate = False to prevent
...
class TestCorruptMemoryGoldenBytes:
    ...
    def test_corrupt_bytes_logs_error(self, caplog: pytest.LogCaptureFixture) -> None:
        ...
        with caplog.at_level(logging.ERROR, logger="cutctx.proxy"):
            apply_session_sticky_memory_tools(
                provider="anthropic",
                session_id=session_id,
                request_id="req-2",
...
class TestCorruptCcrGoldenBytes:
    ...
    def test_valid_golden_bytes_still_injected(self) -> None:
        ...
        from cutctx.ccr.tool_injection import create_ccr_tool_definition
        from cutctx.proxy.helpers import serialize_tool_definition_canonical
