...
* ``StreamingMixin._stream_openai_via_backend`` (Azure/LiteLLM/AnyLLM OpenAI
  ...
  for this path, so ``cutctx perf`` shows 0 cache writes for every
...
Both surface to the user as "Cache write: 0 tokens" in ``cutctx perf``.
"""
...
from cutctx.backends.base import StreamEvent  # noqa: E402
...
class _ListHandler(logging.Handler):
    ...
    stop receiving cutctx-namespaced records. Attaching directly to
    ``cutctx.proxy`` sidesteps that.
    """
...
def test_openai_backend_streaming_perf_zeros_when_upstream_omits_usage() -> None:
    ...
    final usage chunk) the PERF line still has to emit so ``cutctx perf``
    counts the request.
    """
    config = ProxyConfig(
        optimize=False,
