"""Tests for per-project savings attribution (X-Cutctx-Project)."""
...
from cutctx.proxy.outcome import RequestOutcome, emit_request_outcome  # noqa: E402
from cutctx.proxy.project_context import (  # noqa: E402
    classify_project,
    get_current_project,
    set_current_project,
    split_project_path,
...
def test_classify_project_reads_header_case_insensitively():
    assert classify_project({"x-cutctx-project": "frontend"}) == "frontend"
    assert classify_project({"X-Cutctx-Project": " frontend "}) == "frontend"
...
def test_middleware_binds_project_header_to_context(tmp_path, monkeypatch):
    ...
    import cutctx.proxy.server as server_module
    ...
    with TestClient(create_app(config)) as client:
        assert client.get("/health", headers={"X-Cutctx-Project": " my repo "}).status_code == 200
        ...
        assert (
            client.get(
                "/p/prefix-project/health", headers={"X-Cutctx-Project": "header-project"}
            ).status_code
            == 200
        )
