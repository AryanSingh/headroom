...
def test_transform_pipeline_emits_trace_spans() -> None:
    ...
    provider = TracerProvider(resource=Resource.create({"service.name": "cutctx-test"}))
    provider.add_span_processor(SimpleSpanProcessor(exporter))
    set_cutctx_tracer(CutctxTracer(tracer_provider=provider))
    ...
    try:
        ...
        assert len(spans) == 1
        span = spans[0]
        assert span.name == "cutctx.compression.pipeline"
        assert span.attributes["cutctx.model"] == "gpt-4o"
        assert span.attributes["cutctx.tokens.before"] >= 1
        assert span.attributes["cutctx.tokens.after"] >= 1
    finally:
        reset_cutctx_tracing()
...
def test_langfuse_tracing_requires_explicit_enable(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    config = LangfuseTracingConfig.from_env(default_service_name="cutctx-proxy")
