...
def test_stats_reset_warns_when_audit_logging_fails(monkeypatch) -> None:
    ...
    monkeypatch.setattr(server_module.logger, "warning", _capture_warning)
    ...
    assert response.status_code == 200, response.text
    assert "Failed to audit stats reset: audit offline" in warnings
