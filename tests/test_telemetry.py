...
class TestGlobalTelemetryCollector:
    ...
    def test_env_var_disables_telemetry(self, monkeypatch):
        """CUTCTX_TELEMETRY_DISABLED environment variable disables telemetry."""
        reset_telemetry_collector()
        monkeypatch.setenv("CUTCTX_TELEMETRY_DISABLED", "1")
    ...
    def test_cutctx_telemetry_off_disables_collector(self, monkeypatch, off_value):
        """CUTCTX_TELEMETRY=off (and other documented opt-out values) disables
        the collector — closes #390.

        Pre-#390 the collector only honoured CUTCTX_TELEMETRY_DISABLED, which
        is undocumented. Users following the docs set CUTCTX_TELEMETRY=off and
        ...
        """
        reset_telemetry_collector()
        monkeypatch.delenv("CUTCTX_TELEMETRY_DISABLED", raising=False)
        monkeypatch.setenv("CUTCTX_TELEMETRY", off_value)

        collector = get_telemetry_collector()

        assert collector._config.enabled is False, (
            f"CUTCTX_TELEMETRY={off_value!r} must disable the collector — "
    ...
    def test_cutctx_telemetry_on_keeps_collector_enabled(self, monkeypatch):
