...
was retired in favor of the Rust-backed shim (`cutctx._core` via PyO3).
...
`crates/cutctx-core/tests`. Public-API tests (anything calling
...
def _cfg_below_threshold():
    ...
    from cutctx.transforms.diff_compressor import DiffCompressorConfig
...
class TestRoutingGapDetectorScanWindow:
    ...
    def test_detect_picks_up_diff_after_long_commit_message(self):
        from cutctx.transforms.content_detector import (
            ContentType,
            detect_content_type,
        )
