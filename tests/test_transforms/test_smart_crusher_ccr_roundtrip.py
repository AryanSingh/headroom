...
The Rust core integration test (`crates/cutctx-core/tests/ccr_roundtrip.rs`)
...
def _build_extension() -> None:
    try:
        ...
        pytest.skip(
            "cutctx._core not built — run `bash scripts/build_rust_extension.sh`",
...
def _force_lossy_config():
    ...
    from cutctx._core import SmartCrusherConfig
...
def test_native_default_crusher_has_a_store() -> None:
    ...
    from cutctx._core import SmartCrusher
...
def test_shim_lossy_crush_populates_store() -> None:
    ...
    `cutctx.transforms.smart_crusher.SmartCrusher` shim — the path
