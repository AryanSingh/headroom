...
from cutctx.transforms.tag_protector import (
    KNOWN_HTML_TAGS,
    _is_html_tag,
    protect_tags,
    restore_tags,
...
class TestRestoreTags:
    ...
    def test_lost_placeholder_discards_wrap(self):
        ...
        protected = [("{{CUTCTX_TAG_0}}", "<tag>data</tag>")]
    ...
    def test_partial_loss_keeps_present_discards_lost(self):
        ...
        protected = [
            ("{{CUTCTX_TAG_0}}", "<a>1</a>"),
            ("{{CUTCTX_TAG_1}}", "<lost>x</lost>"),
        ]
        result = restore_tags("head {{CUTCTX_TAG_0}} tail", protected)
...
class TestBugFixesPhase3e4:
    ...
    def test_fixed_in_3e4_placeholder_collision_avoided(self):
        """Bug #5: input contains a literal `{{CUTCTX_TAG_…}}`
        ...
        assert protected[0][0] != "{{CUTCTX_TAG_0}}"
