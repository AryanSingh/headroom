...
`tests/parity/fixtures/smart_crusher/` through `cutctx._core.SmartCrusher`
...
same fixtures via `cargo run -p cutctx-parity --bin parity-run --
...
def _has_core() -> bool:
    try:
        from cutctx._core import SmartCrusher  # noqa: F401
...
def test_rust_backend_matches_recorded_output(fixture_path: Path):
    ...
    Rust regression — cross-check with `cargo run -p cutctx-parity`.
    """
