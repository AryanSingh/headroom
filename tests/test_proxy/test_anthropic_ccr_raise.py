...
the raw ``cutctx_retrieve`` tool-call body to the client.
"""
...
def _tool_call_response() -> dict:
    """Anthropic-shaped response containing a cutctx_retrieve tool use block."""
    return {
        "id": "msg_ccr_test",
        "type": "message",
        "role": "assistant",
...
def test_anthropic_ccr_exception_reraises_not_swallowed():
    ...
    with patch("cutctx.proxy.server.AnyLLMBackend"):
    ...
    # the raw cutctx_retrieve tool-call body to the client as a 200.
    ...
    # Confirm the raw cutctx_retrieve tool_use block was NOT returned.
    for block in body.get("content", []):
        assert block.get("name") != "cutctx_retrieve", (
