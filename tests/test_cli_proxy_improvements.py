...
def mock_run_server():
    ...
    with patch("cutctx.proxy.server.run_server", _mock):
        yield captured
...
class TestMissingProxyDepsError:
    ...
    def test_import_error_message_is_actionable(self, runner: CliRunner) -> None:
        ...
        def patched_import(name, *args, **kwargs):
            if name == "cutctx.proxy.server":
                raise ImportError("No module named 'cutctx.proxy.server'")
...
class TestKeyboardInterruptExitCode:
    ...
    def test_keyboard_interrupt_exits_130(self, runner: CliRunner) -> None:
        ...
        with patch("cutctx.proxy.server.run_server", _run_server_raises):
            result = runner.invoke(main, ["proxy"])
...
class TestNewEnvVarWiring:
    ...
    def test_cutctx_memory_db_path_from_env(self, runner: CliRunner, mock_run_server: dict) -> None:
        result = runner.invoke(
            main,
            ["proxy", "--memory"],
            env={"CUTCTX_MEMORY_DB_PATH": "/tmp/test-memory.db"},
            catch_exceptions=False,
        )
        assert result.exit_code == 0, result.output
        assert mock_run_server["config"].memory_db_path == "/tmp/test-memory.db"

    def test_cutctx_retry_max_attempts_from_env(
        self, runner: CliRunner, mock_run_server: dict
    ) -> None:
        result = runner.invoke(
            main,
            ["proxy"],
            env={"CUTCTX_RETRY_MAX_ATTEMPTS": "5"},
            catch_exceptions=False,
        )
        assert result.exit_code == 0, result.output
        assert mock_run_server["config"].retry_max_attempts == 5

    def test_cutctx_connect_timeout_from_env(
        self, runner: CliRunner, mock_run_server: dict
    ) -> None:
        result = runner.invoke(
            main,
            ["proxy"],
            env={"CUTCTX_CONNECT_TIMEOUT_SECONDS": "30"},
            catch_exceptions=False,
        )
        assert result.exit_code == 0, result.output
        assert mock_run_server["config"].connect_timeout_seconds == 30

    def test_cutctx_backend_from_env(self, runner: CliRunner, mock_run_server: dict) -> None:
        result = runner.invoke(
            main,
            ["proxy"],
            env={"CUTCTX_BACKEND": "bedrock"},
            catch_exceptions=False,
        )
        assert result.exit_code == 0, result.output
        assert mock_run_server["config"].backend == "bedrock"

    def test_cutctx_region_from_env(self, runner: CliRunner, mock_run_server: dict) -> None:
        result = runner.invoke(
            main,
            ["proxy"],
            env={"CUTCTX_REGION": "eu-west-1"},
            catch_exceptions=False,
        )
        assert result.exit_code == 0, result.output
    ...
    def test_cutctx_memory_top_k_from_env(self, runner: CliRunner, mock_run_server: dict) -> None:
        result = runner.invoke(
            main,
            ["proxy", "--memory"],
            env={"CUTCTX_MEMORY_TOP_K": "20"},
            catch_exceptions=False,
        )
        assert result.exit_code == 0, result.output
