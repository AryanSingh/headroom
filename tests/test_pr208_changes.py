...
- _cutctx_log_dir lazy resolution via paths module
...
class TestJitterDelayMs:
    """Tests for cutctx.proxy.helpers.jitter_delay_ms."""
...
# ---------------------------------------------------------------------------
# _cutctx_log_dir lazy resolution tests
# ---------------------------------------------------------------------------


class TestCutctxLogDir:
    """Tests for _cutctx_log_dir using cutctx.paths.log_dir."""

    def test_log_dir_respects_workspace_env_var(
        self, monkeypatch: pytest.MonkeyPatch, tmp_path: Path
    ) -> None:
        from cutctx.proxy.helpers import _cutctx_log_dir

        monkeypatch.setenv("CUTCTX_WORKSPACE_DIR", str(tmp_path))
        result = _cutctx_log_dir()
...
class TestIsAnthropicAuth:
    """Tests for cutctx.proxy.helpers.is_anthropic_auth."""

    def test_detects_x_api_key(self) -> None:
        from cutctx.proxy.helpers import is_anthropic_auth
...
class TestSetupFileLogging:
    """Tests for _setup_file_logging using the new _cutctx_log_dir path."""

    def test_setup_file_logging_creates_log_dir(
        self, monkeypatch: pytest.MonkeyPatch, tmp_path: Path
    ) -> None:
        from cutctx.proxy.helpers import _setup_file_logging
        ...
        cutctx_logger.handlers = [
            h for h in cutctx_logger.handlers if not isinstance(h, RotatingFileHandler)
        ]

        _setup_file_logging()

        # Verify a RotatingFileHandler was added
        has_rotating = any(isinstance(h, RotatingFileHandler) for h in cutctx_logger.handlers)
    ...
    def test_setup_file_logging_handles_oserror(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        # Monkey-patch _cutctx_log_dir to return a path that will cause OSError
        def _bad_log_dir():
            return Path("/nonexistent/deeply/nested/path/that/cannot/exist/___test___")

        import cutctx.proxy.helpers as helpers_mod

        monkeypatch.setattr(helpers_mod, "_cutctx_log_dir", _bad_log_dir)
        # Should not raise
        _setup_file_logging()

    def test_importing_server_does_not_install_file_handler(self, tmp_path: Path) -> None:
        """Importing cutctx.proxy.server must NOT attach a RotatingFileHandler
        ...
        script = textwrap.dedent(
            ...
            import logging
            from logging.handlers import RotatingFileHandler
            import cutctx.proxy.server  # noqa: F401
        ...
        env = {**os.environ, "CUTCTX_WORKSPACE_DIR": str(tmp_path)}
        ...
        assert result.stdout.strip() == "CLEAN", (
            f"Importing cutctx.proxy.server attached a file handler: {result.stdout!r}"
        )
...
class TestWrapGetLogPath:
    """Tests for _get_log_path in wrap.py using cutctx.paths."""

    def test_get_log_path_returns_proxy_log(
        self, monkeypatch: pytest.MonkeyPatch, tmp_path: Path
    ) -> None:
        from cutctx.cli.wrap import _get_log_path
    ...
    def test_get_log_path_creates_directory(
        ...
        monkeypatch.setenv("CUTCTX_WORKSPACE_DIR", str(log_subdir))
        result = _get_log_path()
