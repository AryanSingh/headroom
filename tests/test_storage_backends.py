...
from cutctx.storage import JSONLStorage, SQLiteStorage, Storage, create_storage
...
def test_create_storage_builtin_entrypoint_and_fallback(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda group: [FakeEntryPoint()] if group == "cutctx.storage_backend" else [],
    )
    ...
    monkeypatch.setattr("cutctx.storage.SQLiteStorage", FakeSQLiteStorage)
