...
from cutctx.config import CutctxConfig, CutctxMode, TransformResult
from cutctx.hooks import CompressionHooks
from cutctx.pipeline import (
...
from cutctx.transforms import ContentRouter, TransformPipeline
...
def test_pipeline_extension_manager_replaces_events_and_ignores_failures(caplog):
    ...
    with caplog.at_level("WARNING", logger="cutctx.pipeline"):
        event = manager.emit(
...
def test_discover_pipeline_extensions_handles_load_and_init_failures(monkeypatch):
    pipeline_module = importlib.import_module("cutctx.pipeline")
...
def test_cutctx_client_emits_canonical_pipeline_events(tmp_path):
    recorder = RecordingExtension()
    original = DummyOriginalClient()
    config = CutctxConfig(
        store_url=f"jsonl://{tmp_path / 'cutctx.jsonl'}",
    ...
    client = CutctxClient(
        ...
        store_url=f"jsonl://{tmp_path / 'cutctx-client.jsonl'}",
