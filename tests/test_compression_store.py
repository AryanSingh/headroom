...
@contextmanager
def _capture_cutctx_retrieve_events():
    events: list[dict[str, Any]] = []
    prefix = "event=cutctx_retrieve "
    ...
    logger = logging.getLogger("cutctx.cache.compression_store")
...
def test_retrieve_logs_payload_preview():
    ...
    with _capture_cutctx_retrieve_events() as events:
...
class TestCompressionStoreFeedback:
    ...
    @patch("cutctx.cache.compression_feedback.get_compression_feedback")
    @patch("cutctx.telemetry.get_telemetry_collector")
    @patch("cutctx.telemetry.toin.get_toin")
