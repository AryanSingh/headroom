...
def test_merge_helper_only_cutctx() -> None:
    assert merge_anthropic_beta(None, ["a", "b"]) == "a,b"


def test_merge_helper_preserves_client_order_and_appends_cutctx() -> None:
    # Client tokens FIRST in their original order, cutctx AFTER in passed order.
    assert (
        merge_anthropic_beta("client-1,client-2", ["cutctx-a", "cutctx-b"])
        == "client-1,client-2,cutctx-a,cutctx-b"
    )


def test_dedupe_case_insensitive_preserves_first_casing() -> None:
    # Client casing wins over cutctx casing.
    assert (
...
def _isolate_tracker(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.delenv("CUTCTX_BETA_HEADER_STICKY", raising=False)
    monkeypatch.delenv("CUTCTX_BETA_TRACKER_MAX_SESSIONS", raising=False)
    _reset_session_beta_tracker_for_test()
    yield
    _reset_session_beta_tracker_for_test()
...
def test_client_value_preserved_when_no_injection() -> None:
    """First-turn client value flows through unchanged when no cutctx adds."""
    tracker = SessionBetaTracker(max_sessions=10)
...
def test_disabled_mode_passes_through(monkeypatch: pytest.MonkeyPatch) -> None:
    """`CUTCTX_BETA_HEADER_STICKY=disabled` returns client value verbatim."""
    monkeypatch.setenv("CUTCTX_BETA_HEADER_STICKY", "disabled")
    tracker = SessionBetaTracker(max_sessions=10)
...
def test_disabled_mode_invalid_value_raises(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("CUTCTX_BETA_HEADER_STICKY", "yolo")
    with pytest.raises(ValueError, match="CUTCTX_BETA_HEADER_STICKY"):
        get_beta_header_sticky_mode()
...
def test_max_sessions_env_var_custom(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("CUTCTX_BETA_TRACKER_MAX_SESSIONS", "42")
    assert get_beta_tracker_max_sessions() == 42


def test_max_sessions_env_var_invalid_raises(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("CUTCTX_BETA_TRACKER_MAX_SESSIONS", "0")
    with pytest.raises(ValueError):
        get_beta_tracker_max_sessions()
    monkeypatch.setenv("CUTCTX_BETA_TRACKER_MAX_SESSIONS", "-3")
    with pytest.raises(ValueError):
        get_beta_tracker_max_sessions()
    monkeypatch.setenv("CUTCTX_BETA_TRACKER_MAX_SESSIONS", "not-int")
    with pytest.raises(ValueError):
        get_beta_tracker_max_sessions()
...
def test_memory_injection_appends_deterministic_order() -> None:
    ...
    new merge helper. Order is client first, cutctx token after.
    """
