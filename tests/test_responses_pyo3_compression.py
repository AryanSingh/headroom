...
def _ensure_binding():
    ...
    try:
        from cutctx._core import compress_openai_responses_live_zone

        return compress_openai_responses_live_zone
    except ImportError:
        pytest.skip("cutctx._core not built — run scripts/build_rust_extension.sh")
...
class TestModelDefault:
    """Empty `model` defaults to `cutctx_core`'s `DEFAULT_MODEL`."""
...
class TestTelemetryFields:
    ...
    without recounting tokens. See `crates/cutctx-core/src/transforms/
