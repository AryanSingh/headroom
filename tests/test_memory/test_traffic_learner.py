...
from cutctx.memory.traffic_learner import (
    ExtractedPattern,
    PatternCategory,
    TrafficLearner,
...
class TestCommandsRelatedAsRetry:
    ...
    def test_same_binary_unrelated_args_rejected(self):
        ...
        failed = (
            'grep -nE "smoke|CUTCTX_SMOKE_TEST_TIMEOUT|smoke_test" '
...
class TestCommandsRelatedAsRetrySubstantiveToken:
    def test_substantive_token_beats_distance(self):
        ...
        # share the substantive token "cutctx-config", so the token-overlap
        # path accepts the pair.
        failed = "python -m foo --cutctx-config=/etc/h.toml"
        success = "python -m bar --cutctx-config=/etc/h.toml --extra"
...
class TestProjectForPattern:
    def _project(self, path: str):
        ...
        from cutctx.learn.models import ProjectInfo
...
class TestPatternsToRecommendations:
    def test_routes_preference_to_memory_file(self):
        from cutctx.learn.models import RecommendationTarget
...
class TestFlushDebounce:
    ...
    async def test_flush_worker_rate_limits(self, monkeypatch):
        ...
        from cutctx.memory import traffic_learner as tl_mod
...
def _install_plugin_registry(monkeypatch, plugin):
    """Stub out cutctx.learn.registry so flush_to_file uses our fake."""
    import sys
    import types as _types

    fake = _types.ModuleType("cutctx.learn.registry")
    ...
    monkeypatch.setitem(sys.modules, "cutctx.learn.registry", fake)
...
class TestBackendResolution:
    def test_resolve_none_backend(self):
        from cutctx.memory.traffic_learner import _resolve_backend_db_path
