...
from cutctx.proxy import memory_handler as memory_handler_module
...
async def test_execute_memory_tool_save_and_background_dedup(
    ...
    monkeypatch.setattr("cutctx.proxy.memory_handler.asyncio.create_task", fake_create_task)
    backend.search_results = [
        make_result(
            "other",
...
async def test_init_and_import_bridge_success_and_failure(
    ...
    monkeypatch.setitem(
        sys.modules,
        "cutctx.memory.bridge",
        SimpleNamespace(MemoryBridge=FakeMemoryBridge),
    )
    monkeypatch.setitem(
        sys.modules,
        "cutctx.memory.bridge_config",
        SimpleNamespace(
...
def test_memory_handler_init_defaults_and_tool_injection_edges(
    ...
    monkeypatch.setattr(
        "cutctx.paths.native_memory_dir",
        lambda: native_dir,
    )
...
async def test_ensure_initialized_fast_paths_and_qdrant_variants(
    ...
    monkeypatch.setitem(
        sys.modules,
        "cutctx.memory.backends.direct_mem0",
    ...
    def fake_import(name, globals=None, locals=None, fromlist=(), level=0):
        if name == "cutctx.memory.backends.direct_mem0":
