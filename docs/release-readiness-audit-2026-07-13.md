...
| --- | --- | --- | --- |
| Medium — release evidence scope | The Docker provider-routing smoke now passes, but the no-secret image run intentionally used passthrough mode and did not prove in-container compression or fallback with retained trace artifacts. | Provider routing, auth injection, health, stats, dashboard, and assets are verified; compression/fallback remain covered by local integration tests. | Repeat the container scenario with an authorized provider plus redacted compression and fallback trace artifacts before making a full provider-runtime deployment claim. |
