...
The Rust proxy already has a `crates/cutctx-core/src/transforms/log_compressor.rs`
...
2. Injects the `<!-- cutctx:rtk-instructions -->` block into the
...
See `cutctx/cli/wrap/` for the per-agent shims.
...
- `cutctx/cli/wrap/` — wrap-CLI implementation.
- `crates/cutctx-core/src/transforms/log_compressor.rs` — the
