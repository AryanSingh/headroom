...
| **CC6.2** Authentication Mechanisms | Enterprise customers authenticate through OIDC/JWKS or RFC 7662 introspection, supporting identity providers such as Okta, Azure AD, and Google Workspace. JWT signature validation requires `PyJWT[crypto]`; an installation without it rejects SSO validation rather than bypassing verification. SSO-authenticated admins can enroll TOTP MFA and operators can enforce a fresh code with `CUTCTX_MFA_ENFORCE=1`; an unavailable enrollment store fails closed. Team and Business customers authenticate via signed API key. SAML is not implemented or marketed. Audit-event emission for `auth.login`, `auth.failed`, and `auth.key_rotated` remains a separate implementation gap. | `cutctx_ee/sso.py` (OIDC/JWKS validator); `cutctx/security/mfa.py`; `cutctx/proxy/routes/mfa.py`; `cutctx/proxy/server.py` (admin auth and MFA enforcement) | `cutctx_ee/sso.py`, `cutctx/security/mfa.py`, `cutctx/proxy/routes/mfa.py`, `cutctx/proxy/server.py` |
...
| Authentication | **Partial** | OIDC/JWKS, token introspection, API-key authentication, and opt-in TOTP MFA enforcement for enrolled SSO admins are implemented. A missing PyJWT dependency fails SSO validation closed. SAML is not implemented or marketed; live IdP interoperability evidence remains required before an enterprise-launch claim. |
