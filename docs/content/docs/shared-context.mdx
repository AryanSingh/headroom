...
Under the hood, `put()` calls `headroom.compress()` -- the same pipeline used by the CutCtx proxy -- and stores the original in memory. `get()` returns the compressed version. `get(full=True)` returns the original.
