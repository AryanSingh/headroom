...
`OPENAI_API_KEY` is not required for this adapter's Cutctx save, search, or
cleanup operations. The backend connects to the configured Neo4j service and
initializes Graphiti indices on first use. Any credentials required by a
separately configured Graphiti extraction workflow remain that workflow's
responsibility.
...
(`group_id`) before it reaches Graphiti. Raw user and session IDs are
not used as the Graphiti group ID. Searches are restricted to that partition;
do not rely on this backend to query across users or sessions.
