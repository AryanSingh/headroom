---
title: Quickstart
description: Get Cutctx running in 5 minutes as a local-first context control plane. Install, route context through it, and start measuring governed savings.
---

This guide gets you from zero to routed, observable LLM calls in under 5 minutes. By the end, Cutctx is sitting in the request path so you can govern context, measure savings, and keep reversible access to originals.
...
The TypeScript SDK sends messages to a local Cutctx proxy for compression. Start the proxy before using the TS SDK:
...
If you do not want to change any code, run Cutctx as a proxy and point your existing client at it:
...
All requests flow through Cutctx automatically. Check savings at any time:
...
The biggest savings come from tool outputs -- search results, database rows, log files, API responses. Cutctx auto-detects the content type and routes it to the most appropriate compressor. No configuration needed.
