...
Use the managed `cutctx install stop` / `restart` commands, or a normal
container stop (`docker compose stop proxy`), rather than forcibly killing the
proxy. A graceful stop flushes and compacts the asynchronous savings journal.
`SIGKILL`, OOM termination, and power loss can lose monitoring-only savings
updates from the most recent 250 ms persistence window; requests and model
responses are not affected. See [Metrics & Monitoring](/docs/metrics#savings-durability)
for the complete durability contract.
