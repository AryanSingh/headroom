...
description: Run Cutctx without installing Python or Node.js on the host. A native `cutctx` wrapper keeps the proxy and CLI in Docker while your other tools run on the host OS.
---
...
- use a native Python install from the installation docs
...
3. Installs `cutctx` and compatibility `cutctx` wrappers into `~/.local/bin` or `~/bin`.
...
- container-side prep writes Cutctx config, memory, and selected CLI context-tool setup into mounted host files
...
The Docker-native `cutctx` wrapper exposes the persistent Docker lifecycle directly:
...
The `proxy` service uses `restart: unless-stopped`, so compose can act as the always-on Docker runtime for Cutctx:
...
- **`CUTCTX_WORKSPACE`** (host-side) is the directory the compose file bind-mounts into the container as `/workspace`. It behaves like CWD in a native (non-Docker) run.
- **`CUTCTX_WORKSPACE_DIR`** (inside the container) is the canonical Cutctx state root from the [filesystem contract](/docs/filesystem-contract). The compose file sets it to `/tmp/cutctx-home/.cutctx` so the proxy resolves savings, logs, TOIN, and memory under the bind-mounted `${HOME}/.cutctx`.

You do not need to set `CUTCTX_WORKSPACE_DIR` manually when using the shipped compose file — it is already in the `environment:` block.
</Callout>
...
- For persistent service and task installs, use the Python-native `cutctx install ...` workflow — see [Persistent Installs](/docs/persistent-installs).
