...
3. Installs `cutctx` and compatibility `headroom` wrappers into `~/.local/bin` or `~/bin`.
...
- **`HEADROOM_WORKSPACE_DIR`** (inside the container) is the canonical CutCtx state root from the [filesystem contract](/docs/filesystem-contract). The compose file sets it to `/tmp/headroom-home/.headroom` so the proxy resolves savings, logs, TOIN, and memory under the bind-mounted `${HOME}/.headroom`.
...
- For Docker-native `cutctx install ...`, the wrapper persists its profile manifest under `~/.headroom/deploy/<profile>/`.
