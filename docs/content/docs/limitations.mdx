---
title: Limitations
description: When Cutctx helps, when it does not, and what to watch out for. Honest documentation of compression constraints and safety gates.
---

Cutctx is designed to compress LLM context without losing accuracy. This page documents when it helps, when it does not, and the safety gates that prevent harmful compression.

## When Cutctx Helps vs. Does Not
...
### Where Cutctx Adds the Most Value
...
### Where Cutctx Adds Little Value
...
Cutctx includes an AST-aware CodeCompressor (tree-sitter, 8 languages) but it is gated behind safety protections that prevent it from firing in most real-world scenarios. This is intentional.
...
## Evaluation Limits and Honest Claims

- Compression-preservation evaluations compare answers from original and
  compressed contexts. They do not establish a model's absolute benchmark
  accuracy or prove a cross-provider winner.
- A subscription CLI smoke test verifies an authenticated execution route, not
  statistically meaningful quality. Publish its corpus version, sample count,
  prompt policy, and scoring method separately from API-backed results.
- Tool-use evaluations require schema-aware or executable validation. Text
  similarity to an expected tool call is not sufficient evidence; BFCL remains
  excluded from product-quality claims until that evaluator is available.
- Reports with a failed dataset loader, stale manifest, missing provenance, or
  unverified baseline must be removed or labeled unavailable rather than used
  in marketing or release decisions.
