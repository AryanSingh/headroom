...
Cutctx uses a unified GitHub Actions workflow (`.github/workflows/release.yml`) that automatically publishes all three packages, builds version-matched Docker images, generates changelogs from conventional commits, and creates GitHub Releases with package assets attached.
...
|---------|------|----------|----------------------|
| `cutctx-ai` | Python | PyPI | `PYPI_PACKAGE` |
| `cutctx-ai` | TypeScript SDK | npmjs.org | `NPM_SDK_PACKAGE` |
| `cutctx-openclaw` | TypeScript plugin | npmjs.org | `NPM_OPENCLAW_PACKAGE` |
| `@{owner}/cutctx-ai` | TypeScript SDK | GitHub Package Registry | — |
| `@{owner}/cutctx-openclaw` | TypeScript plugin | GitHub Package Registry | — |
| `cutctx-ai-{version}.tar.gz` / `cutctx_ai-{version}-py3-none-any.whl` | Python package distributions | GitHub Release (`{owner}/cutctx`) | — |
| `cutctx-ai-{version}.tgz` / `cutctx-openclaw-{version}.tgz` | Node release assets | GitHub Release (`{owner}/cutctx`) | — |
| `ghcr.io/{owner}/cutctx` | Docker image | GitHub Container Registry | — |
...
- `cutctx/_version.py` — `__version__` — synced at build time
...
- `sdk/typescript/` as `cutctx-ai`
- `plugins/openclaw/` as `cutctx-openclaw`
...
- `sdk/typescript/` as `@{owner}/cutctx-ai`
- `plugins/openclaw/` as `@{owner}/cutctx-openclaw`
...
```yaml
env:
  PYPI_PACKAGE: cutctx-ai
  PYPI_ENVIRONMENT: pypi
  NPM_REGISTRY_URL: https://registry.npmjs.org
  NPM_SDK_PACKAGE: cutctx-ai
  NPM_OPENCLAW_PACKAGE: cutctx-openclaw
...
TypeScript packages (`cutctx-ai` SDK and `cutctx-openclaw`) start at version `0.1.0`, while Python `cutctx-ai` is at `0.5.25`. Before the first automated release:
