---
title: MCP Tools
description: Compression, retrieval, and stats as MCP tools for Claude Code, the Claude Desktop app, Cursor, and any MCP-compatible host.
---

Cutctx's MCP server exposes compression, retrieval, and observability as tools that any MCP-compatible AI coding tool can call -- Claude Code, the Claude Desktop app, Cursor, Codex, and more. No proxy required.
...
The original is stored locally for 1 hour. If the LLM needs the full content later, it calls `cutctx_retrieve`.
...
Sub-agent stats are aggregated via a shared stats file at `~/.cutctx/session_stats.jsonl`.
...
For MCP hosts that let you configure a local stdio server, point them at `cutctx mcp serve`. If you also run the proxy, pass the proxy URL explicitly so retrieval and stats come from the intended proxy instance.
...
| Claude Desktop app | Native | `cutctx mcp install` · `--gateway` auto-compresses MCP tool outputs |
...
The LLM calls `cutctx_compress` on demand. Compression happens locally in the MCP process. Originals are stored in a local `CompressionStore` with 1-hour TTL.
...
`cutctx_retrieve` checks the local store first, then falls back to the proxy's store.

## Troubleshooting

**"MCP SDK not installed"** -- Run `pip install "cutctx-ai[mcp]"`.

**"Proxy not running"** -- Start the proxy with `cutctx proxy` in another terminal. Only needed for proxy-backed retrieval.

**"Entry not found or expired"** -- Local content expires after 1 hour, proxy content after 5 minutes.

**Claude doesn't see Cutctx tools** -- Run `cutctx mcp status`, restart Claude Code, and verify with `/mcp` inside Claude Code.

### Claude Code `/usage` attributes a large share to `cutctx` MCP

Claude Code counts MCP tool calls and MCP tool results as session context. If a
long-running workflow or a subagent-heavy command calls `cutctx_compress`,
`cutctx_retrieve`, or `cutctx_stats` many times, `/usage` can show a visible
share under the `cutctx` MCP server even when Cutctx is saving tokens inside
individual tool results.

That number is not a direct "Cutctx overhead" bill. It means Claude Code kept
Cutctx MCP interactions in the conversation context. Deep research workflows
...
- Run `cutctx_stats` and compare `tokens_saved` with the number of MCP calls.
...
- Prefer the proxy path for automatic compression of normal Claude Code traffic:
  `cutctx proxy` plus `ANTHROPIC_BASE_URL=http://127.0.0.1:8787 claude`.
- Disable the MCP server for sessions where you only want proxy-level
  compression and do not need on-demand `cutctx_compress` or
  `cutctx_retrieve`.
