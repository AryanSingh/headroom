...
Cutctx's CCR (Compress-Cache-Retrieve) architecture makes compression **reversible**. When content is compressed, the original data is cached locally. If the LLM needs the full data, it retrieves it instantly.
...
The client never sees CCR tool calls -- they are handled transparently by Cutctx.
...
import type { CCRConfig } from "cutctx-ai";
...
const ccrConfig: CCRConfig = {
  enabled: true,
  injectTool: true,             // Inject cutctx_retrieve tool
...
# The LLM calls cutctx_retrieve when it needs more data.
...
agent runs, set `CUTCTX_CCR_TTL_SECONDS` before starting the proxy:

```bash
CUTCTX_CCR_TTL_SECONDS=7200 cutctx proxy
```
