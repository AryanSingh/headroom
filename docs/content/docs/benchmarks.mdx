---
title: Benchmarks
description: Compression performance, accuracy preservation, latency overhead, and real-world production telemetry from 250+ Cutctx proxy instances.
---

Cutctx's core promise: compress context without losing accuracy. This page covers compression benchmarks, accuracy evaluations, latency overhead, and production telemetry.
...
## Current Provider-Backed Evaluation

The evaluation goal is **preservation**: does a provider answer equivalently
when given the original context and when given Cutctx-compressed context? This
is not the same as absolute benchmark accuracy, and reports must state that
distinction.

| Benchmark | Task | Cases | Raw-to-compressed preservation | Compression |
|---|---|---:|---:|---:|
| SQuAD v2 | question answering | 100 | 100% | 5% |
| HotpotQA | multi-hop QA | 50 | 100% | 18% |
| CodeSearchNet | code understanding | 50 | 100% | 30% |

The CodeSearchNet run uses the maintained
`code-search-net/code_search_net` corpus and streams samples, so a bounded
evaluation does not download an entire language split. The saved reports are
versioned local evidence, not a public cross-vendor performance claim.

### Subscription CLI Pilot

Claude Code and Codex CLI can be evaluated through their existing interactive
subscription authentication, without putting provider API credentials in the
shell. The initial pilot ran one original/compressed case each from SQuAD v2,
BFCL, HotpotQA, HumanEval, and LongBench/Qasper through both CLIs with zero
transport failures. It validates the execution route only; one sample per task
is not a statistically powered benchmark.

Use the following procedure for a consented local evaluation:

1. Run each provider in an isolated directory with no proxy and a deterministic
   prompt. Claude uses `claude -p ... --output-format json --no-session-persistence`;
   Codex uses `codex exec --ephemeral --ignore-rules --skip-git-repo-check -C /tmp -s read-only`.
2. Send each case twice: original context and compressed context. Store only
   redacted outputs, model identifiers when returned, task scores, and context
   size—not credentials, prompts containing customer data, or session files.
3. Score SQuAD/HotpotQA against accepted answers, execute HumanEval tests, and
   validate BFCL calls structurally against the supplied tool schema. Compare
   baseline and compressed scores before reporting a compression delta.
4. Publish corpus version, sample count, provider route, scoring method, and
   limitations alongside each report. Never combine subscription and API
   results without labeling their provenance.

<Callout type="warning" title="BFCL is not a release-quality signal yet">
The previous BFCL text/F1 evaluator scored baseline answers such as natural
language results instead of validated calls. Its 30% result is a scoring/prompt
validity finding, not evidence that compression reduced tool-use quality. Keep
BFCL out of product claims until structured execution is implemented.
</Callout>

## Historical Component Benchmarks
...
Real-world data from **50,000+ proxy sessions** across 250+ unique instances (March--April 2026). Collected via anonymous telemetry (opt-out: `CUTCTX_TELEMETRY=off`).
