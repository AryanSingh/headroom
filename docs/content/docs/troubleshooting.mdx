...
description: Solutions for common Cutctx issues including proxy startup, connection errors, no token savings, high latency, and installation problems.
---
...
### Repeated “history compress failed” warnings

**Symptom**: OpenCode or another coding harness repeatedly prints:

```text
cutctx: history compress failed, passing through Invalid or missing admin credentials.
```

**Cause**: The harness plugin is calling an agent compression route without
the dedicated client credential. Ordinary compression must use
`CUTCTX_API_KEY`, not the root admin key.

**Fix**:

```bash
# Local proxy: generate/store the credential and restart the harness
cutctx setup

# Remote proxy: enter the provisioned key once
...
Always launch supported harnesses through `cutctx wrap <harness>`. For another
CLI, use `cutctx auth exec --proxy-url URL -- COMMAND`. See
[Client Authentication](/docs/authentication).

### Credential is invalid or expired

Run `cutctx auth status --proxy-url URL`. If the state is `invalid` or
`expired`, run `cutctx auth login --proxy-url URL` again. Cutctx does not
automatically delete the old value, so an operator can distinguish confirmed
revocation from a temporarily unreachable proxy.

### Secure credential storage is unavailable

Interactive setup requires macOS Keychain, Windows Credential Manager, or
Linux Secret Service and does not silently write plaintext credentials.
Configure the platform credential manager, or use `CUTCTX_API_KEY` and
...
**Symptom**: After pointing Claude Code at Cutctx (`ANTHROPIC_BASE_URL`), `/context all`
...
```bash
# Easiest: `cutctx wrap claude` sets ENABLE_TOOL_SEARCH=true automatically.
cutctx wrap claude
...
cutctx wrap claude --tool-search auto
...
See [issue #746](https://github.com/cutctx/cutctx/issues/746) for the full analysis.
...
### pipx installs an older Cutctx version

**Symptom**: PyPI shows a newer `cutctx-ai` release, but `pipx install` or
...
environment uses a Python version that Cutctx does not publish wheels for yet,
...
If you already have Cutctx installed under `pipx`, uninstall it first or
...
4. File an issue at [github.com/cutctx/cutctx](https://github.com/cutctx/cutctx/issues) with your Cutctx version, Python version, provider, debug log output, and minimal reproduction code
