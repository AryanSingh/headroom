---
title: API Reference
description: Complete API reference for the Cutctx Python and TypeScript SDKs. Core client, configuration types, result types, errors, and utilities.
---

Complete API reference for the Cutctx Python and TypeScript SDKs.

## Core

### CutctxClient

The main entry point for the Cutctx SDK.

<Tabs groupId="lang" items={['TypeScript', 'Python']}>
<Tab value="TypeScript">

<TypeTable type={{
  baseUrl: { type: 'string', description: 'Base URL for the Cutctx proxy' },
...
Accepts all standard OpenAI/Anthropic parameters plus Cutctx-specific overrides:

<TypeTable type={{
  cutctx_mode: { type: '"audit" | "optimize" | "simulate"', description: 'Override mode for this request', default: 'Client default' },
  cutctx_query: { type: 'str', description: 'Query for relevance scoring', default: 'None' },
  cutctx_output_buffer_tokens: { type: 'int', description: 'Reserve tokens for output', default: '4000' },
  cutctx_keep_turns: { type: 'int', description: 'Keep last N turns uncompressed', default: '2' },
  cutctx_tool_profiles: { type: 'dict', description: 'Per-tool compression overrides', default: '{}' },
}} />
...
Top-level function to compress messages via the Cutctx proxy.
...
<TypeTable type={{
  ...
  fallback: { type: 'boolean', description: 'Return original messages on failure instead of throwing' },
  retries: { type: 'number', description: 'Number of retry attempts on failure' },
  client: { type: 'CutctxClientInterface', description: 'Pre-configured client instance to use' },
...
Context management is now automatic. Cutctx handles rolling window behavior internally based on the model's context limit and the `cutctx_keep_turns` / `cutctx_output_buffer_tokens` per-request overrides. The `RollingWindowConfig`, `IntelligentContextConfig`, and `ScoringWeights` classes no longer exist as public API.
...
<TypeTable type={{
  ...
  defaultMode: { type: 'CutctxMode', description: 'Default compression mode' },
...
# Note: rolling_window was removed in 0.9.x — use cutctx_keep_turns per-request instead
```
...
Semantic similarity scoring (requires `cutctx-ai[relevance]`):
...
| Exception | Meaning |
|-----------|---------|
| `CutctxError` | Base class for all errors |
| `CutctxConnectionError` | Cannot reach proxy |
| `CutctxAuthError` | 401 from proxy |
| `CutctxCompressError` | Compression failed (includes `statusCode`, `errorType`) |
...
| Exception | Meaning |
|-----------|---------|
| `CutctxError` | Base class for all Cutctx errors |
...
The `/v1/orchestration/*` control-plane routes require the same admin authentication and RBAC checks as other protected management APIs. Send `X-Cutctx-Admin-Key` when API-key administration is enabled.

```bash
curl -H "X-Cutctx-Admin-Key: $CUTCTX_ADMIN_API_KEY" \
  http://127.0.0.1:8000/v1/orchestration/config

curl -X POST -H "X-Cutctx-Admin-Key: $CUTCTX_ADMIN_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"role":"worker","required_capabilities":["tool_calling"]}' \
  http://127.0.0.1:8000/v1/orchestration/route
```

`GET|PUT /config` reads or replaces layered configuration; `GET /providers` lists adapter specifications and configured accounts; credential `PUT|DELETE` routes store encrypted secret objects; provider `test` and model `refresh` perform authenticated discovery; `POST /route` previews a deterministic decision; and `GET /executions` returns redacted telemetry. Configuration responses mask custom-header values with `********`; sending that sentinel back preserves the stored value. Authentication headers and API keys must instead be supplied through the encrypted credential route, normally as `{"api_key":"...","headers":{"Authorization":"Bearer ..."}}`.

### Workload contracts and rollout evidence

Workload contracts are immutable, versioned routing intent. `GET /contracts`
lists persisted versions or read-only legacy contracts generated from existing
roles and bindings. `PUT /contracts/{id}/draft` materializes a new version and
requires `expected_revision` for optimistic concurrency. A draft is never live
merely because it was saved.

`POST /contracts/{id}/simulate` evaluates the contract supplied in the request
body against the same deterministic engine used by live routing. It performs no
provider call and does not mutate the active policy. The response contains live
and draft schema-v2 receipts, eligible and rejected candidates, the compiled
policy hash, and reliability-budget evidence.

Lifecycle actions are available under
`/contracts/{id}/versions/{version}`: `shadow`, `promote`, `pause`, and
`rollback`. Promotion from shadow to canary and canary to active returns `409`
until the version's evidence report is ready. Read or record bounded aggregate
signals at the `evidence` subresource. Raw routed savings and quality-safe
savings are reported separately; quality-safe savings include only samples
that met the contract quality floor.

`GET /receipts/{request_id}` returns an executed request using the same
schema-v2 decision vocabulary as simulation. Reliability budgets distinguish
connect, first-token, attempt, stream-idle, and total deadlines. The total
deadline bounds all retries and deployments, and streaming never switches
deployment after the first visible response byte.

Invalid payload shapes return `422`, invalid or unresolved policy returns `400`/`409`, and missing admin authorization returns `401`/`403`. `/v1/orchestration/execute` is absent unless `CUTCTX_ORCHESTRATION_DIRECT_EXECUTION=true`; it is a development diagnostic and not the production data plane. See [Orchestration platform](/docs/orchestration-platform) for schemas, layering, transport-account constraints, and deployment guidance.
