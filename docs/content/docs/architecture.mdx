...
Set `CUTCTX_REPLAY=1` to record compact proxy lifecycle events in a local SQLite journal. The journal keeps prompt-shape, compression, LLM-send, response, policy, and failure metadata so you can inspect a session through the local-admin-protected `GET /v1/sessions/{session_id}/replay` endpoint after the proxy restarts.
