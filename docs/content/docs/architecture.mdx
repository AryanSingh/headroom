...
Use the same local-admin credential with `GET /v1/sessions/{session_id}/state` to read deterministic operational state derived from the persisted timeline. The state includes event counts, event bounds, prompt and compression totals, sent and completed request counts, detected tool-call counts, policy counts, plus the latest model and pipeline stage. Tool arguments are never retained. It is a read model only; it does not change replay events or add retained request content.
