...
Cutctx provides specialized compressors for text-based content that isn't JSON or source code. Each one understands the structure of its content type and preserves what the LLM needs while dropping the noise.
...
```python
from cutctx.transforms import DiffCompressor
...
<Callout type="info" title="LLMLingua remains optional">
  LLMLingua is still available as an optional transform. Use Kompress and ContentRouter for the default text compression path.
</Callout>
