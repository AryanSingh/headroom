// Package headroom provides a Go client for the Headroom AI context compression proxy.
//
// Headroom reduces LLM API token usage by 60-95% by compressing prompt context
...
// Client wraps an HTTP client to route requests through a Headroom proxy.
type Client struct {
	baseURL    string
...
// NewClient creates a new Headroom client that routes requests through the given proxy URL.
...
func (c *Client) proxyRewrite(target *url.URL) (*url.URL, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("headroom: invalid proxy URL: %w", err)
	}
...
// Do executes an HTTP request through the Headroom proxy.
// The original request URL is passed to the proxy so it can forward to the correct upstream.
func (c *Client) Do(req *http.Request) (*http.Response, error) {
	if c.baseURL == "" {
		return nil, fmt.Errorf("headroom: ProxyURL is required")
	}
