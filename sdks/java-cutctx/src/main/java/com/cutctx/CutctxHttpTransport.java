...
/**
 * An HTTP interceptor that routes all requests through the Cutctx proxy.
 *
 ...
 *     .build();
 *
 * // All requests through httpClient will be proxied through Cutctx
 ...
 * }</pre>
 */
public class CutctxHttpTransport {

    private final CutctxClient client;

    /**
     * Creates a new transport interceptor.
     *
     * @param client The Cutctx client to route requests through
     */
    public CutctxHttpTransport(CutctxClient client) {
        if (client == null) {
    ...
    /**
     * Rewrites an HttpRequest to go through the Cutctx proxy.
     *
    ...
    /**
     * Sends a request through the Cutctx proxy.
     *
