...
about: Report results of testing `cutctx wrap copilot --subscription` on Linux/Windows/macOS
...
- **How you installed cutctx**: (pipx/pip `--pre` wheel · Docker install.sh/ps1 · built from source)
- **cutctx version**: (`cutctx --version`)
...
- **Command run**:
  ```
  cutctx wrap copilot --subscription -- --model gpt-4o -p "Reply with exactly: CUTCTX_OK"
  ```
- **Did it print `CUTCTX_OK`?**: yes / no
...
(logs from `~/.cutctx/logs/proxy.log`, surprises, etc.)
