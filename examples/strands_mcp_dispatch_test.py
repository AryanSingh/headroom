#!/usr/bin/env python3
"""Verify Strands' MCP dispatcher can reach Cutctx's MCP server.
...
Strands Agent receives a model-emitted `cutctx_retrieve` tool_call
...
def main() -> int:
    print("=" * 72)
    print(" Strands MCPClient → Cutctx MCP server  end-to-end probe")
    print("=" * 72)

    print("\n[1/5] Starting Cutctx proxy ...")
    proxy = start_proxy()
    try:
        wait_for_proxy()
        print("  proxy ready.\n")
        ...
        with MCPClient(lambda: stdio_client(server_params)) as mcp:
            ...
            print(f"  tools advertised by Cutctx MCP: {tool_names}")
            if "cutctx_retrieve" not in tool_names:
                print("  ! cutctx_retrieve NOT in tool list — abort.")
                return 4
            ...
            # `cutctx_retrieve` tool_call in a real conversation.
            print("\n[5a/5] Stashing content via MCP cutctx_compress (same dispatcher) ...")
            try:
                ...
                # Find the hash. Cutctx MCP compress is documented to emit
            ...
            print("\n[5b/5] Calling cutctx_retrieve via Strands MCP dispatcher ...")
            try:
        ...
        print(" Strands receives a cutctx_retrieve tool_call → MCP dispatcher")
        print(" → Cutctx MCP server → CompressionStore → original content back.")
        print("=" * 72)
        return 0
    finally:
        print("\n  stopping proxy ...")
