#!/usr/bin/env bash
# Build + install the Rust extension (cutctx._core) into the active venv.
#
...
# in-tree `cutctx/` directory because the dual-package layout left the
# .so in `crates/cutctx-py/python/cutctx/`. That dance is no longer
...
# wheel contains both the Python source and the compiled `cutctx/_core.so`,
...
log "verifying \`from cutctx._core import DiffCompressor, SmartCrusher\`"
python -c '
import sys
try:
    from cutctx._core import DiffCompressor, SmartCrusher
...
log "cutctx._core build + install + verify: OK"
