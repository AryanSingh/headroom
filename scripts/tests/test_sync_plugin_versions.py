...
def test_main_is_noop_on_feature_branch(monkeypatch, capsys) -> None:
    ...
    False (feature branch, no CUTCTX_SYNC_VERSIONS opt-in), main()
...
def test_should_sync_main_branch_runs(monkeypatch) -> None:
    ...
    module = _load_module()
    monkeypatch.delenv("CUTCTX_SYNC_VERSIONS", raising=False)
