...
def temp_project(tmp_path: Path) -> dict[str, Path]:
    ...
    agent_hooks_claude = plugins / "cutctx-agent-hooks" / ".claude-plugin"
    agent_hooks_claude.mkdir(parents=True)
    agent_hooks_github = plugins / "cutctx-agent-hooks" / ".github" / "plugin"
        ...
        json.dumps(
            {
                ...
                "plugins": [{"name": "cutctx-agent-hooks", "version": "0.1.0"}],
            }
        )
    )
    ...
    claude_plugin.write_text(json.dumps({"name": "cutctx-agent-hooks", "version": "0.1.0"}))

    github_plugin = agent_hooks_github / "plugin.json"
    github_plugin.write_text(json.dumps({"name": "cutctx-agent-hooks", "version": "0.1.0"}))
