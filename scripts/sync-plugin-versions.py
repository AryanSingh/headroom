...
Sync now only runs when EITHER:
  ...
  * ``CUTCTX_SYNC_VERSIONS=1`` is set explicitly (release workflow)
...
try:
    from cutctx.release_version import (  # noqa: E402
    ...
    print("skip: cutctx deps not installed (run from a dev venv to enable)")
    sys.exit(0)
...
def _should_sync(root: Path) -> bool:
    ...
    Release workflow opts in via ``CUTCTX_SYNC_VERSIONS=1``; otherwise
    ...
    """
    if os.environ.get("CUTCTX_SYNC_VERSIONS") == "1":
        return True
...
def main() -> None:
    ...
    if not _should_sync(root):
        ...
        print(
            f"sync-plugin-versions: skipping on branch '{branch}' (set CUTCTX_SYNC_VERSIONS=1 to force)"
        )
        return
