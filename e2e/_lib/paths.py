...
These paths mirror the logic in ``cutctx.cli.init`` so e2e tests can
...
def agent_settings_path(agent: Agent, *, scope: Scope, home: Path, project: Path) -> Path:
    """Return the file that ``cutctx init`` should have written for ``agent``.

    ``home`` is the test's simulated HOME directory and ``project`` is the cwd
    used when invoking ``cutctx init``. For global (``-g``) invocations only
    ...
    if agent == "openclaw":
        # OpenClaw init is delegated to `cutctx wrap openclaw`; it writes
