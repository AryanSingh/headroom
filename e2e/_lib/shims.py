...
tests drive `cutctx init` without requiring a real Claude/Codex install.
...
* ``record-args``  — appends a JSON record of (tool, argv, cwd) to the file at
                     ``$CUTCTX_E2E_SHIM_LOG``, then exits 0. Useful for
...
tool="${0##*/}"
log="${CUTCTX_E2E_SHIM_LOG:-/dev/null}"
...
_RECORD_CMD = (
    "@echo off\r\n"
    "setlocal\r\n"
    'if "%CUTCTX_E2E_SHIM_LOG%"=="" set CUTCTX_E2E_SHIM_LOG=NUL\r\n'
    "python -c \"import json,os,sys; name=r'%~n0'; log=os.environ['CUTCTX_E2E_SHIM_LOG']; "
