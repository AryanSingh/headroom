...
logger = logging.getLogger("cutctx.sso")
...
class SsoConfig:
    ...
    def from_env(cls) -> SsoConfig:
        """Create SSO config from CUTCTX_SSO_* environment variables."""
        import os

        role_mapping_raw = os.environ.get("CUTCTX_SSO_ROLE_MAPPING", "")
        ...
        required_scopes_raw = os.environ.get("CUTCTX_SSO_REQUIRED_SCOPES", "")
        required_scopes = [s.strip() for s in required_scopes_raw.split(",") if s.strip()]

        return cls(
            provider_type=os.environ.get("CUTCTX_SSO_PROVIDER_TYPE", "oidc"),
            discovery_url=os.environ.get("CUTCTX_SSO_DISCOVERY_URL"),
            jwks_uri=os.environ.get("CUTCTX_SSO_JWKS_URI"),
            issuer=os.environ.get("CUTCTX_SSO_ISSUER"),
            audience=os.environ.get("CUTCTX_SSO_AUDIENCE"),
            introspection_url=os.environ.get("CUTCTX_SSO_INTROSPECTION_URL"),
            introspection_client_id=os.environ.get("CUTCTX_SSO_INTROSPECTION_CLIENT_ID"),
            introspection_client_secret=os.environ.get("CUTCTX_SSO_INTROSPECTION_CLIENT_SECRET"),
            required_scopes=required_scopes,
            role_mapping=role_mapping,
            default_role=os.environ.get("CUTCTX_SSO_DEFAULT_ROLE", "viewer"),
            jwks_cache_ttl=int(os.environ.get("CUTCTX_SSO_JWKS_CACHE_TTL", "3600")),
            clock_skew_tolerance=int(os.environ.get("CUTCTX_SSO_CLOCK_SKEW_TOLERANCE", "60")),
            http_timeout=int(os.environ.get("CUTCTX_SSO_HTTP_TIMEOUT", "10")),
        )
