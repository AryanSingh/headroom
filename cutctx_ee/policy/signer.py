...
def sign_policy(
    ...
    """
    kid = kid or os.environ.get("CUTCTX_POLICY_KID") or os.environ.get("CUTCTX_LICENSE_KID")
    private_key_hex = (
        private_key_hex
        or os.environ.get("CUTCTX_POLICY_PRIVATE_KEY")
        or os.environ.get("CUTCTX_LICENSE_PRIVATE_KEY")
    )

    if not kid or not private_key_hex:
        raise ValueError(
            "Policy signing keys not configured. Set CUTCTX_POLICY_PRIVATE_KEY and CUTCTX_POLICY_KID."
        )
