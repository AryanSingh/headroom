 ...
 *
 * Run with: CUTCTX_INTEGRATION=1 npx vitest run test/integration.test.ts
 */
...
const RUN_INTEGRATION = process.env.CUTCTX_INTEGRATION === "1";
...
describe.skipIf(!RUN_INTEGRATION)("Integration: compress() with real proxy", () => {
  beforeAll(async () => {
    ...
    try {
      ...
      throw new Error(
        `Proxy not running at ${PROXY_URL}. Start with: cutctx proxy --port 8787\n${e}`,
      );
    }
  });
