...
describe("canonical roots", () => {
  ...
  it("workspaceDir defaults to ~/.cutctx", () => {
    expect(workspaceDir()).toBe(path.join(os.homedir(), ".cutctx"));
  });

  it("workspaceDir honors CUTCTX_WORKSPACE_DIR env override", () => {
    process.env[CUTCTX_WORKSPACE_DIR_ENV] = "/tmp/alt_ws";
    expect(workspaceDir()).toBe("/tmp/alt_ws");
  });

  it("workspaceDir ignores blank env value", () => {
    process.env[CUTCTX_WORKSPACE_DIR_ENV] = "   ";
    expect(workspaceDir()).toBe(path.join(os.homedir(), ".cutctx"));
  });

  it("workspaceDir expands tilde", () => {
    process.env[CUTCTX_WORKSPACE_DIR_ENV] = "~/custom-ws";
    expect(workspaceDir()).toBe(path.join(os.homedir(), "custom-ws"));
  });

  it("configDir defaults to ~/.cutctx/config", () => {
    expect(configDir()).toBe(path.join(os.homedir(), ".cutctx", "config"));
  });

  it("configDir follows CUTCTX_WORKSPACE_DIR when only workspace set", () => {
    process.env[CUTCTX_WORKSPACE_DIR_ENV] = "/tmp/alt_ws";
    expect(configDir()).toBe(path.join("/tmp/alt_ws", "config"));
  });

  it("explicit CUTCTX_CONFIG_DIR env beats workspace env", () => {
    process.env[CUTCTX_WORKSPACE_DIR_ENV] = "/tmp/alt_ws";
    process.env[CUTCTX_CONFIG_DIR_ENV] = "/tmp/alt_cfg";
...
const RESOURCES: ResourceCase[] = [
  {
    name: "savingsPath",
    fn: savingsPath,
    envVar: CUTCTX_SAVINGS_PATH_ENV,
    filename: "proxy_savings.json",
  },
  {
    name: "toinPath",
    fn: toinPath,
    envVar: CUTCTX_TOIN_PATH_ENV,
    filename: "toin.json",
  },
  {
    name: "subscriptionStatePath",
    fn: subscriptionStatePath,
    envVar: CUTCTX_SUBSCRIPTION_STATE_PATH_ENV,
...
describe.each(RESOURCES)(
  ...
  ({ fn, envVar, filename }) => {
    ...
    it("default under ~/.cutctx", () => {
      expect(fn()).toBe(path.join(os.homedir(), ".cutctx", filename));
    });

    it("derived from CUTCTX_WORKSPACE_DIR when set", () => {
      process.env[CUTCTX_WORKSPACE_DIR_ENV] = "/tmp/state";
...
describe("derived-only resources", () => {
  ...
  it("memoryDbPath", () => {
    expect(memoryDbPath()).toBe(
      path.join(os.homedir(), ".cutctx", "memory.db"),
    );
  });

  it("nativeMemoryDir", () => {
    expect(nativeMemoryDir()).toBe(
      path.join(os.homedir(), ".cutctx", "memories"),
    );
  });

  it("licenseCachePath", () => {
    expect(licenseCachePath()).toBe(
      path.join(os.homedir(), ".cutctx", "license_cache.json"),
    );
  });

  it("sessionStatsPath", () => {
    expect(sessionStatsPath()).toBe(
      path.join(os.homedir(), ".cutctx", "session_stats.jsonl"),
    );
  });

  it("syncStatePath", () => {
    expect(syncStatePath()).toBe(
      path.join(os.homedir(), ".cutctx", "sync_state.json"),
    );
  });

  it("bridgeStatePath", () => {
    expect(bridgeStatePath()).toBe(
      path.join(os.homedir(), ".cutctx", "bridge_state.json"),
    );
  });

  it("logDir", () => {
    expect(logDir()).toBe(path.join(os.homedir(), ".cutctx", "logs"));
  });

  it("proxyLogPath", () => {
    expect(proxyLogPath()).toBe(
      path.join(os.homedir(), ".cutctx", "logs", "proxy.log"),
    );
  });

  it("debug400Dir", () => {
    expect(debug400Dir()).toBe(
      path.join(os.homedir(), ".cutctx", "logs", "debug_400"),
    );
  });

  it("binDir", () => {
    expect(binDir()).toBe(path.join(os.homedir(), ".cutctx", "bin"));
  });
...
// ---------------------------------------------------------------------------
// Derived-only helpers must follow CUTCTX_WORKSPACE_DIR end-to-end
// ---------------------------------------------------------------------------
...
describe("plugin dirs", () => {
  ...
  it("pluginConfigDir namespaced under configDir/plugins", () => {
    ...
    expect(a).not.toBe(b);
    expect(a).toBe(
      path.join(os.homedir(), ".cutctx", "config", "plugins", "alpha"),
    );
  });
  ...
  it("pluginWorkspaceDir namespaced under workspaceDir/plugins", () => {
    ...
    expect(a).not.toBe(b);
    expect(a).toBe(
      path.join(os.homedir(), ".cutctx", "plugins", "alpha"),
    );
  });
