...
This benchmark demonstrates WHY Cutctx matters by showing:
...
2. THE SOLUTION: Cutctx's impact on real workloads
...
def benchmark_coding_agent_explosion() -> BenchmarkResult:
    ...
    # Apply Cutctx transforms using convenience function
...
def benchmark_rag_scaling() -> BenchmarkResult:
    """
    Show how RAG context grows and how Cutctx manages it.
...
def benchmark_conversation_scaling() -> list[BenchmarkResult]:
    ...
    and shows the scaling curve with and without Cutctx.
    """
    results = []
...
def _generate_terminal_report(results: list[BenchmarkResult]) -> str:
    ...
    lines.append(f"    Without Cutctx:    ${total_baseline * 1_000_000:.2f}")
    lines.append(f"    With Cutctx:       ${(total_baseline - total_savings) * 1_000_000:.2f}")
...
def _generate_markdown_report(results: list[BenchmarkResult]) -> str:
    ...
    lines.append("")
    lines.append("This benchmark demonstrates Cutctx's impact on real-world agent workloads:")
    lines.append("")
    ...
    lines.append("")
    lines.append("| Scale | Without Cutctx | With Cutctx | Monthly Savings |")
    lines.append("|-------|------------------|---------------|-----------------|")
...
def main():
    parser = argparse.ArgumentParser(description="Cutctx Agent Cost Benchmark")
