...
from .metrics import _cutctx_version, _parse_bool, _parse_key_value_pairs
...
class LangfuseTracingConfig:
    ...
    def from_env(cls, *, default_service_name: str = "cutctx") -> LangfuseTracingConfig:
        ...
        return cls(
            enabled=_parse_bool(
                os.environ.get("CUTCTX_LANGFUSE_ENABLED"),
                default=False,
            ),
            ...
            service_name=os.environ.get(
                "CUTCTX_LANGFUSE_SERVICE_NAME", default_service_name
            ).strip()
            or default_service_name,
            resource_attributes=_parse_key_value_pairs(
                os.environ.get("CUTCTX_LANGFUSE_RESOURCE_ATTRIBUTES")
            ),
        )
...
class CutctxTracer:
    ...
    def __init__(self, tracer_provider: Any | None = None):
        if tracer_provider is None:
            self._tracer = trace.get_tracer(_SCOPE_NAME, _cutctx_version())
        else:
            self._tracer = tracer_provider.get_tracer(_SCOPE_NAME, _cutctx_version())
...
def get_cutctx_tracer() -> CutctxTracer:
...
def set_cutctx_tracer(cutctx_tracer: CutctxTracer) -> CutctxTracer:
    global _global_tracer
    with _tracing_lock:
        _global_tracer = cutctx_tracer
    return cutctx_tracer
...
def configure_langfuse_tracing(
    ...
    try:
        ...
        logger.warning(
            ...
            "Install cutctx-ai[otel] to enable Langfuse OTLP tracing."
        )
        return get_cutctx_tracer()
    ...
    cutctx_tracer = CutctxTracer(tracer_provider=tracer_provider)
...
def shutdown_cutctx_tracing() -> None:
    global _global_tracer
    global _owned_tracer_provider
    global _owned_langfuse_config
