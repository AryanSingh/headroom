...
def _cutctx_version() -> str:
    try:
        return package_version("cutctx-ai")
...
class OTelMetricsConfig:
    ...
    def from_env(cls, *, default_service_name: str = "cutctx") -> OTelMetricsConfig:
        exporter_raw = (
            os.environ.get("CUTCTX_OTEL_METRICS_EXPORTER", "otlp_http")
            .strip()
            .lower()
            .replace("-", "_")
        )
        if exporter_raw not in {"console", "otlp_http"}:
            logger.warning(
                "Unknown CUTCTX_OTEL_METRICS_EXPORTER=%s; falling back to otlp_http",
                exporter_raw,
            )
            exporter_raw = "otlp_http"

        return cls(
            enabled=_parse_bool(os.environ.get("CUTCTX_OTEL_METRICS_ENABLED"), default=False),
            service_name=os.environ.get("CUTCTX_OTEL_SERVICE_NAME", default_service_name).strip()
            or default_service_name,
            exporter=exporter_raw,  # type: ignore[arg-type]
            endpoint=os.environ.get("CUTCTX_OTEL_METRICS_ENDPOINT") or None,
            headers=_parse_key_value_pairs(os.environ.get("CUTCTX_OTEL_METRICS_HEADERS")),
            export_interval_millis=_parse_int(
                os.environ.get("CUTCTX_OTEL_METRICS_EXPORT_INTERVAL_MS"),
                _DEFAULT_EXPORT_INTERVAL_MS,
            ),
            resource_attributes=_parse_key_value_pairs(
                os.environ.get("CUTCTX_OTEL_RESOURCE_ATTRIBUTES")
            ),
        )
...
class CutctxOtelMetrics:
    ...
    def __init__(self, meter_provider: Any | None = None):
        if meter_provider is None:
            self._meter = metrics.get_meter(_SCOPE_NAME, _cutctx_version())
        else:
            self._meter = meter_provider.get_meter(_SCOPE_NAME, _cutctx_version())

        self._proxy_requests = self._meter.create_counter(
            "cutctx.proxy.requests",
            description="Proxy requests handled by Cutctx.",
            unit="1",
        )
        self._proxy_cached_requests = self._meter.create_counter(
            "cutctx.proxy.requests.cached",
        ...
        self._proxy_cache_write_ttl_tokens = self._meter.create_counter(
            "cutctx.proxy.cache.write_ttl_tokens",
...
def configure_otel_metrics(config: OTelMetricsConfig | None = None) -> CutctxOtelMetrics:
    ...
    try:
        ...
        logger.warning(
            ...
            "Install cutctx-ai[otel] to enable managed OTEL metric export."
        )
        return get_otel_metrics()
    ...
    resource = Resource.create(
        {
            ...
            SERVICE_VERSION: _cutctx_version(),
