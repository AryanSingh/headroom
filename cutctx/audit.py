...
historical ``cutctx.audit`` path so existing call sites keep working when the
...
try:
    import cutctx_ee.audit as _impl
except ImportError as _e:  # commercial component not installed (community edition)
    raise ImportError(
        "cutctx.audit requires the proprietary 'cutctx_ee' distribution "
...
if TYPE_CHECKING:
    ...
    # sys.modules rebind above makes `from cutctx.audit import X` resolve to
    # cutctx_ee.audit; this makes the same names visible to static analysis.
    from cutctx_ee.audit import *  # noqa: F401,F403
