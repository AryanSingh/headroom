...
happened so an offline aggregator (`cutctx.cli.toin_publish`) can emit
...
# Usage
    ...
    #   python -m cutctx.cli.toin_publish --output recommendations.toml
...
TOIN_PATH_ENV_VAR = "CUTCTX_TOIN_PATH"

# Default TOIN storage directory and file
DEFAULT_TOIN_DIR = ".cutctx"
...
def get_default_toin_storage_path() -> str:
    ...
    Checks for the CUTCTX_TOIN_PATH environment variable first.
    Falls back to ``${CUTCTX_WORKSPACE_DIR}/toin.json`` (which defaults
    to ``~/.cutctx/toin.json``) when unset.

    Returns:
        The path string for TOIN storage.
    """
    # Preserve legacy behavior: when CUTCTX_TOIN_PATH is set we return the
...
class TOINConfig:
    ...
    # Storage
    # Default path is ~/.cutctx/toin.json (or CUTCTX_TOIN_PATH env var)
...
class ToolIntelligenceNetwork:
    ...
    `cutctx.cli.toin_publish` and consumed by the Rust proxy at startup.
    ...
    def iter_patterns(self) -> list[tuple[PatternKey, ToolPattern]]:
        ...
        Used by `cutctx.cli.toin_publish` to walk every aggregated
...
TOIN_BACKEND_ENV_VAR = "CUTCTX_TOIN_BACKEND"


def _create_default_toin_backend() -> Any:
    """Create a TOIN backend from env (e.g. CUTCTX_TOIN_BACKEND=redis).

    Loads adapters via setuptools entry point 'cutctx.toin_backend'.
    ...
    try:
        ...
        all_eps = entry_points(group="cutctx.toin_backend")
        ep = next((e for e in all_eps if e.name == backend_type), None)
        if ep is None:
            logger.warning(
                "CUTCTX_TOIN_BACKEND=%s but no entry point cutctx.toin_backend[%s]",
                backend_type,
                backend_type,
            )
            return None
        ...
        kwargs = {
            "url": os.environ.get("CUTCTX_TOIN_URL", ""),
            "tenant_prefix": os.environ.get("CUTCTX_TOIN_TENANT_PREFIX", ""),
        }
        return fn(**kwargs)
    except Exception as e:
...
def get_toin(config: TOINConfig | None = None) -> ToolIntelligenceNetwork:
    ...
    On first call, checks CUTCTX_TOIN_BACKEND env var. If set, loads the
    backend via setuptools entry point 'cutctx.toin_backend'. Otherwise
