...
To enable network egress to Supabase, set:
    CUTCTX_TELEMETRY_EGRESS=1

Opt out of local telemetry capture with:
    CUTCTX_TELEMETRY=off cutctx proxy
    cutctx proxy --no-telemetry
"""
...
from cutctx.telemetry.context import detect_install_mode, detect_stack
...
def is_telemetry_enabled() -> bool:
    ...
    val = os.environ.get("CUTCTX_TELEMETRY", "on").lower().strip()
...
def is_telemetry_warn_enabled() -> bool:
    ...
    Set CUTCTX_TELEMETRY_WARN=off to suppress startup/wrap notices.
    ...
    """
    val = os.environ.get("CUTCTX_TELEMETRY_WARN", "on").lower().strip()
    return val not in _OFF_VALUES
...
def format_telemetry_notice(*, prefix: str = "") -> str:
    ...
    return (
        ...
        "Disable: CUTCTX_TELEMETRY=off or --no-telemetry"
    )
...
class TelemetryBeacon:
    ...
    async def _report(self) -> None:
        ...
        try:
            from cutctx._version import __version__ as cutctx_version
        except Exception:
            cutctx_version = "unknown"

        # Core identity (always present)
        payload: dict = {
            "session_id": self._session_id,
            "instance_id": self._instance_id,
            "cutctx_version": cutctx_version,
            "python_version": (
            ...
            "cutctx_stack": detect_stack(stats),
        }
