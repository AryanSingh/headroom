...
* ``cutctx_stack`` — how Cutctx is being invoked
...
# from unbounded label explosion when clients send arbitrary X-Cutctx-Stack
# header values.
...
def detect_install_mode(port: int) -> str:
    ...
    1. ``CUTCTX_AGENT_TYPE`` env var set → ``wrapped`` (spawned by ``cutctx wrap``).
    ...
    try:
        if os.environ.get("CUTCTX_AGENT_TYPE"):
            return "wrapped"
...
def detect_stack(stats: dict[str, Any] | None = None) -> str:
    ...
    1. ``CUTCTX_STACK`` env var set → use that slug verbatim.
    2. ``CUTCTX_AGENT_TYPE`` env var set → ``wrap_<agent>``.
    ...
    try:
        explicit = normalize_stack(os.environ.get("CUTCTX_STACK"))
        if explicit:
            return explicit

        agent_type = os.environ.get("CUTCTX_AGENT_TYPE")
        if agent_type:
