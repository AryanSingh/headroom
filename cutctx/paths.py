...
* ``CUTCTX_CONFIG_DIR`` -- read-mostly configuration (defaults to
  ``~/.cutctx/config``). Holds model catalogs, plugin settings, and other
  configuration that users or admins edit.
* ``CUTCTX_WORKSPACE_DIR`` -- read-write state (defaults to ``~/.cutctx``).
...
per-resource override (``CUTCTX_SAVINGS_PATH``, ``CUTCTX_TOIN_PATH``,
...
CUTCTX_CONFIG_DIR_ENV = "CUTCTX_CONFIG_DIR"
CUTCTX_WORKSPACE_DIR_ENV = "CUTCTX_WORKSPACE_DIR"

# ---------------------------------------------------------------------------
# Legacy per-resource env vars (kept for backward compatibility)
# ---------------------------------------------------------------------------

CUTCTX_SAVINGS_PATH_ENV = "CUTCTX_SAVINGS_PATH"
CUTCTX_TOIN_PATH_ENV = "CUTCTX_TOIN_PATH"
CUTCTX_SUBSCRIPTION_STATE_PATH_ENV = "CUTCTX_SUBSCRIPTION_STATE_PATH"

# ---------------------------------------------------------------------------
# Default sub-path fragments
# ---------------------------------------------------------------------------

_WORKSPACE_DIR_DEFAULT = ".cutctx"
...
def workspace_dir() -> Path:
    ...
    1. ``$CUTCTX_WORKSPACE_DIR`` (trimmed, tilde-expanded) if set.
    2. ``~/.cutctx`` otherwise.
    """

    env_value = _env(CUTCTX_WORKSPACE_DIR_ENV)
    if env_value:
        return Path(env_value).expanduser()
...
def config_dir() -> Path:
    ...
    1. ``$CUTCTX_CONFIG_DIR`` (trimmed, tilde-expanded) if set.
    2. ``$CUTCTX_WORKSPACE_DIR/config`` when the workspace env var is set
       so that a single override relocates both roots coherently.
    3. ``~/.cutctx/config`` otherwise.
    """

    env_value = _env(CUTCTX_CONFIG_DIR_ENV)
    if env_value:
        return Path(env_value).expanduser()
    workspace_env = _env(CUTCTX_WORKSPACE_DIR_ENV)
    if workspace_env:
...
def toin_path(explicit: str | os.PathLike[str] | None = None) -> Path:
    ...
    ``~/.cutctx/toin.json`` to preserve byte-for-byte backward compat.
    """
...
def models_config_path() -> Path:
    ...
    Note: the ``CUTCTX_MODEL_LIMITS`` env var is a *content* override
    ...
    location and deliberately ignores ``CUTCTX_MODEL_LIMITS``.
    """
