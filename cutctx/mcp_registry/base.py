...
The MCP protocol itself is universal, and the cutctx MCP server
(``cutctx mcp serve``) is a single stdio binary that any compliant client
...
The orchestrator in :mod:`cutctx.mcp_registry.install` calls a fleet of
registrars to install cutctx across every detected agent.
"""
