...
A stdio MCP server that exposes cutctx's memory backend as tools
...
Usage:
    ...
    python -m cutctx.memory.mcp_server --db /path/to/.cutctx/memory.db

    # Registered in Codex config.toml (done by `cutctx wrap codex --memory`):
    [mcp_servers.cutctx_memory]
    command = "python"
    args = ["-m", "cutctx.memory.mcp_server", "--db", ".cutctx/memory.db"]
"""
...
logger = logging.getLogger("cutctx.memory.mcp")
...
def create_memory_server(db_path: str, user_id: str = "default") -> Server:
    """Create an MCP server backed by cutctx's local memory."""

    server = Server("cutctx-memory")
...
def main() -> None:
    ...
    parser.add_argument(
        "--db",
        default=str(Path.cwd() / ".cutctx" / "memory.db"),
