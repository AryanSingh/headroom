...
_MEMORY_VECTOR_GROUP = "cutctx.memory_vector"
_MEMORY_TEXT_GROUP = "cutctx.memory_text"
...
def _load_external_backend(
    ...
    `cutctx.cache.compression_store._create_default_ccr_backend`.
    """
    if not name:
        raise ValueError(
...
def _create_embedder(config: MemoryConfig) -> Embedder:
    ...
    with _EMBEDDER_CACHE_LOCK:
        ...
        if config.embedder_backend == EmbedderBackend.LOCAL:
            ...
            from cutctx.memory.adapters.embedders import OllamaEmbedder
...
def _create_vector_index(config: MemoryConfig) -> VectorIndex:
    ...
    if backend == VectorBackend.SQLITE_VEC:
        from cutctx.memory.adapters import SQLITE_VEC_AVAILABLE
    ...
    if backend == VectorBackend.HNSW:
        from cutctx.memory.adapters import HNSW_AVAILABLE
...
def _create_cache(config: MemoryConfig) -> MemoryCache:
    ...
    """
    from cutctx.memory.adapters.cache import LRUMemoryCache
