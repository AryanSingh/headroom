...
Provides bidirectional sync between cutctx's memory DB and any
...
Usage:
    from cutctx.memory.sync import sync, SyncResult
    from cutctx.memory.sync_adapters.claude_code import ClaudeCodeAdapter
...
logger = logging.getLogger("cutctx.memory.sync")

# State file for fast no-op detection (workspace bucket, respects
# CUTCTX_WORKSPACE_DIR). Resolved at import time, matching prior behavior.
    ...
    async def write_memories(self, memories: list[dict[str, Any]]) -> int:
        ...
        Args:
            memories: List of dicts with keys: content, category, importance,
                      cutctx_id, source_agent, content_hash.
...
async def sync(
    ...
    """Bidirectional sync between cutctx DB and an agent's memory.
...
# ---------------------------------------------------------------------------
# CLI entry point: python -m cutctx.memory.sync --db ... --user ... --agent ...
# ---------------------------------------------------------------------------
...
def main() -> None:
    ...
    async def _run() -> None:
        ...
        if args.agent == "claude":
            ...
            from cutctx.memory.sync_adapters.codex_agent import CodexAdapter
