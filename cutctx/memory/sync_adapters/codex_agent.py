...
Syncs memories to/from a cutctx-managed section in AGENTS.md.
...
from cutctx.memory.sync import AgentMemory, AgentMemoryAdapter

_MARKER_START = "<!-- cutctx:memory:start -->"
_MARKER_END = "<!-- cutctx:memory:end -->"
...
class CodexAdapter(AgentMemoryAdapter):
    ...
    async def read_memories(self) -> list[AgentMemory]:
        """Read memories from the cutctx section of AGENTS.md."""
    ...
    async def write_memories(self, memories: list[dict[str, Any]]) -> int:
        """Write memories into the cutctx section of AGENTS.md."""
        if not memories:
            return 0
