...
* ``USER``: one DB per ``x-cutctx-user-id`` (no project axis).
...
class ProjectResolver:
    ...
    def resolve(self, ctx: RequestContext) -> tuple[str, str] | None:
        ...
        explicit = self._first_nonempty_header(ctx.headers, "x-cutctx-project-id")
        if explicit:
        ...
        explicit_cwd = self._first_nonempty_header(ctx.headers, "x-cutctx-cwd")
        if explicit_cwd:
...
class TeamSyncWrapper:
    ...
    def _trigger_sync(self) -> None:
        ...
        from cutctx.memory.sync import sync_team_memory
        ...
        if ident is None:
            ...
            if fallback == "empty":
                ...
                logger.warning(
                    ...
                    "hint='set x-cutctx-project-id or x-cutctx-cwd header, "
