...
class StoreBackend(Enum):
    ...
    SQLITE = "sqlite"
    EXTERNAL = "external"  # Loaded from entry_points(group="cutctx.memory_store")
...
class TextBackend(Enum):
    ...
    FTS5 = "fts5"
    EXTERNAL = "external"  # Loaded from entry_points(group="cutctx.memory_text")
...
class MemoryConfig:
    ...
    db_path: Path = field(default_factory=lambda: Path("cutctx_memory.db"))
