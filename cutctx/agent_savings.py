...
class AgentSavingsProfile:
    ...
    def proxy_env(self) -> dict[str, str]:
        ...
        return {
            "CUTCTX_MODE": self.proxy_mode,
            "CUTCTX_SAVINGS_PROFILE": self.name,
            "CUTCTX_SAVINGS_TARGET": f"{self.target_savings:.2f}",
            "CUTCTX_TARGET_RATIO": f"{self.target_ratio:.2f}",
            "CUTCTX_COMPRESS_USER_MESSAGES": ("1" if self.compress_user_messages else "0"),
            "CUTCTX_COMPRESS_SYSTEM_MESSAGES": ("1" if self.compress_system_messages else "0"),
            "CUTCTX_PROTECT_RECENT": str(self.protect_recent),
