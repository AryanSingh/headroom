...
class RelevanceScorerConfig:
    ...
    For full hybrid support, install: pip install cutctx[relevance]
...
class CCRConfig:
    ...
    4. RETRIEVE: If LLM needs more, it calls cutctx_retrieve(hash, query)
    ...
    inject_tool: bool = True  # Inject cutctx_retrieve tool into tools array
...
class CutctxConfig:
    ...
    # Env var CUTCTX_INTERCEPT_ENABLED=1 also enables (for CLI `--intercept-tool-results`).
