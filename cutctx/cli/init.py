...
from cutctx.install.models import ConfigScope, InstallPreset, RuntimeKind, SupervisorKind
from cutctx.install.paths import claude_settings_path, codex_config_path, validate_profile_name
from cutctx.install.planner import build_manifest
from cutctx.install.providers import _apply_unix_env_scope, _apply_windows_env_scope
...
from cutctx.install.state import load_manifest, save_manifest
from cutctx.install.supervisors import start_supervisor
from cutctx.providers.gemini.install import build_install_env as _build_gemini_install_env

from .main import main

logger = logging.getLogger(__name__)

_VERBOSE_HANDLER_ATTR = "_cutctx_init_verbose_handler"

_GLOBAL_PROFILE = "init-user"
_CLAUDE_HOOK_MARKER = "cutctx-init-claude"
_COPILOT_HOOK_MARKER = "cutctx-init-copilot"
_CODEX_HOOK_MARKER = "cutctx-init-codex"
_GEMINI_HOOK_MARKER = "cutctx-init-gemini"
...
def _hook_command(*parts: str) -> str:
    return _command_string([*resolve_cutctx_command(), "init", "hook", "ensure", *parts])
...
def _retain_non_cutctx_hook_entries(entries: list[Any], marker: str) -> list[Any]:
    retained: list[Any] = []
    for entry in entries:
        if not isinstance(entry, dict):
            retained.append(entry)
...
def _marketplace_source() -> str:
    override = os.environ.get("CUTCTX_MARKETPLACE_SOURCE")
    if override:
        return override
...
def _install_claude_marketplace(scope: str) -> None:
    ...
    _run_checked(
        [claude_bin, "plugin", "install", "cutctx@cutctx-marketplace", "--scope", scope],
...
def _install_copilot_marketplace() -> None:
    ...
    _run_checked(
        [copilot_bin, "plugin", "install", "cutctx@cutctx-marketplace"],
...
def _init_openclaw(*, global_scope: bool, port: int) -> None:
    ...
    command = [*resolve_cutctx_command(), "wrap", "openclaw", "--proxy-port", str(port)]
...
def _install_cutctx_mcp_for_targets(*, targets: list[str], port: int) -> None:
    """Install the Cutctx MCP server into each detected target agent."""
    from cutctx.mcp_registry import format_results, install_everywhere
...
def init_windsurf(ctx: click.Context) -> None:
    """Install durable Cutctx integration for Windsurf."""
    _run_init_targets(
        targets=["windsurf"],
        global_scope=bool(_ctx_value(ctx, "global_scope")),
        port=int(_ctx_value(ctx, "port") or 8787),
...
def init_opencode(ctx: click.Context) -> None:
    """Install durable Cutctx integration for opencode."""
    _run_init_targets(
        targets=["opencode"],
        global_scope=bool(_ctx_value(ctx, "global_scope")),
        port=int(_ctx_value(ctx, "port") or 8787),
