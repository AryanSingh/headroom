...
External plugins register via 'cutctx.learn_plugin' entry point.
...
def learn(
    ...
    be installed via pip (entry point: cutctx.learn_plugin).

    \b
    Examples:
        cutctx learn                        # Auto-detect agent & model
        cutctx learn --apply                # Write recommendations
        cutctx learn --model gpt-4o         # Use GPT-4o for analysis
        cutctx learn --all                  # Analyze all projects
        cutctx learn --agent codex --all    # Analyze all Codex sessions
    """
    import os
    ...
    if dry_run and apply:
        click.echo(
            ...
            "Use --dry-run to preview without writing, or --apply to write recommendations.",
            err=True,
        )
        raise SystemExit(1)
    ...
    for agent_name, plugin in agent_configs:
        ...
        if analyze_all:
            ...
            if not targets and len(agent_configs) == 1:
                ...
                click.echo("Try: cutctx learn --all  or  cutctx learn --project <path>")
