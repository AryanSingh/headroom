...
def perf(hours: float, raw: bool, output_format: str) -> None:
    ...
    \b
    Reads logs from ~/.cutctx/logs/proxy.log and shows:
    ...
    \b
    Examples:
        cutctx perf                      Analyze last 7 days
        cutctx perf --hours 24           Analyze last 24 hours
        cutctx perf --raw                Show raw parsed records
        cutctx perf --format json        Aggregated report as JSON
        cutctx perf --format csv --hours 24 > last-24h.csv
        cutctx perf --format json --raw  Raw records as a JSON array
    """
    ...
    if raw:
        ...
        if not report.perf_records:
            click.echo("No PERF records found. Run the proxy first: cutctx proxy")
    else:
