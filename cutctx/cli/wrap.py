...
_WRAP_PROXY_TIMEOUT_ENV = "CUTCTX_WRAP_PROXY_TIMEOUT"
...
def _selected_context_tool() -> str:
    ...
    ``CUTCTX_CONTEXT_TOOL=lean-ctx`` to let lean-ctx configure the supported
...
def _wrap_agent_savings_profile(agent_type: str) -> str | None:
    ...
    if agent_type not in _AGENT_SAVINGS_WRAP_AGENTS:
        return None
    return os.environ.get("CUTCTX_SAVINGS_PROFILE") or _DEFAULT_AGENT_SAVINGS_PROFILE
...
def _print_telemetry_notice() -> None:
    ...
    Respects the CUTCTX_TELEMETRY and CUTCTX_TELEMETRY_WARN feature flags.
    Does nothing when telemetry or warnings are disabled.
    """
    from cutctx.telemetry.beacon import format_telemetry_notice
...
def _start_proxy(
    ...
    *,
    ...
    Logs are written to ~/.cutctx/logs/proxy.log to avoid pipe buffer
    deadlocks (macOS pipe buffer is ~64KB — a busy proxy fills it quickly,
    blocking the process).
    """
    cmd = [sys.executable, "-m", "cutctx.cli", "proxy", "--port", str(port)]

    # Forward CUTCTX_MODE env var so the proxy respects the user's mode choice
    cutctx_mode = os.environ.get("CUTCTX_MODE")
    if cutctx_mode:
        cmd.extend(["--mode", cutctx_mode])
    ...
    _backend = backend or os.environ.get("CUTCTX_BACKEND")
    if _backend:
        cmd.extend(["--backend", _backend])

    _anyllm = anyllm_provider or os.environ.get("CUTCTX_ANYLLM_PROVIDER")
    if _anyllm:
        cmd.extend(["--anyllm-provider", _anyllm])

    _region = region or os.environ.get("CUTCTX_REGION")
    if _region:
    ...
    if agent_type != "unknown":
        proxy_env["CUTCTX_AGENT_TYPE"] = agent_type
        proxy_env.setdefault("CUTCTX_STACK", f"wrap_{agent_type}")
    savings_profile = _wrap_agent_savings_profile(agent_type)
    if savings_profile is not None:
        from cutctx.agent_savings import apply_agent_savings_env_defaults
...
def _setup_rtk(verbose: bool = False) -> Path | None:
    ...
    from cutctx.rtk import get_rtk_path
    from cutctx.rtk.installer import ensure_rtk, register_claude_hooks
...
def _setup_lean_ctx_agent(agent: str, verbose: bool = False) -> Path | None:
    ...
    from cutctx.lean_ctx import get_lean_ctx_path
    from cutctx.lean_ctx.installer import ensure_lean_ctx
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="cutctx-lean-ctx-") as setup_cwd:
...
def _setup_cutctx_mcp(
    registrar: Any, port: int, *, verbose: bool = False, force: bool = False
) -> None:
    """Register the cutctx MCP server with the given agent (idempotent).
    ...
    nothing — the agent has no ``cutctx_retrieve`` tool to call.
    ...
    """
    from cutctx.mcp_registry import build_cutctx_spec, format_result
...
def _setup_serena_mcp(
    ...
    from cutctx.mcp_registry import build_serena_spec, format_result
    from cutctx.mcp_registry.base import RegisterStatus
...
def _remove_cutctx_installed_serena_mcp(registrar: Any) -> str:
    """Remove Serena MCP only if the ledger proves Cutctx installed it."""
    from cutctx.mcp_registry.ledger import clear_install, cutctx_installed_matching

    current = registrar.get_server("serena")
    if not cutctx_installed_matching(registrar.name, current):
        return "not_cutctx_owned"
...
def _setup_code_graph(verbose: bool = False) -> bool:
    ...
    """
    from cutctx.graph.installer import ensure_cbm, get_cbm_path
...
_RTK_MARKER = "<!-- cutctx:rtk-instructions -->"

# Memory MCP markers
_MEMORY_MCP_MARKER = "# --- Cutctx memory MCP (auto-injected) ---"
_MEMORY_MCP_END = "# --- end Cutctx memory ---"
_MEMORY_AGENTS_MARKER = "<!-- cutctx:memory-instructions -->"
...
# before running wrap.
_CODEX_CONFIG_BACKUP_SUFFIX = ".cutctx-backup"
...
def _strip_codex_cutctx_blocks(content: str, *, remove_mcp: bool = False) -> str:
    ...
    # Strip any orphaned provider table with the fields we
    ...
    # happen to have a differently configured provider with the same name.
    for provider_name in _CODEX_PROVIDER_ALIASES:
...
def _snapshot_codex_config_if_unwrapped(config_file: Path, backup_file: Path) -> None:
    ...
    ``config.toml``.  Guarantees that ``cutctx unwrap codex`` can restore the
...
def _ensure_rtk_binary(verbose: bool = False) -> Path | None:
    ...
    from cutctx.rtk import get_rtk_path
    from cutctx.rtk.installer import ensure_rtk
...
# case-insensitively by cutctx.proxy.project_context.PROJECT_HEADER).
_PROJECT_HEADER_NAME = "X-Cutctx-Project"


def _project_name_from_cwd() -> str | None:
    """Project label for X-Cutctx-Project: basename of the launch directory.

    The proxy sanitizes and caps the value server-side
    (cutctx.proxy.savings_tracker.sanitize_project_name), so the raw
...
def _apply_project_header_env(env: dict[str, str]) -> None:
    """Inject X-Cutctx-Project into ``ANTHROPIC_CUSTOM_HEADERS``.
    ...
    Cutctx proxy uses the X-Cutctx-Project header for per-project
    savings attribution.  An existing user-supplied x-cutctx-project
...
def _inject_codex_provider_config(port: int) -> None:
    ...
    * ``openai_base_url = "http://127.0.0.1:{port}/v1"`` — overrides the
    ...
    ``config.toml.cutctx-backup`` so ``cutctx unwrap codex``
    ...
    try:
        ...
        if config_file.exists():
            ...
            content = _strip_codex_cutctx_blocks(content)
    ...
    * ``"noop"``     — nothing to undo; no Cutctx marker and no backup.
    ...
    if config_file.exists():
        ...
        if _CODEX_TOP_LEVEL_MARKER in original or _CODEX_END_MARKER in original:
            cleaned = _strip_codex_cutctx_blocks(original, remove_mcp=True)
...
def _inject_memory_mcp_config(db_path: str, user_id: str) -> None:
    ...
    mcp_section = (
        f"\n{_MEMORY_MCP_MARKER}\n"
        f"[mcp_servers.cutctx_memory]\n"
        f'command = "{python_bin}"\n'
        f'args = ["-m", "cutctx.memory.mcp_server", "--db", "{db_path_toml}", "--user", "{user_id}"]\n'
...
def _inject_memory_agents_md(file_path: Path) -> bool:
    ...
    memory_block = (
        ...
        "## Memory\n\n"
        "Use the `cutctx_memory` MCP server for persistent cross-session knowledge.\n\n"
...
def _inject_continue_rtk_systemmessage(config_file: Path, verbose: bool = False) -> bool:
    ...
    * each ``models[i].systemMessage`` where ``models[i]`` is a dict
    ...
    The RTK marker (``<!-- cutctx:rtk-instructions -->``) is the idempotency
...
def _proxy_needs_version_restart(payload: dict[str, Any] | None) -> bool:
    ...
    return (
        ...
        and _CUTCTX_VERSION != "unknown"
        and running_version != _CUTCTX_VERSION
    )
...
def _find_persistent_manifest(port: int) -> Any:
    ...
    from cutctx.install.state import list_manifests
...
def _recover_persistent_proxy(port: int) -> bool:
    ...
    from cutctx.install.models import InstallPreset, SupervisorKind
    from cutctx.install.runtime import start_detached_agent, start_persistent_docker, wait_ready
...
def _should_use_copilot_oauth(
    *,
    ...
    effective_backend = backend or os.environ.get("CUTCTX_BACKEND")
    ...
    if not no_proxy:
                ...
                if helpers._proxy_needs_version_restart(health_payload):
                    ...
                    if active_sessions > 0 or other_wrappers:
                        ...
                        click.echo(
                            ...
                            f"current CLI is {_CUTCTX_VERSION}."
                        )
                        click.echo(
        ...
        if helpers._check_proxy(port):
            ...
            if helpers._proxy_needs_version_restart(health_payload):
                ...
                click.echo(
                    f"  Proxy on port {port} is running Cutctx {running_version}; "
                    f"restarting with {_CUTCTX_VERSION}..."
                )
...
def _make_cleanup(proxy_proc_holder: list, port: int = 8787) -> Any:
    ...
    Only kills the proxy when no other live cutctx-wrapped clients remain,
...
def _write_openclaw_plugin_entry(openclaw_bin: str, entry: dict[str, object]) -> None:
    ...
    _run_checked(
        [
            ...
            "config",
            "set",
            "plugins.entries.cutctx",
            json.dumps(entry, separators=(",", ":")),
            "--strict-json",
        ],
        action="openclaw config set plugins.entries.cutctx",
    )
...
def claude(
    ...
    # Memory sync BEFORE proxy startup — sync cutctx DB ↔ Claude's files
    if memory:
        try:
            import subprocess as _sp

            mem_dir = Path.cwd() / ".cutctx"
    ...
    try:
        ...
        if not no_mcp:
            from cutctx.mcp_registry import ClaudeRegistrar

            _setup_cutctx_mcp(ClaudeRegistrar(), port, verbose=verbose)
        elif verbose:
...
def unwrap_claude(
    ...
    if not keep_mcp:
        ...
        if registrar.detect():
            removed_cutctx = registrar.unregister_server("cutctx")
            removed_code_graph = registrar.unregister_server(_CBM_MCP_SERVER_NAME)
            serena_status = _remove_cutctx_installed_serena_mcp(registrar)
            if removed_cutctx:
...
@click.option(
    "--backend",
    default=None,
    help="API backend for the proxy: 'anthropic', 'anyllm', 'litellm-vertex', etc. (env: CUTCTX_BACKEND)",
)
@click.option(
    "--anyllm-provider",
    default=None,
...
def codex(
    ...
    # `cutctx unwrap codex` can restore the user's pre-wrap state
    ...
    # Register cutctx MCP server in Codex config.toml so Codex can
    # call cutctx_retrieve on compression markers from the proxy.
    if not no_mcp:
        ...
        _setup_cutctx_mcp(CodexRegistrar(), port, verbose=verbose, force=True)
    elif verbose:
        click.echo("  Skipping MCP retrieve tool (--no-mcp)")
    ...
    if memory:
        ...
        try:
            ...
            from cutctx.memory.sync import sync_import
...
def cline(
    ...
    \b
    Uninstall: there is no ``cutctx unwrap cline`` subcommand. To remove the
    injected guidance, hand-edit ``.clinerules`` at the project root and
    delete everything between ``<!-- cutctx:rtk-instructions -->`` and
    ``<!-- /cutctx:rtk-instructions -->`` (inclusive). If ``lean-ctx`` mode
...
def continue_dev(
    ...
    \b
    Uninstall: there is no ``cutctx unwrap continue`` subcommand. To remove
    the injected guidance, hand-edit ``.continue/config.json`` and delete
    everything between ``<!-- cutctx:rtk-instructions -->`` and
    ``<!-- /cutctx:rtk-instructions -->`` (inclusive) from every
...
def windsurf(
    ...
    """
    from cutctx.providers.windsurf import render_setup_lines as _render_windsurf_setup_lines
...
def zed(
    ...
    """
    from cutctx.providers.zed import render_setup_lines as _render_zed_setup_lines
...
def goose(
    ...
    \b
    Uninstall: there is no ``cutctx unwrap goose`` subcommand. To remove the
...
def openhands(
    ...
    ``<!-- cutctx:rtk-instructions -->`` marker. To uninstall, simply do not
...
def openclaw(
    ...
    Example:
      ...
      cutctx wrap openclaw --plugin-path C:\\git\\cutctx\\plugins\\openclaw
    """
    if prepare_only:
    ...
    existing_entry = _read_openclaw_config_value(openclaw_bin, "plugins.entries.cutctx")
    entry = _build_openclaw_plugin_entry(
        existing_entry=existing_entry,
        proxy_port=proxy_port,
        startup_timeout_ms=startup_timeout_ms,
    ...
    inspect_result = _run_checked(
        ...
        action="openclaw plugins inspect cutctx",
    )
