...
baseline vs Cutctx accuracy + token savings.
"""
...
class BenchmarkRunResult:
    ...
    # For lm-eval (standard benchmarks): baseline vs cutctx scores
    baseline_score: float | None = None
    cutctx_score: float | None = None
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        if self.cutctx_score is not None:
            d["cutctx_score"] = round(self.cutctx_score, 4)
...
class SuiteResult:
    ...
    def standard_benchmarks(self) -> list[BenchmarkRunResult]:
        """Benchmarks with baseline vs cutctx comparison."""
...
def generate_markdown(result: SuiteResult) -> str:
    ...
    if standard:
        ...
        for b in standard:
            ...
            cutctx_str = f"{b.cutctx_score:.3f}" if b.cutctx_score is not None else "N/A"
...
def generate_html(result: SuiteResult) -> str:
    ...
    for b in standard:
        ...
        standard_rows += f"""        <tr>
            ...
            <td>{baseline_str}</td><td>{cutctx_str}</td><td>{delta_str}</td>
            <td>{comp_str}</td>
...
<html>
<head>
    <title>Cutctx Accuracy Report Card</title>
    <style>
        body {{
...
<body>
    <h1>Cutctx Accuracy Report Card</h1>
    <div class="meta">
    ...
    <footer>Generated by <strong>Cutctx Evaluation Framework</strong> | <code>pip install cutctx-ai[evals]</code></footer>
</body>
</html>"""
