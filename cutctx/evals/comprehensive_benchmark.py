...
Usage:
    ...
    python -m cutctx.evals.comprehensive_benchmark --quick

    # Full benchmark
    python -m cutctx.evals.comprehensive_benchmark --tasks mmlu,hellaswag

    # Compare with baseline
    python -m cutctx.evals.comprehensive_benchmark --compare
"""
...
class ComparisonResult:
    ...
    def delta(self) -> float:
        return self.cutctx_score - self.baseline_score

    @property
    def accuracy_preserved(self) -> bool:
        """True if Cutctx score is within 2% of baseline."""
        return self.cutctx_score >= self.baseline_score - 0.02
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "cutctx": round(self.cutctx_score, 4),
...
class BenchmarkSuiteResult:
    ...
    cutctx_results: list[BenchmarkResult] = field(default_factory=list)
    ...
    def summary(self) -> dict[str, Any]:
        return {
            ...
            "cutctx": [r.to_dict() for r in self.cutctx_results],
    ...
    def print_summary(self) -> None:
        ...
        print("\n" + "=" * 70)
        print("CUTCTX COMPREHENSIVE BENCHMARK RESULTS")
        print("=" * 70)
        ...
        if self.comparisons:
            ...
            for c in self.comparisons:
                ...
                print(
                    f"{c.task:<20} {c.baseline_score:>10.4f} {c.cutctx_score:>10.4f} {delta_str:>10} {status:>10}"
                )
            print("-" * 70)
...
def run_cutctx_benchmark(
    ...
    base_url = f"http://localhost:{cutctx_port}/v1/chat/completions"
...
def compare_results(
    ...
    cutctx_by_task = {r.task: r for r in cutctx}

    for task in baseline_by_task:
        if task in cutctx_by_task:
            b = baseline_by_task[task]
            h = cutctx_by_task[task]
            comparisons.append(
                ComparisonResult(
                    task=task,
                    metric=b.metric,
                    baseline_score=b.score,
                    cutctx_score=h.score,
                )
            )
...
def run_comprehensive_benchmark(
    ...
    Args:
        ...
        cutctx_port: Port where Cutctx proxy is running
    ...
    suite.cutctx_results = run_cutctx_benchmark(
        model=model,
        tasks=tasks,
        limit=limit,
        cutctx_port=cutctx_port,
    )

    # Compare results
    if compare_baseline:
        suite.comparisons = compare_results(suite.baseline_results, suite.cutctx_results)

    suite.total_duration_seconds = time.time() - start_time
    return suite


def check_cutctx_proxy(port: int = 8787) -> bool:
...
def main() -> None:
    ...
    parser.add_argument(
        "--cutctx-only", action="store_true", help="Only run through Cutctx (skip baseline)"
    )
    ...
    if not check_cutctx_proxy(args.port):
        print(f"ERROR: Cutctx proxy not running on port {args.port}")
        print(f"Start it with: cutctx proxy --port {args.port}")
        sys.exit(1)

    # Run benchmarks
    compare = args.compare and not args.cutctx_only
