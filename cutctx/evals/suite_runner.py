...
if TYPE_CHECKING:
    from cutctx.evals.reports.report_card import SuiteResult
...
class SuiteRunner:
    ...
    def _run_lm_eval_benchmark(
        ...
        from cutctx.evals.comprehensive_benchmark import (
            compare_results,
            run_baseline_benchmark,
            run_cutctx_benchmark,
        )
        ...
        cutctx_results = run_cutctx_benchmark(
            model=spec.model or self.model,
            tasks=tasks,
            limit=limit,
            cutctx_port=self.cutctx_port,
        )

        # Compare
        comparisons = compare_results(baseline_results, cutctx_results)
    ...
    def _run_before_after_benchmark(
        ...
        from cutctx.evals.core import EvalMode
        from cutctx.evals.datasets import load_dataset_by_name, load_tool_output_samples
        ...
        proxy_url = (
            f"http://localhost:{self.cutctx_port}" if _check_proxy(self.cutctx_port) else None
        )
    ...
    def run(self) -> SuiteResult:
        ...
        from cutctx.evals.cost_tracker import CostTracker
        ...
        if has_lm_eval:
            ...
            if not proxy_available:
                ...
                print("  Start with: cutctx proxy --port 8787")
                    ...
                    if spec.runner_type == "lm_eval":
                        ...
                        results.append(
                            BenchmarkRunResult(
                                ...
                                cutctx_score=raw.get("cutctx_score"),
                                delta=raw.get("delta"),
