"""CLI entry point for Cutctx evaluation framework.

Usage:
    python -m cutctx.evals quick              # Quick sanity check (5 samples)
    python -m cutctx.evals benchmark          # Full benchmark suite
    python -m cutctx.evals list               # List available datasets
    python -m cutctx.evals --help             # Show help

Install dependencies:
    pip install cutctx-ai[evals]
"""
...
if TYPE_CHECKING:
    from cutctx.evals.core import EvalResult


def cmd_quick(args: argparse.Namespace) -> None:
    """Run quick sanity check."""
    from cutctx.evals.runners.before_after import run_quick_eval
...
def cmd_list(args: argparse.Namespace) -> None:
    ...
    from cutctx.evals.datasets import DATASET_REGISTRY, list_available_datasets
    ...
    print("\n" + "=" * 60)
    print("Usage: python -m cutctx.evals benchmark --dataset <name> -n <samples>")
    print("=" * 60 + "\n")
...
def cmd_benchmark(args: argparse.Namespace) -> None:
    ...
    from cutctx.evals.runners.before_after import BeforeAfterRunner, LLMConfig

    print("=" * 60)
    print("CUTCTX COMPRESSION ACCURACY BENCHMARK")
    print("=" * 60)
    ...
    for name in dataset_names:
        ...
        try:
            ...
            print("  (Install with: pip install cutctx-ai[evals])")
...
def cmd_suite(args: argparse.Namespace) -> None:
    ...
    from cutctx.evals.reports.report_card import save_reports
    from cutctx.evals.suite_runner import SuiteRunner

    tiers = list(range(1, args.tier + 1))

    print("=" * 60)
    print("CUTCTX EVALUATION SUITE")
    print("=" * 60)
...
<html>
<head>
    <title>Cutctx Evaluation Report</title>
    <style>
        body {{
...
<body>
    <h1>Cutctx Compression Accuracy Report</h1>
...
"""
    ...
    html += """
    <footer>
        <p>Generated by <strong>Cutctx Evaluation Framework</strong></p>
        <p>Install: <code>pip install cutctx-ai[evals]</code></p>
    </footer>
</body>
</html>
"""
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Cutctx Evaluation Framework - Prove compression preserves accuracy",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python -m cutctx.evals quick                        # Quick 5-sample check
  python -m cutctx.evals quick -n 10                  # Quick 10-sample check
  python -m cutctx.evals list                         # List available datasets
  python -m cutctx.evals benchmark                    # Run tool_outputs benchmark
  python -m cutctx.evals benchmark --dataset hotpotqa -n 50
  python -m cutctx.evals benchmark --dataset rag      # Run all RAG datasets
  python -m cutctx.evals benchmark --dataset all      # Run ALL datasets
  python -m cutctx.evals suite --tier 1               # Run Tier 1 suite (~$3)
  python -m cutctx.evals suite --tier 2               # Run Tiers 1+2 (~$8)
  python -m cutctx.evals suite --tier 1 --ci          # CI mode (exit 1 on fail)
  python -m cutctx.evals report -i results.json       # Generate HTML report
