...
CCR_TTL_SECONDS_ENV = "CUTCTX_CCR_TTL_SECONDS"
...
class CompressionStore:
    ...
    def _search_items_from_original(self, original_content: str) -> list[Any]:
        ...
        else so `cutctx_retrieve(hash, query=...)` can find plain-text
        originals.
        """
...
_request_ccr_store: ContextVar[CompressionStore | None] = ContextVar(
    "cutctx_request_ccr_store", default=None
)
...
def _create_default_ccr_backend() -> CompressionStoreBackend | None:
    """Create a CCR backend from env (e.g. CUTCTX_CCR_BACKEND=redis).

    Loads adapters via setuptools entry point 'cutctx.ccr_backend'.
    If CUTCTX_CCR_BACKEND is empty or "sqlite", returns SqliteBackend pointing to ~/.cutctx/ccr.db.
    Returns None to use InMemoryBackend if CUTCTX_CCR_BACKEND="memory".

    In stateless mode (CUTCTX_STATELESS=true), always returns None so the
    ...
    if is_stateless():
        return None
    backend_type = (os.environ.get("CUTCTX_CCR_BACKEND") or "sqlite").strip().lower()
    if backend_type == "memory":
        return None
    if backend_type == "sqlite":
        from .backends.sqlite import SqliteBackend

        return SqliteBackend(os.path.expanduser("~/.cutctx/ccr.db"))
    try:
        from importlib.metadata import entry_points

        all_eps = entry_points(group="cutctx.ccr_backend")
        ep = next((e for e in all_eps if e.name == backend_type), None)
        if ep is None:
            logger.warning(
                "CUTCTX_CCR_BACKEND=%s but no entry point cutctx.ccr_backend[%s]",
                backend_type,
                backend_type,
            )
            return None
        fn = ep.load()
        kwargs = {
            "url": os.environ.get("CUTCTX_REDIS_URL", ""),
            "tenant_prefix": os.environ.get("CUTCTX_CCR_TENANT_PREFIX", ""),
        }
...
def get_compression_store(
    ...
    explicitly or created from env (CUTCTX_CCR_BACKEND) when building the global.
    ...
    Args:
        ...
        default_ttl: Default TTL (only used on first call for global store).
            When omitted, CUTCTX_CCR_TTL_SECONDS overrides the 300-second default.
