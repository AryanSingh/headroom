"""Plugin registry for cutctx learn.

Discovers built-in plugins from cutctx.learn.plugins.* and external
plugins registered via the ``cutctx.learn_plugin`` entry point group.

Follows the same pattern as cutctx.storage_backend (storage/__init__.py).
"""
...
def _discover() -> dict[str, LearnPlugin]:
    ...
    # 1. Built-in: scan cutctx.learn.plugins.* submodules
    from cutctx.learn import plugins as plugins_pkg

    for _, mod_name, _ in pkgutil.iter_modules(plugins_pkg.__path__):
        try:
            mod = importlib.import_module(f"cutctx.learn.plugins.{mod_name}")
    ...
    # 2. External: entry_points(group="cutctx.learn_plugin")
    try:
        from importlib.metadata import entry_points

        for ep in entry_points(group="cutctx.learn_plugin"):
            try:
                obj = ep.load()
