...
from cutctx.providers.install_registry import build_install_target_envs
...
def build_manifest(
    *,
    ...
    base_env = {
        "CUTCTX_PORT": str(port),
        "CUTCTX_HOST": "127.0.0.1",
        "CUTCTX_MODE": proxy_mode,
        "CUTCTX_BACKEND": backend,
    }
    if anyllm_provider:
        base_env["CUTCTX_ANYLLM_PROVIDER"] = anyllm_provider
    if region:
        base_env["CUTCTX_REGION"] = region
    if not telemetry_enabled:
        base_env["CUTCTX_TELEMETRY"] = "off"
    if memory_enabled:
        base_env["CUTCTX_MEMORY_ENABLED"] = "1"
    ...
    container_name = f"cutctx-{normalized_profile}"
    return DeploymentManifest(
        ...
        image=image,
        service_name=f"cutctx-{normalized_profile}",
