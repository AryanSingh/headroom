...
from .runtime import resolve_cutctx_command
...
def _command_for_script(*parts: str) -> list[str]:
    return [*resolve_cutctx_command(), *parts]
...
def _macos_launchd_plist(
    ...
    plist_path = base_dir / f"com.cutctx.{manifest.profile}.plist"
    ...
    keys = [
        ...
        "<dict>",
        "  <key>Label</key>",
        f"  <string>com.cutctx.{manifest.profile}</string>",
...
def _linux_task_spec(manifest: DeploymentManifest, ensure_script: Path) -> tuple[Path | None, str]:
    ...
    marker_start = f"# >>> cutctx {manifest.profile} >>>"
    marker_end = f"# <<< cutctx {manifest.profile} <<<"
    content = (
...
def install_supervisor(manifest: DeploymentManifest) -> list[ArtifactRecord]:
    ...
    if (
        ...
        unit_path, content = _linux_service_unit(manifest, artifact_paths["run-cutctx.sh"])
    ...
    if sys.platform.startswith("linux") and manifest.supervisor_kind == SupervisorKind.TASK.value:
        cron_path, content = _linux_task_spec(manifest, artifact_paths["ensure-cutctx.sh"])
    ...
    if sys.platform == "darwin":
        if manifest.supervisor_kind == SupervisorKind.SERVICE.value:
            plist_path, content = _macos_launchd_plist(manifest, artifact_paths["run-cutctx.sh"])
        else:
            plist_path, content = _macos_launchd_plist(
                manifest, artifact_paths["ensure-cutctx.sh"], interval=300
            )
...
def start_supervisor(manifest: DeploymentManifest) -> None:
    ...
    if sys.platform.startswith("linux"):
        ...
        return
    if sys.platform == "darwin":
        label = f"com.cutctx.{manifest.profile}"
        domain = (
            "system"
            if manifest.scope == "system"
            and manifest.supervisor_kind == SupervisorKind.SERVICE.value
