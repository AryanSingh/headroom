"""Analyze cutctx proxy logs for performance insights.

Parses PERF log lines from ~/.cutctx/logs/proxy.log* and produces
...
# Matches: 2026-03-07 13:38:31,009 - cutctx.proxy - INFO - [hr_...] PERF model=... ...
_PERF_RE = re.compile(
...
def parse_log_files(last_n_hours: float = 168.0) -> PerfReport:
    ...
    def _within_window(ts_str: str | None) -> bool:
        ...
        # alternative (silent drop) makes `cutctx perf` lie about coverage.
        if cutoff is None:
            return True
...
def format_report(report: PerfReport) -> str:
    ...
    if not report.perf_records and not report.router_records:
        lines.append("No performance data found in ~/.cutctx/logs/")
        lines.append("")
        lines.append("Start the proxy to begin collecting data:")
        lines.append("  cutctx proxy")
        return "\n".join(lines)
...
def _format_toin_highlights() -> list[str]:
    ...
    try:
        from cutctx.telemetry.toin import get_toin as _get
    ...
    lines.append(
        ...
        f"(eligible for `python -m cutctx.cli.toin_publish`)"
    )
    lines.append("")
