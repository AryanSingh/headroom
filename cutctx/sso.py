...
historical ``cutctx.sso`` path so existing call sites keep working when the
...
try:
    import cutctx_ee.sso as _impl
except ImportError as _e:  # commercial component not installed (community edition)
    raise ImportError(
        "cutctx.sso requires the proprietary 'cutctx_ee' distribution "
...
if TYPE_CHECKING:
    ...
    # sys.modules rebind above makes `from cutctx.sso import X` resolve to
    # cutctx_ee.sso; this makes the same names visible to static analysis.
    from cutctx_ee.sso import *  # noqa: F401,F403
