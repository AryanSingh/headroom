...
`pip install cutctx-ai` pulls `ast-grep-cli` as a proper PyPI binary wheel
...
Env vars:
    CUTCTX_BINARIES_MIRROR   base URL that replaces https://github.com
    CUTCTX_BINARIES_CACHE    override cache dir
    CUTCTX_BINARIES_OFFLINE  if set, never reach the network
"""
...
class OfflineError(BinaryError):
    """Raised when a network fetch is required but CUTCTX_BINARIES_OFFLINE is set."""
...
def cache_dir() -> Path:
    override = os.environ.get("CUTCTX_BINARIES_CACHE")
    if override:
...
def _asset_for_platform(tool: str, plat: PlatformKey) -> dict[str, Any]:
    ...
    if _is_pypi_tool(tool):
        raise PlatformNotSupported(
            f"{tool}: distributed via PyPI only; `pip install cutctx-ai` "
...
def _mirror_url(url: str) -> str:
    mirror = os.environ.get("CUTCTX_BINARIES_MIRROR")
    if not mirror:
        return url
...
def _download(url: str, dest: Path, *, progress: bool = True) -> None:
    if os.environ.get("CUTCTX_BINARIES_OFFLINE"):
        raise OfflineError(f"offline mode (CUTCTX_BINARIES_OFFLINE=1) but fetch required: {url}")
    ...
    try:
        with urllib.request.urlopen(req, timeout=60, context=context) as resp:  # noqa: S310 (https)
...
def resolve(tool: str) -> Path:
    ...
    OfflineError if a fetch is required but CUTCTX_BINARIES_OFFLINE is set,
    ...
    with tempfile.TemporaryDirectory(prefix="cutctx-fetch-") as tmp:
        tmp_dir = Path(tmp)
        ...
        # `cutctx proxy` starts don't race on the same .partial path.
...
def ensure_tools(quiet: bool = False) -> dict[str, Path | None]:
    ...
    Called at proxy startup and on first `cutctx` CLI invocation so that no
    ...
    for name in _registry().get("tools", {}):
        try:
            ...
            out[name] = None
            if not quiet:
                print(f"cutctx: skipping {name}: {e}", file=sys.stderr)
    return out
...
def status() -> list[dict[str, Any]]:
    ...
    Used by `cutctx tools doctor`. Never fetches — only inspects.
    """
