...
_DEFAULT_CLOUD_URL = (
    os.environ.get("CUTCTX_API_URL")
    or os.environ.get("CUTCTX_LICENSE_API_URL")
    or os.environ.get("PITCHTOSHIP_URL")
    or "https://udeekuvifncmqvoywhlg.supabase.co/functions/v1"
)
...
class CompressionMiddleware:
    ...
    Two modes:
    - Local (default): Compresses in-process using cutctx.compress().
    ...
    - x-cutctx-tokens-before: original token count
    - x-cutctx-tokens-after: compressed token count
    - x-cutctx-tokens-saved: tokens removed
    - x-cutctx-compressed: "true" if compression occurred
    """
    ...
    def __init__(
        ...
        self._api_key = api_key or os.environ.get("CUTCTX_API_KEY", "").strip() or None
        self._api_url = (
            api_url or os.environ.get("CUTCTX_API_URL", "").strip() or _DEFAULT_CLOUD_URL
        ).rstrip("/")
    ...
    async def __call__(self, scope: Scope, receive: Receive, send: Send) -> None:
        ...
        try:
            ...
            if messages:
                ...
                if result and result.get("tokens_saved", 0) > 0 and "messages" in result:
                    ...
                    logger.info(
                        "Cutctx%s: %d→%d tokens (saved %d, %.0f%%)",
        ...
        async def metrics_send(message: MutableMapping[str, Any]) -> None:
            if message["type"] == "http.response.start" and tokens_saved > 0:
                ...
                headers.append((b"x-cutctx-compressed", b"true"))
                headers.append((b"x-cutctx-tokens-before", str(tokens_before).encode()))
                headers.append((b"x-cutctx-tokens-after", str(tokens_after).encode()))
                headers.append((b"x-cutctx-tokens-saved", str(tokens_saved).encode()))
    ...
    def _local_compress(self, messages: list[dict], model: str) -> dict[str, Any] | None:
        """Compress locally using cutctx.compress()."""
