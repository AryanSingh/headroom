"""LiteLLM callback — add Cutctx compression to LiteLLM with one line.
    ...
    litellm.callbacks = [CutctxCallback(api_key="hdr_xxx")]
...
class CutctxCallback:
    ...
    def __init__(
        ...
        # Falls back to CUTCTX_API_KEY env var
    ...
    async def async_pre_call_hook(
        ...
        try:
            ...
            if result and result.get("tokens_saved", 0) > 0 and "messages" in result:
                ...
                logger.info(
                    "Cutctx%s: %d→%d tokens (saved %d, %.0f%%) [total saved: %d]",
