...
This module provides CutctxToolWrapper and wrap_tools_with_cutctx
...
Example:
    ...
    wrapped_tools = wrap_tools_with_cutctx(tools)
...
class CutctxToolWrapper:
    ...
    Example:
        from langchain.tools import Tool
        from cutctx.integrations import CutctxToolWrapper
    ...
    def _record_metrics(self, original: str, compressed: str, was_compressed: bool) -> None:
        ...
        if was_compressed and chars_saved > 0:
            logger.info(
                f"CutctxToolWrapper[{self.name}]: {chars_before} -> {chars_after} chars "
