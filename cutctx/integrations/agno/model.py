...
class CutctxAgnoModel(Model):  # type: ignore[misc]
    ...
    Example:
        ...
        optimized = CutctxAgnoModel(wrapped_model=model, cutctx_config=config)
    ...
    id: str = field(default="cutctx-wrapper")
    name: str | None = field(default=None)
    provider: str | None = field(default=None)

    # CutctxAgnoModel specific fields
    wrapped_model: Any = field(default=None)
    cutctx_config: CutctxConfig | None = field(default=None)
    cutctx_mode: CutctxMode | None = field(default=None)
    ...
    _cutctx_provider: Any = field(default=None, repr=False, compare=False)
    ...
    def __post_init__(self) -> None:
        ...
        if hasattr(self.wrapped_model, "id"):
            self.id = f"cutctx:{self.wrapped_model.id}"
        ...
        # Initialize config
        if self.cutctx_config is None:
            self.cutctx_config = CutctxConfig()

        # Handle deprecated mode parameter
        if self.cutctx_mode is not None:
            warnings.warn(
                "The 'cutctx_mode' parameter is deprecated. Use CutctxConfig(default_mode=...) instead.",
                DeprecationWarning,
                stacklevel=2,
            )
            ...
            with self._lock:
                ...
                if self._pipeline is None:
                    if self.auto_detect_provider:
                        self._cutctx_provider = get_cutctx_provider(self.wrapped_model)
                        logger.debug(
                            f"Auto-detected provider: {self._cutctx_provider.__class__.__name__}"
                        )
                    else:
                        self._cutctx_provider = OpenAIProvider()
                    self._pipeline = TransformPipeline(
                        config=self.cutctx_config,
                        provider=self._cutctx_provider,
                    )
    ...
    def _optimize_messages(self, messages: list[Any]) -> tuple[list[Any], OptimizationMetrics]:
        ...
        model_limit = (
            self._cutctx_provider.get_context_limit(model) if self._cutctx_provider else 128000
        )
