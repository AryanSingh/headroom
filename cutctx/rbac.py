...
historical ``cutctx.rbac`` path so existing call sites keep working when the
...
try:
    import cutctx_ee.rbac as _impl
except ImportError as _e:  # commercial component not installed (community edition)
    raise ImportError(
        "cutctx.rbac requires the proprietary 'cutctx_ee' distribution "
...
if TYPE_CHECKING:
    ...
    # sys.modules rebind above makes `from cutctx.rbac import X` resolve to
    # cutctx_ee.rbac; this makes the same names visible to static analysis.
    from cutctx_ee.rbac import *  # noqa: F401,F403
