...
historical ``cutctx.org`` path so existing call sites keep working when the
...
try:
    import cutctx_ee.org as _impl
except ImportError as _e:  # commercial component not installed (community edition)
    raise ImportError(
        "cutctx.org requires the proprietary 'cutctx_ee' distribution "
...
if TYPE_CHECKING:
    ...
    # sys.modules rebind above makes `from cutctx.org import X` resolve to
    # cutctx_ee.org; this makes the same names visible to static analysis.
    from cutctx_ee.org import *  # noqa: F401,F403
