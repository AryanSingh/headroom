...
# Memory tool names for detection (Cutctx's custom tools)
MEMORY_TOOL_NAMES = {
    "memory_save",
    "memory_search",
    "memory_update",
...
class MemoryConfig:
    ...
    Qdrant connection fields default to values read from ``CUTCTX_QDRANT_*``
    environment variables (see :mod:`cutctx.memory.qdrant_env`). Passing an
    explicit value to the constructor always wins over the environment.
    """

    enabled: bool = False
    backend: Literal["local", "qdrant-neo4j"] = "local"
    db_path: str = "cutctx_memory.db"
    ...
    native_memory_dir: str = ""  # Directory for native memory files (default: ~/.cutctx/memories)
    # Qdrant+Neo4j config (Qdrant defaults resolve from CUTCTX_QDRANT_* env vars)
    ...
    def __post_init__(self) -> None:
        if not self.neo4j_password or self.neo4j_password == "password":
            self.neo4j_password = secrets.token_urlsafe(16)
            logger.warning(
...
class MemoryHandler:
    ...
    def _init_native_memory_dir(self) -> None:
        ...
        if self.config.native_memory_dir:
            ...
            # Default: workspace memories directory (respects CUTCTX_WORKSPACE_DIR)
    ...
    async def _init_backend_locked(self) -> None:
        ...
        if self.config.backend == "local":
            ...
            # Opt-in GPU offload: CUTCTX_EMBEDDER_RUNTIME=pytorch_mps routes embedding
            ...
            if os.environ.get("CUTCTX_EMBEDDER_RUNTIME", "").strip().lower() == "pytorch_mps":
                try:
                    ...
                    if torch.backends.mps.is_available():
                        ...
                        logger.info(
                            "Memory: CUTCTX_EMBEDDER_RUNTIME=pytorch_mps → "
                            "torch embedder on Apple GPU (MPS)"
                        )
                    else:
                        logger.warning(
                            "Memory: CUTCTX_EMBEDDER_RUNTIME=pytorch_mps requested but "
    ...
    def _get_memory_tools(self) -> list[dict[str, Any]]:
        ...
        if self._memory_tools is None:
            from cutctx.memory.tools import get_memory_tools_optimized
        ...
        *,
        ...
        Args:
            user_id: User identifier for memory scoping (the base user
                id, derived from ``x-cutctx-user-id`` upstream).
            ...
            ranker: Optional :class:`~cutctx.proxy.memory_ranker.MemoryRanker`
        ...
        # but no x-cutctx-project-id / x-cutctx-cwd / system-prompt
        ...
        try:
            ...
            if ranker is not None:
                from cutctx.proxy.memory_ranker import MemoryCandidate
    ...
    def _append_to_latest_user_tail(
        ...
        *,
        ...
        if provider == "anthropic":
            ...
            # cutctx.proxy.handlers.anthropic which imports MemoryHandler.
        ...
        if provider == "openai":
            from cutctx.proxy.helpers import append_text_to_latest_user_chat_message
...
"""
    ...
    # ``cutctx/proxy/routes/dsr.py``. They are best-effort: each
