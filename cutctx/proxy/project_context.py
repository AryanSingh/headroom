...
``cutctx wrap`` launches agents with an ``X-Cutctx-Project`` header
...
from cutctx.proxy.savings_tracker import sanitize_project_name

PROJECT_HEADER = "x-cutctx-project"
PROJECT_PATH_PREFIX = "/p/"

_current_project: ContextVar[str | None] = ContextVar("cutctx_current_project", default=None)
...
def classify_project(headers: Mapping[str, Any] | Any) -> str | None:
    ...
    if get is None:
        return None
    value = get(PROJECT_HEADER) or get("X-Cutctx-Project")
...
def with_project_prefix(base_url: str, project: str | None) -> str:
    ...
    ``cutctx wrap`` for clients that cannot send custom headers.
