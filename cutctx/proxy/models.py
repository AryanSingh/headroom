...
from cutctx.providers.registry import ProviderApiOverrides
...
class RequestLog:
    ...
    # See ``cutctx.proxy.helpers.compute_turn_id`` for the derivation. None
...
class ProxyConfig:
    ...
    # CLI: --ccr-ttl-seconds; env: CUTCTX_CCR_TTL_SECONDS.
    ...
    # CLI: --query-aware; env: CUTCTX_QUERY_AWARE=1.
    ...
    # CLI: --selective-filter; env: CUTCTX_SELECTIVE_FILTER=1.
    ...
    # CLI: --exclude-tools <name1,name2>; env: CUTCTX_EXCLUDE_TOOLS=<name1,name2>
    ...
    # to enable from the `cutctx.proxy_extension` group, or `["*"]` for
    # wildcard. Empty/None means no extensions run, even if installed.
    # CLI: --proxy-extension <name1,name2>; env: CUTCTX_PROXY_EXTENSIONS.
    ...
    memory_db_path: str = ""  # Empty = auto: {cwd}/.cutctx/memory.db
    ...
    # x-cutctx-user-id only. ``global`` keeps the pre-fix single-DB
    ...
    # Qdrant connection (defaults resolve from CUTCTX_QDRANT_* env vars)
    ...
    # new sessions. Gated on CUTCTX_EPISODIC_MEMORY_ENABLED=1.
    ...
    # CLI: --entitlement-tier <tier>. Env: CUTCTX_ENTITLEMENT_TIER=<tier>.
    ...
    # origins (not recommended for production). Env: CUTCTX_CORS_ORIGINS.
    # Examples:
    #   CUTCTX_CORS_ORIGINS="https://app.example.com,https://staging.example.com"
    #   CUTCTX_CORS_ORIGINS="*"  # open (dev only)
    cors_origins: list[str] = field(default_factory=list)

    # Maximum request body size in MB for the compression path.
    # Bodies larger than this are forwarded unchanged. Env: CUTCTX_MAX_BODY_MB.
    ...
    scim_db_path: str | None = None  # None = ~/.cutctx/scim.db
    ...
    # Env: CUTCTX_STRUCTURED_OUTPUT_ENABLED=1
    structured_output_enabled: bool = True
    structured_output_max_retries: int = 3

    # Multi-Model Ensemble — fan-out to multiple models + evaluator
    # When enabled, requests to cutctx-ensemble-v1 are fanned out.
    # Env: CUTCTX_ENSEMBLE_ENABLED=1
    ...
    # Env: CUTCTX_TASK_AWARE_ENABLED=1
    ...
    # Env: CUTCTX_SSO_DISCOVERY_URL, CUTCTX_SSO_JWKS_URI, etc.
    ...
    # ``CUTCTX_COMPRESSION_MAX_WORKERS``.
    #
    ...
    # ``CutctxProxy._run_compression_in_executor``.
