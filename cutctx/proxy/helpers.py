...
_CODEX_WIRE_DEBUG_ENV = "CUTCTX_CODEX_WIRE_DEBUG"
_CODEX_WIRE_DEBUG_DIR_ENV = "CUTCTX_CODEX_WIRE_DEBUG_DIR"
...
#     forwards untouched.
#
# Configurable via CUTCTX_MEMORY_INJECTION_MODE env var. There is no
...
_MEMORY_INJECTION_MODE_ENV = "CUTCTX_MEMORY_INJECTION_MODE"
...
_PYTHON_FORWARDER_MODE_ENV = "CUTCTX_PROXY_PYTHON_FORWARDER_MODE"
...
def extract_tags(headers: Any) -> dict[str, str]:
    """Extract ``x-cutctx-*`` tags from inbound headers.
...
def _cutctx_bypass_enabled(headers: Any) -> bool:
    ...
    try:
        bypass = str(headers.get("x-cutctx-bypass", "")).strip().lower() == "true"
        passthrough = str(headers.get("x-cutctx-mode", "")).strip().lower() == "passthrough"
...
RTK_STATS_CACHE_TTL_SECONDS = float(os.environ.get("CUTCTX_CONTEXT_TOOL_STATS_TTL_SECONDS", "60"))
...
# configurable via CUTCTX_MAX_BODY_MB env var).
...
_SSE_EVENT_MAX_BYTES_ENV = "CUTCTX_SSE_BUFFER_MAX_BYTES"
...
# Configurable via CUTCTX_PROXY_BODY_TOO_LARGE_STATUS for operators who need
# to override (no expected production use; documentation knob).
_BODY_TOO_LARGE_STATUS_ENV = "CUTCTX_PROXY_BODY_TOO_LARGE_STATUS"
...
# ``CUTCTX_WS_FAIL_OPEN_ON_COMPRESSION_FAILURE=1``. The oversize threshold
# below which transient errors still fall through to passthrough is
# configurable via ``CUTCTX_WS_COMPRESSION_FAIL_THRESHOLD_BYTES``
# (default 256 KiB ≈ 64K tokens).
WS_COMPRESSION_FAIL_OPEN_ENV = "CUTCTX_WS_FAIL_OPEN_ON_COMPRESSION_FAILURE"
WS_COMPRESSION_OVERSIZE_BYTES_ENV = "CUTCTX_WS_COMPRESSION_FAIL_THRESHOLD_BYTES"
...
# Always-on file logging to the workspace logs directory for `cutctx perf` analysis.
...
def is_stateless() -> bool:
    ...
    Checks the ``CUTCTX_STATELESS`` env var. Used by components that don't
...
def _cutctx_log_dir() -> Path:
    return _paths.log_dir()


def _setup_file_logging() -> None:
    """Add a RotatingFileHandler to the cutctx root logger.

    Writes to ~/.cutctx/logs/proxy.log with automatic rotation:
    - Rotates at 10 MB
    - Keeps 5 backups (~50 MB max)

    Skipped entirely when CUTCTX_STATELESS=true (no filesystem writes).
    """
    ...
    try:
        ...
        # Attach to the cutctx root logger so all sub-loggers are captured.
...
# ---------------------------------------------------------------------------
#
# `x-cutctx-*` request headers (e.g. ``x-cutctx-bypass``,
# ``x-cutctx-mode``, ``x-cutctx-user-id``, ``x-cutctx-stack``,
# ``x-cutctx-base-url``) are internal control flags consumed by the
...
# ``x-cutctx-*``, memory ``x-cutctx-user-id`` lookup) keep using
# the original dict / ``request.headers``. The stripped copy is what
# every upstream-bound forwarder receives.
#
# Note: response-side ``X-Cutctx-*`` injection (e.g.
# ``x-cutctx-tokens-saved``) is unrelated — the proxy is allowed to
# tell its client about its own work. This helper only filters
# request-side headers.

_INTERNAL_HEADER_PREFIX = "x-cutctx-"

# Operator opt-in env var. ``enabled`` (default) strips internal
# ``x-cutctx-*`` headers from every upstream-bound forwarder.
...
_STRIP_INTERNAL_HEADERS_ENV = "CUTCTX_STRIP_INTERNAL_HEADERS"
...
def _strip_internal_headers(headers: dict[str, str]) -> dict[str, str]:
    """Return a copy of ``headers`` with internal ``x-cutctx-*`` keys stripped.

    Used at every upstream call site to prevent fingerprinting / leakage of
    internal flags like ``x-cutctx-bypass``, ``x-cutctx-mode``,
    ``x-cutctx-user-id``, ``x-cutctx-stack``, ``x-cutctx-base-url``.
    Case-insensitive on the prefix. Returns a NEW dict; never mutates the
    caller's mapping. Pure function. No regex.

    When the operator opt-in ``CUTCTX_STRIP_INTERNAL_HEADERS=disabled``
...
#
# Operator opt-in `CUTCTX_BETA_HEADER_STICKY=disabled` short-circuits
...
_BETA_HEADER_STICKY_ENV = "CUTCTX_BETA_HEADER_STICKY"
BetaHeaderStickyMode = Literal["enabled", "disabled"]
_BETA_HEADER_STICKY_DEFAULT: BetaHeaderStickyMode = "enabled"

_BETA_TRACKER_MAX_SESSIONS_ENV = "CUTCTX_BETA_TRACKER_MAX_SESSIONS"
...
def _merge_beta_tokens(client_value: str | None, cutctx_required: list[str]) -> str:
...
def merge_anthropic_beta(client_value: str | None, cutctx_required: list[str]) -> str:
    ...
    client tokens first (in their original order), then cutctx tokens
    ...
    Returns ``""`` when both inputs are empty.
    """
    return _merge_beta_tokens(client_value, cutctx_required)


def merge_openai_beta(client_value: str | None, cutctx_required: list[str]) -> str:
...
class SessionBetaTracker:
    ...
    def record_and_get_sticky_betas(
        ...
        When `CUTCTX_BETA_HEADER_STICKY=disabled` returns the client
...
def get_session_beta_tracker() -> SessionBetaTracker:
    ...
    Lazily constructed so the env-var bound (`CUTCTX_BETA_TRACKER_MAX_SESSIONS`)
...
def log_beta_header_merge(
    *,
    ...
    cutctx_added: list[str],
    request_id: str | None,
) -> None:
    """Structured log for every cache-affecting beta-header merge.

    `cutctx_added` is a list of public, documented beta tokens
    ...
    logger.info(
        ...
        "client_betas=%d sticky_betas=%d cutctx_added=%s request_id=%s",
        provider,
        session_id or "",
        client_betas_count,
        sticky_betas_count,
        ",".join(cutctx_added) if cutctx_added else "",
        request_id or "",
    )
...
#
# Operator opt-in `CUTCTX_TOOL_INJECTION_STICKY=disabled` short-
...
_TOOL_INJECTION_STICKY_ENV = "CUTCTX_TOOL_INJECTION_STICKY"
...
def serialize_tool_definition_canonical(tool_definition: dict[str, Any]) -> bytes:
    ...
    ``cutctx/proxy/memory_handler.py`` and
    ``cutctx/proxy/memory_tool_adapter.py`` already do.
...
def get_session_tool_tracker() -> SessionToolTracker:
    ...
    (`CUTCTX_TOOL_TRACKER_MAX_SESSIONS`) is honored at first use.
...
def apply_session_sticky_memory_tools(
    *,
    ...
    Logic (guide §6.3 #2):
      ...
      * If ``CUTCTX_TOOL_INJECTION_STICKY=disabled``: bypass tracker,
...
# `cutctx_retrieve` tool stays registered in `body["tools"]` for every
...
class SessionCcrTracker:
    ...
    Two pieces of state per session:
      ...
      * ``golden_tool_bytes``: canonical serialization of the
        ``cutctx_retrieve`` tool definition recorded the first time
...
def apply_session_sticky_ccr_tool(
    *,
    ...
    """
    from cutctx.ccr.tool_injection import CCR_TOOL_NAME, create_ccr_tool_definition
...
# ``cutctx wrap claude``). For users who run ``claude`` manually we cannot
...
def format_tool_search_disabled_hint(tools: list[Any]) -> str:
    ...
    return (
        ...
        "or launch via `cutctx wrap claude` (which sets it automatically). "
        "See https://github.com/cutctx/cutctx/issues/746"
    )
