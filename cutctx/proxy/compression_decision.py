...
This is the input-side analog of :class:`cutctx.proxy.outcome.RequestOutcome`.
...
``x-cutctx-bypass: true`` requests were silently ignored on every
    ...
    def decide(
        ...
        *,
        ...
        headers
            ...
            Both ``x-cutctx-bypass: true`` and
            ``x-cutctx-mode: passthrough`` trigger bypass — semantics
            mirrored from :func:`cutctx.proxy.helpers._cutctx_bypass_enabled`.
        config
            ``CutctxConfig``-shaped object; only ``optimize: bool``
            is read.
        usage_reporter
