...
``CutctxProxy.startup`` so that request-time use shares one in-process
singleton and concurrent first-use callers do not trigger N parallel loads.

The registry is populated by ``CutctxProxy.startup`` and exposed as
...
class WarmupRegistry:
    """Shared preloaded asset registry populated by ``CutctxProxy.startup``.

    One instance per ``CutctxProxy``. All writes happen on the startup
