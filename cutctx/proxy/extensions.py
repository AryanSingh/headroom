...
entry point in the ``cutctx.proxy_extension`` group in their ``pyproject.toml``:

    [project.entry-points."cutctx.proxy_extension"]
...
Enabling extensions:
  ...
  * CLI:  ``cutctx proxy --proxy-extension shield_enterprise,mypkg``
  * Env:  ``CUTCTX_PROXY_EXTENSIONS=shield_enterprise,mypkg``
...
ENTRY_POINT_GROUP = "cutctx.proxy_extension"
ENV_VAR = "CUTCTX_PROXY_EXTENSIONS"
...
def _resolve_enabled(enabled: Iterable[str] | None) -> set[str]:
    ...
    Precedence: explicit ``enabled`` argument > ``CUTCTX_PROXY_EXTENSIONS``
