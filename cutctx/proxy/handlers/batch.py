...
class BatchHandlerMixin:
    ...
    async def handle_google_batch_create(
        ...
        _pre_strip_count_gb = sum(1 for k in headers if k.lower().startswith("x-cutctx-"))
    ...
    async def _google_batch_passthrough(
        ...
        _pre_strip_count_gpt = sum(1 for k in headers if k.lower().startswith("x-cutctx-"))
        headers = _strip_internal_headers(headers)
        log_outbound_headers(
        ...
        from cutctx.proxy.helpers import get_python_forwarder_mode
    ...
    async def handle_google_batch_passthrough(
        ...
        _pre_strip_count_gp = sum(1 for k in headers if k.lower().startswith("x-cutctx-"))
        headers = _strip_internal_headers(headers)
        log_outbound_headers(
    ...
    async def handle_google_batch_results(
        ...
        _pre_strip_count_gbr = sum(1 for k in headers if k.lower().startswith("x-cutctx-"))
        headers = _strip_internal_headers(headers)
        log_outbound_headers(
    ...
    async def handle_batch_create(self, request: Request) -> Response:
        ...
        _pre_strip_count_oacc = sum(1 for k in headers if k.lower().startswith("x-cutctx-"))
        headers = _strip_internal_headers(headers)
        log_outbound_headers(
        ...
        try:
            ...
            compression_metadata = {
                **metadata,
                "cutctx_compressed": "true",
                "cutctx_original_file_id": input_file_id,
                "cutctx_total_requests": str(stats["total_requests"]),
                "cutctx_tokens_saved": str(stats["total_tokens_saved"]),
                "cutctx_original_tokens": str(stats["total_original_tokens"]),
                "cutctx_compressed_tokens": str(stats["total_compressed_tokens"]),
                "cutctx_savings_percent": f"{stats['savings_percent']:.1f}",
            }
            ...
            response_headers.pop("content-encoding", None)
            response_headers.pop("content-length", None)
            response_headers["x-cutctx-tokens-saved"] = str(stats["total_tokens_saved"])
            response_headers["x-cutctx-savings-percent"] = f"{stats['savings_percent']:.1f}"
    ...
    async def _batch_passthrough(self, request: Request, body: dict) -> Response:
        ...
        headers.pop("content-length", None)
        # PR-A5 (P5-49): strip internal x-cutctx-* before forwarding upstream.
        _pre_strip_count_obp = sum(1 for k in headers if k.lower().startswith("x-cutctx-"))
        headers = _strip_internal_headers(headers)
        log_outbound_headers(
