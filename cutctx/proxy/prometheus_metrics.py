...
if TYPE_CHECKING:
    from cutctx.observability import CutctxOtelMetrics
    from cutctx.proxy.cost import CostTracker

from cutctx.observability import get_otel_metrics
...
class PrometheusMetrics:
    ...
    def __init__(
        ...
        # Populated via X-Cutctx-Stack header (TS SDK adapters, etc.)
        ...
        # `cutctx_compressions_total{strategy="smart_crusher"}`
    ...
    def record_stack(self, stack: str | None) -> None:
        ...
        ``stack`` is the ``X-Cutctx-Stack`` header value (e.g.
        ...
        from cutctx.telemetry.context import MAX_DISTINCT_STACKS, normalize_stack
    ...
    def record_compression(
        ...
        """Implements `cutctx.transforms.observability.CompressionObserver`.
    ...
    async def export(self) -> str:
        ...
        async with self._lock:
            ...
            _append_metric(
                lines,
                name="cutctx_requests_cached_total",
                metric_type="counter",
            ...
            _append_metric(
                lines,
                name="cutctx_requests_rate_limited_total",
                metric_type="counter",
            ...
            _append_metric(
                lines,
                name="cutctx_requests_failed_total",
                metric_type="counter",
            ...
            _append_metric(
                lines,
                name="cutctx_inbound_requests_total",
                metric_type="counter",
            ...
            _append_metric(
                lines,
                name="cutctx_inbound_requests_completed_total",
                metric_type="counter",
            ...
            _append_metric(
                lines,
                name="cutctx_inbound_requests_active",
                metric_type="gauge",
            ...
            _append_metric(
                lines,
                name="cutctx_tokens_input_total",
                metric_type="counter",
            ...
            _append_metric(
                lines,
                name="cutctx_tokens_output_total",
                metric_type="counter",
            ...
            _append_metric(
                lines,
                name="cutctx_tokens_saved_total",
                metric_type="counter",
            ...
            _append_metric(
                lines,
                name="cutctx_cache_bust_tokens_lost_total",
                metric_type="counter",
            ...
            lines.extend(
                [
                    "# HELP cutctx_requests_by_provider Requests by provider",
                    "# TYPE cutctx_requests_by_provider counter",
                ]
            )
            for provider, count in self.requests_by_provider.items():
                lines.append(f'cutctx_requests_by_provider{{provider="{provider}"}} {count}')
            lines.append("")

            lines.extend(
                [
                    "# HELP cutctx_requests_by_model Requests by model",
                    "# TYPE cutctx_requests_by_model counter",
                ]
            )
            for model, count in self.requests_by_model.items():
                lines.append(f'cutctx_requests_by_model{{model="{model}"}} {count}')
            lines.append("")

            if self.transform_timing_sum:
                lines.extend(
                    [
                        "# HELP cutctx_transform_timing_ms_sum Sum of transform timing in milliseconds",
                        "# TYPE cutctx_transform_timing_ms_sum counter",
                    ]
                )
                for name, total in self.transform_timing_sum.items():
                    lines.append(
                        f'cutctx_transform_timing_ms_sum{{transform="{_escape_label_value(name)}"}} {round(total, 2)}'
                    )
                lines.extend(
                    [
                        "",
                        "# HELP cutctx_transform_timing_ms_count Count of transform timing samples",
                        "# TYPE cutctx_transform_timing_ms_count counter",
                    ]
                )
                for name, count in self.transform_timing_count.items():
                    lines.append(
                        f'cutctx_transform_timing_ms_count{{transform="{_escape_label_value(name)}"}} {count}'
                    )
                lines.extend(
                    [
                        "",
                        "# HELP cutctx_transform_timing_ms_max Maximum transform timing in milliseconds",
                        "# TYPE cutctx_transform_timing_ms_max gauge",
                    ]
                )
                for name, max_value in self.transform_timing_max.items():
                    lines.append(
                        f'cutctx_transform_timing_ms_max{{transform="{_escape_label_value(name)}"}} {round(max_value, 2)}'
                    )
                lines.append("")
            ...
            if stage_timing_sum_snapshot:
                lines.extend(
                    [
                        "# HELP cutctx_stage_timing_ms_sum Sum of per-stage handler timings in milliseconds",
                        "# TYPE cutctx_stage_timing_ms_sum counter",
                    ]
                )
                for (path_label, stage), total in stage_timing_sum_snapshot.items():
                    lines.append(
                        f'cutctx_stage_timing_ms_sum{{path="{_escape_label_value(path_label)}",stage="{_escape_label_value(stage)}"}} {round(total, 2)}'
                    )
                lines.extend(
                    [
                        "",
                        "# HELP cutctx_stage_timing_ms_count Count of per-stage handler timing samples",
                        "# TYPE cutctx_stage_timing_ms_count counter",
                    ]
                )
                for (path_label, stage), count in stage_timing_count_snapshot.items():
                    lines.append(
                        f'cutctx_stage_timing_ms_count{{path="{_escape_label_value(path_label)}",stage="{_escape_label_value(stage)}"}} {count}'
                    )
                lines.extend(
                    [
                        "",
                        "# HELP cutctx_stage_timing_ms_max Maximum per-stage handler timing in milliseconds",
                        "# TYPE cutctx_stage_timing_ms_max gauge",
                    ]
                )
                for (path_label, stage), max_value in stage_timing_max_snapshot.items():
                    lines.append(
                        f'cutctx_stage_timing_ms_max{{path="{_escape_label_value(path_label)}",stage="{_escape_label_value(stage)}"}} {round(max_value, 2)}'
                    )
                lines.append("")
            ...
            lines.extend(
                [
                    ...
                    f"cutctx_active_relay_tasks {self.active_relay_tasks}",
                    "",
                ]
            )
            if self.ws_session_duration_sum_ms:
                lines.extend(
                    [
                        "# HELP cutctx_ws_session_duration_ms_sum Sum of Codex WS session durations",
                        "# TYPE cutctx_ws_session_duration_ms_sum counter",
                    ]
                )
                for cause, total in self.ws_session_duration_sum_ms.items():
                    lines.append(
                        f'cutctx_ws_session_duration_ms_sum{{cause="{_escape_label_value(cause)}"}} {round(total, 2)}'
                    )
                lines.extend(
                    [
                        "",
                        "# HELP cutctx_ws_session_duration_ms_count Count of completed Codex WS sessions",
                        "# TYPE cutctx_ws_session_duration_ms_count counter",
                    ]
                )
                for cause, count in self.ws_session_duration_count.items():
                    lines.append(
                        f'cutctx_ws_session_duration_ms_count{{cause="{_escape_label_value(cause)}"}} {count}'
                    )
                lines.extend(
                    [
                        "",
                        "# HELP cutctx_ws_session_duration_ms_max Maximum Codex WS session duration",
                        "# TYPE cutctx_ws_session_duration_ms_max gauge",
                    ]
                )
                for cause, max_value in self.ws_session_duration_max_ms.items():
                    lines.append(
                        f'cutctx_ws_session_duration_ms_max{{cause="{_escape_label_value(cause)}"}} {round(max_value, 2)}'
                    )
                lines.append("")
            ...
            if self.waste_signals_total:
                lines.extend(
                    [
                        "# HELP cutctx_waste_signal_tokens_total Tokens attributed to detected waste signals",
                        "# TYPE cutctx_waste_signal_tokens_total counter",
                    ]
                )
            ...
            if self.cache_by_provider:
                lines.extend(
                    [
                        "# HELP cutctx_cache_read_tokens_total Provider cache read tokens",
                        "# TYPE cutctx_cache_read_tokens_total counter",
                    ]
                )
                for provider, stats in self.cache_by_provider.items():
                    lines.append(
                        f'cutctx_cache_read_tokens_total{{provider="{provider}"}} {stats["cache_read_tokens"]}'
                    )
                lines.extend(
                    [
                        "",
                        "# HELP cutctx_cache_write_tokens_total Provider cache write tokens",
                        "# TYPE cutctx_cache_write_tokens_total counter",
                    ]
                )
                for provider, stats in self.cache_by_provider.items():
                    lines.append(
                        f'cutctx_cache_write_tokens_total{{provider="{provider}"}} {stats["cache_write_tokens"]}'
                    )
                lines.extend(
                    [
                        "",
                        "# HELP cutctx_cache_write_ttl_tokens_total Provider cache write tokens by observed TTL bucket",
                        "# TYPE cutctx_cache_write_ttl_tokens_total counter",
                    ]
                )
                for provider, stats in self.cache_by_provider.items():
                    lines.append(
                        f'cutctx_cache_write_ttl_tokens_total{{provider="{provider}",ttl="5m"}} {stats["cache_write_5m_tokens"]}'
                    )
                    lines.append(
                        f'cutctx_cache_write_ttl_tokens_total{{provider="{provider}",ttl="1h"}} {stats["cache_write_1h_tokens"]}'
                    )
                lines.extend(
                    [
                        "",
                        "# HELP cutctx_cache_write_ttl_requests_total Provider cache write requests by observed TTL bucket",
                        "# TYPE cutctx_cache_write_ttl_requests_total counter",
                    ]
                )
                for provider, stats in self.cache_by_provider.items():
                    lines.append(
                        f'cutctx_cache_write_ttl_requests_total{{provider="{provider}",ttl="5m"}} {stats["cache_write_5m_requests"]}'
                    )
                    lines.append(
                        f'cutctx_cache_write_ttl_requests_total{{provider="{provider}",ttl="1h"}} {stats["cache_write_1h_requests"]}'
                    )
                lines.extend(
                    [
                        "",
                        "# HELP cutctx_uncached_input_tokens_total Input tokens not served from provider cache",
                        "# TYPE cutctx_uncached_input_tokens_total counter",
                    ]
                )
                for provider, stats in self.cache_by_provider.items():
                    lines.append(
                        f'cutctx_uncached_input_tokens_total{{provider="{provider}"}} {stats["uncached_input_tokens"]}'
                    )
                lines.extend(
                    [
                        "",
                        "# HELP cutctx_provider_cache_requests_total Requests with provider cache observations",
                        "# TYPE cutctx_provider_cache_requests_total counter",
                    ]
                )
                for provider, stats in self.cache_by_provider.items():
                    lines.append(
                        f'cutctx_provider_cache_requests_total{{provider="{provider}"}} {stats["requests"]}'
                    )
                lines.extend(
                    [
                        "",
                        "# HELP cutctx_provider_cache_hit_requests_total Requests with provider cache reads",
                        "# TYPE cutctx_provider_cache_hit_requests_total counter",
                    ]
                )
                for provider, stats in self.cache_by_provider.items():
                    lines.append(
                        f'cutctx_provider_cache_hit_requests_total{{provider="{provider}"}} {stats["hit_requests"]}'
                    )
                lines.extend(
                    [
                        "",
                        "# HELP cutctx_provider_cache_bust_total Provider-specific cache bust count",
                        "# TYPE cutctx_provider_cache_bust_total counter",
                    ]
                )
                for provider, stats in self.cache_by_provider.items():
                    lines.append(
                        f'cutctx_provider_cache_bust_total{{provider="{provider}"}} {stats["bust_count"]}'
                    )
                lines.extend(
                    [
                        "",
                        "# HELP cutctx_provider_cache_bust_write_tokens_total Provider cache write tokens attributed to busts",
                        "# TYPE cutctx_provider_cache_bust_write_tokens_total counter",
                    ]
                )
                for provider, stats in self.cache_by_provider.items():
                    lines.append(
                        f'cutctx_provider_cache_bust_write_tokens_total{{provider="{provider}"}} {stats["bust_write_tokens"]}'
                    )
                lines.append("")
            ...
            # init.
            from cutctx.proxy.request_logger import redactions_total
            ...
            # `cutctx wrap` CLI (cutctx.cli.wrap); the proxy
            ...
            from cutctx.cli.wrap_rtk_metrics import rtk_invocation_counts
