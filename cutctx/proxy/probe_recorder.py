...
``cutctx evals probes`` can measure what compression removed from real
proxied sessions. Activated only when ``CUTCTX_PROBE_RECORD_DIR`` is set;
...
RECORD_DIR_ENV = "CUTCTX_PROBE_RECORD_DIR"
...
def probe_recorder_from_env() -> CompressionEventRecorder | None:
    """Build a recorder when ``CUTCTX_PROBE_RECORD_DIR`` is set, else None.
