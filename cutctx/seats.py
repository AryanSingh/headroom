...
historical ``cutctx.seats`` path so existing call sites keep working when the
...
try:
    import cutctx_ee.seats as _impl
except ImportError as _e:  # commercial component not installed (community edition)
    raise ImportError(
        "cutctx.seats requires the proprietary 'cutctx_ee' distribution "
...
if TYPE_CHECKING:
    ...
    # sys.modules rebind above makes `from cutctx.seats import X` resolve to
    # cutctx_ee.seats; this makes the same names visible to static analysis.
    from cutctx_ee.seats import *  # noqa: F401,F403
