...
Usage:
    ...
    # CUTCTX_SENTENCE_TRANSFORMER=intfloat/e5-small-v2
    # CUTCTX_SIGLIP=google/siglip-base-patch16-224
"""
...
class MLModelConfig:
    ...
    - CUTCTX_SENTENCE_TRANSFORMER
    - CUTCTX_SIGLIP
    - CUTCTX_SPACY
    - CUTCTX_TECHNIQUE_ROUTER
    ...
    sentence_transformer: str = field(
        default_factory=lambda: os.environ.get("CUTCTX_SENTENCE_TRANSFORMER", "all-MiniLM-L6-v2")
    )
    sentence_transformer_dim: int = 384

    # Image Embeddings (SIGLIP)
    siglip: str = field(
        default_factory=lambda: os.environ.get("CUTCTX_SIGLIP", "google/siglip-base-patch16-224")
    )

    # Named Entity Recognition (spaCy)
    spacy: str = field(default_factory=lambda: os.environ.get("CUTCTX_SPACY", "en_core_web_sm"))

    # Image Technique Router
    technique_router: str = field(
        default_factory=lambda: os.environ.get(
            "CUTCTX_TECHNIQUE_ROUTER", "chopratejas/technique-router"
        )
    )
