...
- Thread-safe state updates via threading.Lock (consistent with cutctx patterns)
...
from cutctx.subscription.base import QuotaTracker
from cutctx.subscription.client import SubscriptionClient
...
_PERSIST_FILE_ENV = _paths.CUTCTX_SUBSCRIPTION_STATE_PATH_ENV
_DEFAULT_PERSIST_DIR = ".cutctx"
...
# without uninstalling the binary or unsetting ``CUTCTX_CONTEXT_TOOL``.
# Used for diagnostics and for environments where RTK is intentionally
# excluded from cutctx accounting (e.g. shadow tests).
#
...
_RTK_WIRING_ENV = "CUTCTX_RTK_WIRING"
...
# ``cutctx/proxy/server.py``: a non-blocking ``fcntl.flock`` on
# ``CUTCTX_RTK_POLL_LOCK`` (default under the workspace dir). Only the
...
_RTK_POLL_LOCK_ENV = "CUTCTX_RTK_POLL_LOCK"
...
def _validate_rtk_env_at_startup() -> None:
    ...
    Raises ``ValueError`` loudly if ``CUTCTX_RTK_WIRING`` is set to an
...
class SubscriptionTracker(QuotaTracker):
    ...
    Implements :class:`~cutctx.subscription.base.QuotaTracker` so it can
    be registered with :func:`~cutctx.subscription.base.get_quota_registry`
    ...
    def __init__(
        ...
        # :func:`~cutctx.proxy.helpers._get_context_tool_stats`) so it
    ...
    def update_contribution(
        ...
        *,
        ...
        """Update cutctx contribution counters for the current session window.
        ...
        :func:`cutctx.proxy.helpers._get_rtk_stats`) when the caller does
    ...
    def _poll_rtk_delta(self) -> int:
        ...
        :func:`cutctx.proxy.helpers._get_rtk_stats` and reads the
        ...
        - ``CUTCTX_RTK_WIRING=disabled`` — operator opt-out.
        ...
        PR-G2 remediation (H1): ``CUTCTX_RTK_WIRING`` is now validated at
        ...
        try:
            ...
            from cutctx.proxy.helpers import _get_rtk_stats
    ...
    def _try_acquire_rtk_poll_lock(self) -> bool:
        ...
        ``cutctx/proxy/server.py`` (fcntl.flock, LOCK_EX | LOCK_NB).
        """
    ...
    def _compute_used_since_reset(self, window: RateLimitWindow | None) -> int | None:
        ...
        if now < window.resets_at:
            return None
        try:
            from cutctx.subscription import session_tracking
    ...
    async def _maybe_poll(self) -> None:
        ...
        if not is_active:
            ...
            from cutctx.subscription.client import read_cached_oauth_token
        ...
        try:
            from cutctx.observability.metrics import get_otel_metrics
    ...
    def _maybe_reset_contribution(self, snapshot: SubscriptionSnapshot) -> None:
        ...
        if (
            ...
            logger.info("5h window rolled over; resetting cutctx contribution counters")
...
def configure_subscription_tracker(
    ...
    a typo (``CUTCTX_RTK_WIRING=enabld``) crashes the proxy at startup
