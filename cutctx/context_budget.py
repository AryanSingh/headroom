...
class BudgetPolicy:
    ...
    def from_env(cls, policy: str = "balanced") -> BudgetPolicy:
        ...
        Environment variables override defaults:
            CUTCTX_BUDGET_GREEN (default 0.60)
            CUTCTX_BUDGET_YELLOW (default 0.80)
            CUTCTX_BUDGET_RED (default 0.95)
            CUTCTX_BUDGET_WINDOW_YELLOW (default 10)
            CUTCTX_BUDGET_WINDOW_RED (default 5)

        If none set, uses named preset: 'conservative', 'balanced', 'aggressive'.
        """
        # Check for explicit env vars first
        green = os.getenv("CUTCTX_BUDGET_GREEN")
        yellow = os.getenv("CUTCTX_BUDGET_YELLOW")
        red = os.getenv("CUTCTX_BUDGET_RED")
        window_yellow = os.getenv("CUTCTX_BUDGET_WINDOW_YELLOW")
        window_red = os.getenv("CUTCTX_BUDGET_WINDOW_RED")
...
class ContextBudgetController:
    ...
    def _compress_messages_in_zone(
        ...
        try:
            ...
            logger.warning("cutctx.compress not available; returning messages unchanged")
            return messages
    ...
    def _get_cost_per_token(self) -> float:
        ...
        """
        try:
            from cutctx.proxy.cost import _get_litellm_module
