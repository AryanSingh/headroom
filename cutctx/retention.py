...
historical ``cutctx.retention`` path so existing call sites keep working when the
...
try:
    import cutctx_ee.retention as _impl
except ImportError as _e:  # commercial component not installed (community edition)
    raise ImportError(
        "cutctx.retention requires the proprietary 'cutctx_ee' distribution "
...
if TYPE_CHECKING:
    ...
    # sys.modules rebind above makes `from cutctx.retention import X` resolve to
    # cutctx_ee.retention; this makes the same names visible to static analysis.
    from cutctx_ee.retention import *  # noqa: F401,F403
