...
def _load_custom_model_config() -> dict[str, Any]:
    ...
    Checks (in order):
    1. CUTCTX_MODEL_LIMITS environment variable (JSON string or file path)
    2. ~/.cutctx/models.json config file
    ...
    env_config = os.environ.get("CUTCTX_MODEL_LIMITS", "")
    if env_config:
        try:
            ...
            logger.debug(f"Loaded custom model config from CUTCTX_MODEL_LIMITS: {loaded}")
        except (json.JSONDecodeError, OSError) as e:
            logger.warning(f"Failed to load CUTCTX_MODEL_LIMITS: {e}")
...
class AnthropicProvider(Provider):
    ...
    Custom Model Configuration:
        ...
        1. Environment variable (JSON string):
           export CUTCTX_MODEL_LIMITS='{"context_limits": {"my-model": 200000}}'

        2. Environment variable (file path):
           export CUTCTX_MODEL_LIMITS=/path/to/models.json

        3. Config file (~/.cutctx/models.json):
           {
             "anthropic": {
    ...
    def get_context_limit(self, model: str) -> int:
        ...
        2. CUTCTX_MODEL_LIMITS environment variable
        3. ~/.cutctx/models.json config file
