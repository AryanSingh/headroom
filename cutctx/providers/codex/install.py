...
from cutctx.install.models import ConfigScope, DeploymentManifest, ManagedMutation, ToolTarget
from cutctx.install.paths import codex_config_path
...
_ORPHAN_PROVIDER_TABLE = re.compile(
    r"(?ms)^\[model_providers\.(cutctx|cutctx)\][^\[]*?"
...
def revert_provider_scope(mutation: ManagedMutation, manifest: DeploymentManifest) -> None:
    ...
    # left outside the marker block (mirrors wrap.py _strip_codex_cutctx_blocks).
    content = _ORPHAN_MODEL_PROVIDER.sub("", content)
    content = _ORPHAN_OPENAI_BASE_URL.sub("", content)
    content = _ORPHAN_PROVIDER_TABLE.sub("", content)
