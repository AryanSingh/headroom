...
LEGACY_STATS_TOOL_NAME = "cutctx_stats"
LEGACY_READ_TOOL_NAME = "cutctx_read"

logger = logging.getLogger("cutctx.ccr.mcp")

# Feature flag: enable cutctx_read tool (file read caching via CCR)
# Set CUTCTX_MCP_READ=on to enable
_READ_ENABLED = os.environ.get("CUTCTX_MCP_READ", "off").lower().strip() in (
    "on",
    "true",
    "1",
    "yes",
...
def _format_session_summary(summary: dict[str, Any], local_stats: dict[str, Any]) -> str:
    ...
    without = cost.get("without_cutctx_usd", 0)
    with_hr = cost.get("with_cutctx_usd", 0)
...
# cutctx_stats aggregates across all instances within the session window.
# Respects CUTCTX_WORKSPACE_DIR.
...
class CutctxMCPServer:
    ...
    def _setup_handlers(self) -> None:
        ...
        async def call_tool(name: str, arguments: dict[str, Any]) -> list[TextContent]:
            ...
            try:
                ...
                elif name in ("cutctx_proxy_start", "cutctx_scan", "cutctx_audit", "cutctx_orgs"):
                    result = await self._handle_proxy_tools(name, arguments)
                else:
                    result = [
                        TextContent(
                            type="text",
    ...
    async def _handle_read(self, arguments: dict[str, Any]) -> list[TextContent]:
        ...
        # in cutctx/ccr/ — use the centralized safe-log decoder so
    ...
    async def _handle_proxy_tools(self, name: str, arguments: dict[str, Any]) -> list[TextContent]:
        ...
        if name == "cutctx_proxy_start":
            ...
            try:
                ...
                if not violations:
                    return [TextContent(type="text", text="No security violations detected.")]
