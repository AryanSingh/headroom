...
from ..observability import get_cutctx_tracer, get_otel_metrics
...
class TransformPipeline:
    ...
    def __init__(
        ...
        self._breaker_threshold = _breaker_env("CUTCTX_PIPELINE_BREAKER_THRESHOLD", 3, int)
        self._breaker_cooldown_s = _breaker_env("CUTCTX_PIPELINE_BREAKER_COOLDOWN_S", 60.0, float)
    ...
    def _build_default_transforms(self) -> list[Transform]:
        ...
        # CUTCTX_INTERCEPT_ENABLED=1. Off by default while this ships — lets
    ...
    def apply(
        ...
        **kwargs: Any,
        ...
        tracer = get_cutctx_tracer()
        span_attributes = {
            "cutctx.model": model,
            "cutctx.provider": provider_name or "unknown",
            "cutctx.message_count": len(messages),
            "cutctx.tokens.before": tokens_before,
        }
        pipeline_span_context = (
            tracer.start_as_current_span(
                "cutctx.compression.pipeline",
        ...
        with pipeline_span_context as pipeline_span:
            ...
            for transform in self.transforms:
                ...
                transform_span_context = (
                    tracer.start_as_current_span(
                        "cutctx.compression.transform",
                        attributes={
                            "cutctx.model": model,
                            "cutctx.provider": provider_name or "unknown",
                            "cutctx.transform": transform.name,
                        },
                    )
                    if record_metrics
                    else nullcontext()
                ...
                with transform_span_context as transform_span:
                    ...
                    if transform_span is not None and transform_span.is_recording():
                        ...
                        transform_span.set_attribute("cutctx.duration_ms", duration_ms)
                        transform_span.set_attribute(
                            "cutctx.transforms_applied",
            ...
            if pipeline_span is not None and pipeline_span.is_recording():
                pipeline_span.set_attribute("cutctx.tokens.after", tokens_after)
                pipeline_span.set_attribute("cutctx.tokens.saved", total_saved)
                pipeline_span.set_attribute("cutctx.duration_ms", pipeline_ms)
                pipeline_span.set_attribute("cutctx.transforms_applied", len(all_transforms))
                pipeline_span.set_attribute("cutctx.warnings", len(all_warnings))
