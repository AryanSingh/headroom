...
All array compression now goes through `cutctx._core.SmartCrusher`
(built from `crates/cutctx-py`). Byte-equality of the two
...
removed; the Rust crate has its own coverage in `crates/cutctx-core/`
...
The `cutctx._core` extension is a hard import: there is no Python
...
# `crates/cutctx-core/.../compaction/mod.rs`.
...
class SmartCrusher(Transform):
    """Rust-backed `SmartCrusher` (via PyO3 / `cutctx._core`).
    ...
    def __init__(
        ...
        # `observer`: see `cutctx.transforms.observability`. The
        ...
        # back (`cutctx.proxy.server` does). Both `enabled=False` and
        ...
        # CUTCTX_COMPACTION_FORMAT env var when the kwarg is None.
        # Ignored when with_compaction=False.
        resolved_format = compaction_format or os.environ.get(
            "CUTCTX_COMPACTION_FORMAT", "csv-schema"
        )
        ...
        if self._ccr_config.enabled:
            ...
            if is_stateless():
                ...
                ccr_db_path = os.environ.get(
                    "CUTCTX_CCR_DB_PATH", os.path.expanduser("~/.cutctx/ccr.db")
                )
    ...
    def _mirror_single_hash_to_python_store(
        ...
        # the Rust store's `DEFAULT_TTL` (see crates/cutctx-core/src/
    ...
    def _notify_observer(self, original_tokens: int, compressed_tokens: int) -> None:
        ...
        `CompressionObserver` (see `cutctx.transforms.observability`).
