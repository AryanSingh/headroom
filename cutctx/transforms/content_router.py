...
def _detect_content(content: str) -> DetectionResult:
    ...
    Stage-3d (PR5) wired this through `cutctx._core.detect_content_type`,
    ...
    """
    from cutctx._core import detect_content_type as _rust_detect
...
class RouterCompressionResult:
    ...
    def tokens_saved_estimate(self) -> int:
        ...
        Some call-sites (e.g. ``cutctx.proxy.router`` consumers) access
...
class ContentRouter(Transform):
    ...
    def __init__(
        ...
        Args:
            ...
            observer: Optional `CompressionObserver` (see
                `cutctx.transforms.observability`) called once per
    ...
    def _apply_strategy_to_content(
        ...
        try:
            if strategy == CompressionStrategy.CODE_AWARE:
                ...
                # Drain3 path — runs independently of enable_log_compressor
    ...
    def apply(
        ...
        **kwargs: Any,
        ...
        if pending_tasks:
            max_workers = min(
                len(pending_tasks), int(os.environ.get("CUTCTX_COMPRESS_WORKERS", "4"))
            )
