...
primary path delegates to `cutctx._core.DiffCompressor` (built from
`crates/cutctx-py`).  However, as of the 2026-06 audit the Rust
...
class DiffCompressor:
    """Rust-backed `DiffCompressor` (via PyO3 / `cutctx._core`).
...
class DifftasticBackend:
    ...
    def compress(self, content: str, context: str = "") -> DiffCompressionResult:
        ...
        try:
            structural = interceptor.do_transform(exe, content)
        except Exception:
            structural = None
