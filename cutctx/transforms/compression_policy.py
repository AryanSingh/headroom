...
Hand-mirrored port of `cutctx_core::compression_policy::CompressionPolicy`
...
See `crates/cutctx-core/src/compression_policy.rs` for the canonical
...
from cutctx.proxy.auth_mode import AuthMode

# ── F2.2 per-mode default values (CONSERVATIVE pending bake telemetry) ──
# Mirrors the Rust ``pub(crate) const`` block in
# ``crates/cutctx-core/src/compression_policy.rs``. Centralised here so
...
def policy_default_payg() -> CompressionPolicy:
    ...
    ``CUTCTX_PROXY_AUTH_MODE_POLICY_ENFORCEMENT`` flag is disabled
...
_ENFORCEMENT_ENV = "CUTCTX_PROXY_AUTH_MODE_POLICY_ENFORCEMENT"
...
def is_enforcement_enabled() -> bool:
    ...
    # (`cutctx/telemetry/beacon.py::_OFF_VALUES`) so operators don't
