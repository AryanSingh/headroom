...
signing (``cutctx_ee``) is optional — the module degrades gracefully when
...
logger = logging.getLogger("cutctx.security.residency_proof")
...
class ResidencyAttestation:
    ...
    proxy_version:
        Semver string of the running cutctx proxy.
    timestamp_iso:
...
class ResidencyProver:
    ...
    audit_store:
        ...
        attempt a lazy import of ``cutctx_ee.audit.store.AuditStore``
    ...
    def generate(
        ...
        sign:
            ...
            configured via ``CUTCTX_LICENSE_KID`` /
            ``CUTCTX_LICENSE_PRIVATE_KEY`` environment variables.
            Raises ``RuntimeError`` if ``cutctx_ee`` is not installed.
    ...
    def verify(self, attestation: ResidencyAttestation) -> bool:
        ...
        - the attestation carries a valid signature that verifies against the
          public key obtained from ``CUTCTX_LICENSE_PUBLIC_KEY``; *or*
    ...
    def _get_audit_chain_tail(self) -> str | None:
        ...
        if store is None:
            try:
                ...
                from cutctx_ee.audit.store import AuditStore  # type: ignore[import]

                db_url = os.environ.get(
                    "CUTCTX_AUDIT_DB_URL",
                    "sqlite:///cutctx_audit.db",
                )
                store = AuditStore(db_url=db_url)
            except ImportError:
                logger.debug(
                    "_get_audit_chain_tail(): cutctx_ee not available; "
    ...
    def _sign(self, attest: ResidencyAttestation) -> ResidencyAttestation:
        ...
        try:
            ...
            from cutctx_ee.billing.license_token import (  # type: ignore[import]
        ...
        if not kid or not priv_hex:
            raise RuntimeError(
                "CUTCTX_LICENSE_KID and CUTCTX_LICENSE_PRIVATE_KEY must be set "
