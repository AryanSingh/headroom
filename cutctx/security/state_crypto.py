...
Design:
    ...
    * HMAC key comes from ``CUTCTX_LICENSE_HMAC_SECRET`` env var. When unset,
...
logger = logging.getLogger("cutctx.security.state_crypto")

# ---------------------------------------------------------------------------
# Machine-derived encryption key (Fernet)
# ---------------------------------------------------------------------------

_FERNET_KEY_ENV = "CUTCTX_STATE_ENCRYPTION_KEY"
...
_ENCRYPTED_MARKER = "CUTCTX_ENCRYPTED_V1"
...
def write_encrypted_json(path: Path, data: dict[str, Any]) -> None:
    ...
    File format:
        Line 1: marker string ("CUTCTX_ENCRYPTED_V1")
...
_STRICT_STATE_ENV = "CUTCTX_STRICT_STATE"


def read_encrypted_json(path: Path) -> dict[str, Any] | None:
    """Read and decrypt a JSON dict from a file.

    Falls back to reading as plain JSON only if ``CUTCTX_STRICT_STATE`` is
    ...
    if strict:
        logger.error(
            "State file at %s is not encrypted and CUTCTX_STRICT_STATE=1 is set. "
    ...
    logger.warning(
        ...
        "Set CUTCTX_STRICT_STATE=1 to reject unencrypted files in production.",
        path,
    )
    try:
