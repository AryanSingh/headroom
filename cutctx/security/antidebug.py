...
pure-Python fallback covers the case where ``cutctx._core`` has not
...
logger = logging.getLogger("cutctx.security.antidebug")

# Environment variable that disables the guard in controlled dev environments.
# Set ``CUTCTX_ALLOW_DEBUG=1`` in pytest / profiling sessions only.
_ALLOW_DEBUG_ENV = "CUTCTX_ALLOW_DEBUG"
...
def guard_ee_entry() -> None:
    ...
    Execution path:
      ...
      4. No-op if ``CUTCTX_ALLOW_DEBUG=1`` is set.

    Raises:
        RuntimeError: A debugger is attached and ``CUTCTX_ALLOW_DEBUG``
            is not set.
    """
    if _is_debug_allowed():
        logger.debug("antidebug: CUTCTX_ALLOW_DEBUG set, skipping guard")
        return
...
def _abort_debugger_detected() -> NoReturn:
    logger.error(
        "cutctx_ee: debugger detected — refusing to load proprietary modules. "
        "Set CUTCTX_ALLOW_DEBUG=1 only in approved development environments."
    )
    raise RuntimeError(
        "cutctx_ee: debugger detected. "
        "Set CUTCTX_ALLOW_DEBUG=1 to allow debugging in non-production environments."
    )
