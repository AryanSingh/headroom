...
from cutctx.compression.task_aware import (
    RelevanceModulator,
    TaskAwareCompressor,
    TaskAwareResult,
    TaskExtractor,
)
from cutctx.compression.universal import CompressionResult, ContentType
...
def mock_bm25_scorer():
    ...
    with patch("cutctx.compression.task_aware.BM25Scorer") as mock_class:
        mock_instance = MagicMock()
...
class TestRelevanceModulatorBasic:
    ...
    def test_score_high_relevance_exact_match(self):
        ...
        with patch("cutctx.compression.task_aware.BM25Scorer") as mock_bm25:
