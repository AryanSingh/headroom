...
historical ``cutctx.scim`` path so existing call sites keep working when the
...
try:
    import cutctx_ee.scim as _impl
except ImportError as _e:  # commercial component not installed (community edition)
    raise ImportError(
        "cutctx.scim requires the proprietary 'cutctx_ee' distribution "
...
if TYPE_CHECKING:
    ...
    # sys.modules rebind above makes `from cutctx.scim import X` resolve to
    # cutctx_ee.scim; this makes the same names visible to static analysis.
    from cutctx_ee.scim import *  # noqa: F401,F403
