...
Env knobs
---------
HEADROOM_DIFFTASTIC_MIN_CHARS  minimum diff size (chars) before we bother;
                               default 200.
HEADROOM_DIFFTASTIC_TIMEOUT    per-file subprocess timeout in seconds;
                               default 10.
"""
...
import headroom.binaries as binaries_module
...
MIN_CHARS_TO_TRANSFORM = int(os.environ.get("HEADROOM_DIFFTASTIC_MIN_CHARS", "200"))
SUBPROCESS_TIMEOUT_SECONDS = int(os.environ.get("HEADROOM_DIFFTASTIC_TIMEOUT", "10"))
...
_HUNK_MARKER_RE = re.compile(r"^@@\s+")
...
def _reconstruct_old_new(section: str) -> tuple[str, str, str]:
    """Rebuild old and new content from a unified-diff section.

    Returns ``(old_content, new_content, extension)``.
    Extension is inferred from the file path in the ``--- a/`` header.
    """
    ...
    # Capture extension from path headers.
    for line in lines:
        m = re.match(r"^---\s+(?:a/)?(.+)$", line)
        if m:
            p = m.group(1).strip()
            ext = Path(p).suffix or ".txt"
            break
        m = re.match(r"^\+\+\+\s+(?:b/)?(.+)$", line)
        if m:
            p = m.group(1).strip()
    ...
    for line in lines:
        ...
        if stripped.startswith("-") and not stripped.startswith("---"):
            ...
            # Also add to new_lines as an "empty" placeholder so line
            # numbering stays aligned; difft will see deletion-only content.
...
def _run_difft(
    ...
    """
    tmp_dir = tempfile.mkdtemp(prefix="headroom-difft-")
    try:
...
class DifftasticInterceptor:
    ...
    def _get_exe(self) -> str | None:
        ...
        3. ``headroom.binaries.resolve("difft")``.
        """
    ...
    def transform(
        ...
        if exe is None:
            return None
        result = self._do_transform(exe, tool_output)
        if result is None:
            return None
        # Never enlarge
    ...
    def _do_transform(self, exe: str, unified_diff: str) -> str | None:
        ...
        for path_a, path_b, section in file_diffs:
            ...
            if difft_out is None:
                ...
                header = f"[headroom: structural diff via difft for {path_a} -> {path_b}]\n"
