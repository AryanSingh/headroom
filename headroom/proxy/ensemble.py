...
logger = logging.getLogger("headroom.proxy.ensemble")
...
class EnsembleConfig:
    ...
    def from_env(cls) -> EnsembleConfig:
        return cls(
            enabled=os.environ.get("HEADROOM_ENSEMBLE_ENABLED", "").strip() == "1",
            evaluator_model=os.environ.get(
                "HEADROOM_ENSEMBLE_EVALUATOR_MODEL", "claude-3-haiku-20240307"
            ),
            timeout_seconds=float(os.environ.get("HEADROOM_ENSEMBLE_TIMEOUT", "120")),
            require_all_models=os.environ.get("HEADROOM_ENSEMBLE_REQUIRE_ALL", "").strip() == "1",
        )
