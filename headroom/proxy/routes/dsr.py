...
``privacy.dsr`` RBAC permission:
  ...
  GET  /v1/me/export
      ...
      ``request.state.headroom_user_id`` (set by SSO auth), or the
      ``X-Headroom-User-Id`` request header.
...
def _resolve_target_user_id(
    ...
    Priority:
      ...
      3. ``request.state.headroom_user_id`` (set by SSO auth)
      4. ``X-Headroom-User-Id`` request header (SSO claim passthrough)
    ...
    state_user = getattr(request.state, "headroom_user_id", None)
    if state_user:
        return str(state_user)
    header_user = request.headers.get("X-Headroom-User-Id", "").strip()
    if header_user:
        return header_user
    raise HTTPException(
        status_code=400,
...
def create_dsr_router(
    ...
    if require_rbac_permission is not None:
        ...
        # ``headroom_ee/rbac.py`` (mapped to AdminRole.ADMIN).
