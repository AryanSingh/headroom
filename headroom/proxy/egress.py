...
(headroom/proxy/airgap.py) only logged warnings and was never
...
The policy is enforced in two places:
  ...
  - ``headroom/proxy/server.py:check_offline_compat`` is called
    at server boot. If ``HEADROOM_OFFLINE_MODE=1`` and no
    ``CUTCTX_EGRESS_POLICY`` is set, the proxy refuses to start
    (fail-closed for air-gap mode).
  - ``headroom/proxy/routing/failover.py:FailoverRouter`` calls
