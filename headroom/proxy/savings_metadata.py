...
SAVINGS_METADATA_HEADER = "x-headroom-savings-metadata"
...
def _merge_headers(
    ...
    _merge(
        ...
        tokens=_header(headers, "x-headroom-provider-cache-tokens")
        or _header(headers, "x-headroom-provider-cache-hit-tokens"),
    )
    _merge(
        result,
        SavingsSource.SEMANTIC_CACHE,
        tokens=_header(headers, "x-headroom-semantic-cache-tokens")
        or _header(headers, "x-headroom-semantic-cache-avoided-tokens"),
    )
    _merge(
        result,
    ...
    _merge(
        ...
        tokens=_header(headers, "x-headroom-model-routing-tokens")
        or _header(headers, "x-headroom-model-routing-tokens-saved"),
        usd=_header(headers, "x-headroom-model-routing-usd")
        or _header(headers, "x-headroom-model-routing-usd-saved"),
    )
...
def extract_savings_metadata(
    *,
    ...
    savings without leaking Headroom control headers to providers.
    """
    result: dict[str, dict[str, Any]] = {}
    _merge_headers(result, request_headers)
    _merge_headers(result, response_headers)
    if isinstance(body, Mapping):
        _merge_payload(result, body.get("headroom_savings_metadata"))
