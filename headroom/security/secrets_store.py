...
This module ships a production-grade local backend that:
  ...
  1. Stores secrets in a SQLite database with Fernet (AES-128-CBC
     ...
     to ``HEADROOM_LICENSE_HMAC_SECRET`` for air-gapped deploys.
...
operators expect. See ``headroom/proxy/routes/secrets.py``.
"""
...
def _resolve_encryption_key(strict: bool) -> bytes | None:
    ...
    Priority:
      ...
      2. ``HEADROOM_LICENSE_HMAC_SECRET`` (legacy / air-gap)
    ...
    logger.warning(
        ...
        "HEADROOM_LICENSE_HMAC_SECRET in production."
    )
