...
logger = logging.getLogger("headroom.security")
...
class FirewallConfig:
    ...
    def from_env(cls) -> FirewallConfig:
        """Create config from HEADROOM_FIREWALL_* env vars."""
        return cls(
            enabled=os.environ.get("HEADROOM_FIREWALL_ENABLED", "").strip() == "1",
            block_pii=os.environ.get("HEADROOM_FIREWALL_BLOCK_PII", "1").strip() != "0",
            block_injection=os.environ.get("HEADROOM_FIREWALL_BLOCK_INJECTION", "1").strip() != "0",
            block_jailbreak=os.environ.get("HEADROOM_FIREWALL_BLOCK_JAILBREAK", "1").strip() != "0",
            redact_streaming=os.environ.get("HEADROOM_FIREWALL_REDACT_STREAMING", "1").strip() != "0",
            max_buffer_tokens=int(os.environ.get("HEADROOM_FIREWALL_MAX_BUFFER_TOKENS", "50")),
            buffer_timeout_ms=float(os.environ.get("HEADROOM_FIREWALL_BUFFER_TIMEOUT_MS", "10")),
        )
...
_JAILBREAK_PATTERNS: list[tuple[str, re.Pattern[str], float]] = [
    ...
    ("numbered_list_jailbreak", re.compile(
...
_PII_PATTERNS: list[tuple[str, re.Pattern[str], str]] = [
    ...
    # AWS secret key
    ("aws_secret", re.compile(r"(?:aws_secret_access_key|secret_key)\s*[:=]\s*['\"]?[A-Za-z0-9/+=]{40}['\"]?"), "AWS secret"),
    # Private keys
    ("private_key", re.compile(r"-----BEGIN\s+(?:RSA\s+)?PRIVATE\s+KEY-----"), "private key"),
    # API keys (generic patterns)
    ("api_key_bearer", re.compile(r"(?:api[_-]?key|bearer|token|secret)\s*[:=]\s*[\"']?[A-Za-z0-9_\-]{20,}[\"']?"), "API key"),
...
class FirewallScanner:
    ...
    def _scan_pii(self, text: str) -> list[Violation]:
        violations: list[Violation] = []
        for name, pattern, label in _PII_PATTERNS:
            m = pattern.search(text)
            if m:
                violations.append(Violation(
    ...
    def _scan_exfiltration(self, text: str) -> list[Violation]:
        violations: list[Violation] = []
        for name, pattern, label in _EXFIL_PATTERNS:
            m = pattern.search(text)
            if m:
                violations.append(Violation(
...
class StreamingRedactor:
    ...
    _STREAM_PII: list[tuple[str, re.Pattern[str], str]] = [
        ...
        ("private_key", re.compile(r"-----BEGIN\s+(?:RSA\s+)?PRIVATE\s+KEY-----"), "[REDACTED:KEY]"),
    ]
