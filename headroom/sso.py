...
historical ``headroom.sso`` path so existing call sites keep working when the
...
try:
    import headroom_ee.sso as _impl
except ImportError as _e:  # commercial component not installed (community edition)
    raise ImportError(
        "headroom.sso requires the proprietary 'headroom_ee' distribution "
...
if TYPE_CHECKING:
    ...
    # sys.modules rebind above makes `from headroom.sso import X` resolve to
    # headroom_ee.sso; this makes the same names visible to static analysis.
    from headroom_ee.sso import *  # noqa: F401,F403
