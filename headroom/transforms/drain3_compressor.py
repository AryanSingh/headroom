...
Requires: pip install headroom-ai[log-ml]  (``drain3>=0.9.11``).
"""
...
class Drain3LogCompressor:
    ...
    def _get_miner(self) -> Any:
        ...
        miner_config.max_clusters = self.config.max_clusters
        miner_config.sim_threshold = str(self.config.sim_threshold)
        miner_config.depth = self.config.depth
        miner_config.max_children = self.config.max_children
        miner_config.extra_delimiters = []  # Use defaults
    ...
    def _get_fallback(self) -> Any:
        """Lazy-load the standard LogCompressor fallback."""
        if self._fallback is None:
            try:
                ...
                logger.debug("LogCompressor not available for fallback")
