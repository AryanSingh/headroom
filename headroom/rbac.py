...
historical ``headroom.rbac`` path so existing call sites keep working when the
...
try:
    import headroom_ee.rbac as _impl
except ImportError as _e:  # commercial component not installed (community edition)
    raise ImportError(
        "headroom.rbac requires the proprietary 'headroom_ee' distribution "
...
if TYPE_CHECKING:
    ...
    # sys.modules rebind above makes `from headroom.rbac import X` resolve to
    # headroom_ee.rbac; this makes the same names visible to static analysis.
    from headroom_ee.rbac import *  # noqa: F401,F403
