...
historical ``headroom.entitlements`` path so existing call sites keep working when the
...
try:
    import headroom_ee.entitlements as _impl
except ImportError as _e:  # commercial component not installed (community edition)
    raise ImportError(
        "headroom.entitlements requires the proprietary 'headroom_ee' distribution "
...
if TYPE_CHECKING:
    ...
    # sys.modules rebind above makes `from headroom.entitlements import X` resolve to
    # headroom_ee.entitlements; this makes the same names visible to static analysis.
    from headroom_ee.entitlements import *  # noqa: F401,F403
