...
| Item | Status | Notes |
|------|--------|-------|
| Stripe integration | Transitional / hosted-helper state | OSS/EE helpers still reference hosted checkout helpers; self-serve production readiness must be verified separately |
| License server (remote validation) | Hosted validation path present | Remote verification is configurable via hosted license-service URLs; rollout readiness must be verified separately |
| Invoice generation | Operator-managed today | Treat invoicing as an operator workflow until the hosted billing path is fully verified |
| Usage-based billing metering | Not yet customer-proven | Do not treat hosted metering as launch-ready without end-to-end verification |
...
- [~] Billing infrastructure exists in transitional form (hosted helpers + license validation), but self-serve checkout still requires end-to-end verification
