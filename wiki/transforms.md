...
Cutctx provides several transforms that work together to optimize LLM context.
...
```python
from cutctx.transforms import IntelligentContextManager, IntelligentContextConfig
...
```python
from cutctx.config import IntelligentContextConfig, RollingWindowConfig
...
`pip install 'cutctx-ai[llmlingua]'` no longer resolves; use the
...
```bash
pip install "cutctx-ai[code]"  # Adds tree-sitter-language-pack
```

### Configuration

```python
from cutctx.transforms import CodeAwareCompressor, CodeCompressorConfig, DocstringMode
...
```python
from cutctx.transforms import CodeAwareCompressor
...
```python
from cutctx.transforms import is_tree_sitter_available, unload_tree_sitter
...
```python
from cutctx.transforms import ContentRouter, ContentRouterConfig, CompressionStrategy
...
```python
from cutctx import TransformPipeline, SmartCrusher, CacheAligner, RollingWindow
