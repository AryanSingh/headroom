...
The Cutctx proxy server is a production-ready HTTP server that applies context optimization to all requests passing through it.
...
`cutctx wrap copilot` uses Copilot CLI's BYOK provider settings under the hood. In `provider-type=auto`, it chooses Cutctx's Anthropic route for the default proxy backend and the OpenAI-compatible `/v1` route for translated backends such as `anyllm` and LiteLLM.

Anonymous aggregate telemetry is enabled by default. Opt out with `CUTCTX_TELEMETRY=off` or `cutctx proxy --no-telemetry`. Downstream apps can set `CUTCTX_SDK=cutctx-app` to override the anonymous telemetry `sdk` label; the default remains `proxy`.

Operational OTEL metrics are configured separately and are **off by default**. Install `cutctx-ai[proxy,otel]` and set:
...
Use `CUTCTX_OTEL_METRICS_EXPORTER=console` for local smoke testing. `CUTCTX_TELEMETRY` controls the anonymous data-flywheel beacon only; it does not disable or enable OTEL export.

Langfuse can be enabled alongside this OTEL path for **trace ingestion**. Langfuse does **not** ingest OTEL metrics, so Cutctx keeps metrics and Langfuse traces as complementary signals:
...
When configured, Cutctx emits OTLP traces for the shared compression pipeline to Langfuse while continuing to expose metrics through `/metrics` and OTEL metric exporters.
...
| `--code-aware` | true | Enable AST-based code compression (env: CUTCTX_CODE_AWARE_ENABLED) |
...
Cutctx proxy has two explicit run modes:
...
CUTCTX_MODE=cache cutctx proxy
```
...
`/readyz` returns HTTP 503 when Cutctx has not completed startup or a required enabled subsystem is unavailable. This is the endpoint used by the container health checks.
...
other Cutctx frontends. It returns:
...
Set `CUTCTX_SAVINGS_PATH` to override the location directly, or set
`CUTCTX_WORKSPACE_DIR` to relocate the full state root. See the
...
**Headers:**
- `x-cutctx-bypass: true` — skip compression, return messages as-is
...
```
cutctx_requests_total
cutctx_tokens_saved_total
cutctx_cost_usd_total
cutctx_latency_ms_sum
```
...
> **Note:** `build-essential` is required at install time because `cutctx-ai` includes `hnswlib`, a C++ extension that must be compiled from source. It is removed after installation to keep the image slim.
