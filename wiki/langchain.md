...
Cutctx provides seamless integration with LangChain, enabling automatic context optimization across all LangChain patterns: chat models, memory, retrievers, agents, and observability.

## Installation

```bash
pip install "cutctx-ai[langchain]"
```

This installs Cutctx with LangChain dependencies (`langchain-core`).
...
The `CutctxChatModel` wraps any LangChain `BaseChatModel`:
...
llm = CutctxChatModel(ChatAnthropic(model="claude-3-5-sonnet-20241022"))
...
`CutctxChatMessageHistory` wraps any chat history with automatic compression:

```python
from langchain.memory import ConversationBufferMemory
from langchain_community.chat_message_histories import ChatMessageHistory
from cutctx.integrations import CutctxChatMessageHistory

# Wrap any history
base_history = ChatMessageHistory()
compressed_history = CutctxChatMessageHistory(
    base_history,
...
**Why this matters**: Long conversations can blow up to 50K+ tokens. CutctxChatMessageHistory automatically compresses older turns while preserving recent context.
...
`CutctxDocumentCompressor` filters retrieved documents by relevance:

```python
from langchain.retrievers import ContextualCompressionRetriever
from langchain_community.vectorstores import FAISS
from cutctx.integrations import CutctxDocumentCompressor

# Create vector store retriever (retrieve many for recall)
vectorstore = FAISS.from_documents(documents, embeddings)
base_retriever = vectorstore.as_retriever(search_kwargs={"k": 50})

# Wrap with Cutctx compression (keep best for precision)
...
**Why this matters**: Vector search often returns many marginally-relevant documents. CutctxDocumentCompressor uses BM25-style scoring to keep only the most relevant ones, reducing context size while improving answer quality.

---

### 4. Agent Tool Wrapping

`wrap_tools_with_cutctx` compresses tool outputs for agents:

```python
from langchain.agents import create_openai_tools_agent, AgentExecutor
from langchain_core.tools import tool
from cutctx.integrations import wrap_tools_with_cutctx
...
wrapped_tools = wrap_tools_with_cutctx(
    tools,
...
```python
from cutctx.integrations import get_tool_metrics
...
```python
from cutctx.integrations import StreamingMetricsTracker
...
```python
from cutctx.integrations import StreamingMetricsCallback
...
Add Cutctx metrics to LangSmith traces:

```python
from cutctx.integrations import CutctxLangSmithCallbackHandler

# Create callback handler
langsmith_handler = CutctxLangSmithCallbackHandler()
...
# - cutctx.tokens_before
# - cutctx.tokens_after
# - cutctx.tokens_saved
# - cutctx.compression_ratio
```
...
from cutctx.integrations import CutctxChatModel, wrap_tools_with_cutctx
...
# Wrap model with Cutctx
llm = CutctxChatModel(ChatOpenAI(model="gpt-4o"))

# Wrap tools with compression
tools = wrap_tools_with_cutctx([search_web, query_database])
...
**Without Cutctx**: Each tool call adds 10-50K tokens to context.
**With Cutctx**: Tool outputs compressed to 1-2K tokens, agent runs faster and cheaper.
...
from cutctx.integrations.langchain import create_compress_tool_messages_node
...
```python
from cutctx.integrations.langchain import compress_tool_messages
...
from langchain.retrievers import ContextualCompressionRetriever
from cutctx.integrations import CutctxChatModel, CutctxDocumentCompressor
...
# Cutctx compressor for precision
compressor = CutctxDocumentCompressor(
...
- With Cutctx: 5 docs × ~500 tokens = 2.5K context tokens (90% reduction)
...
from cutctx.integrations import CutctxChatModel, CutctxChatMessageHistory
...
**Impact**: Without compression, 100-turn conversation = 100K+ tokens. With CutctxChatMessageHistory, it stays under 8K tokens while preserving recent context.
...
# Wrap everything
llm = CutctxChatModel(ChatOpenAI(model="gpt-4o"))
tools = wrap_tools_with_cutctx([search_arxiv, search_github, fetch_documentation])
...
CutctxChatModel(
    ...
    cutctx_config=CutctxConfig(),  # Cutctx configuration
...
```python
from cutctx.integrations import langchain_available

if not langchain_available():
    print("Install with: pip install cutctx-ai[langchain]")
```
