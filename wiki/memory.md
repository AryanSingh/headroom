...
## What Makes Cutctx Memory Different?
...
cutctx wrap claude --memory    # Claude Code with persistent memory
cutctx wrap codex --memory     # Codex with the SAME memory store
cutctx wrap aider --memory     # Aider shares it too
```
...
Memory is stored per-project at `{cwd}/.cutctx/memory.db`. Each
...
> canonical `CUTCTX_WORKSPACE_DIR` env var. This preserves the
...
User ID is auto-detected from `$USER` (your OS username). Override per-request with the `x-cutctx-user-id` header. All memories are scoped to the user — multiple developers on the same project have separate memory stores.
...
When the LLM calls `memory_save`, cutctx:
...
The LLM then decides whether to merge — using the user's own LLM, not a separate model. No extra cost to cutctx.
...
from cutctx import with_memory
...
Memories evolve over time. When facts change, Cutctx creates a **supersession chain** preserving history:
...
```python
import asyncio
from cutctx.memory import (
    HierarchicalMemory,
    MemoryConfig,
    MemoryCategory,
    EmbedderBackend,
)
from cutctx.memory.ports import MemoryFilter, VectorFilter
...
```python
from cutctx.memory import MemoryConfig, EmbedderBackend
...
Cutctx Memory uses **Protocol interfaces** (ports) for all components, enabling easy swapping:
...
**Letta** pioneered inline memory extraction. Cutctx builds on this with:
...
**When to use Cutctx**: You want memory as a layer on your existing stack.

### vs Mem0

**Mem0** provides a managed memory service. Cutctx differs:
...
**When to use Cutctx**: You want embedded memory with no external services.

### Feature Matrix

| Feature | Cutctx | Letta | Mem0 |
|---------|:--------:|:-----:|:----:|
