...
cutctx learn --project ~/my-project --apply

# Analyze all projects
cutctx learn --all --apply
```
...
The core innovation. Instead of cataloging failures ("Read failed 5 times"), Cutctx finds what the model did to fix each failure:
...
Cutctx manages a clearly-delimited section in each file:

```markdown
<!-- cutctx:learn:start -->
## Cutctx Learned Patterns
*Auto-generated by `cutctx learn` — do not edit manually*
...
<!-- cutctx:learn:end -->
```
...
Cutctx Learn uses a plugin architecture where each agent is a self-contained plugin:
...
**Plugins** bundle scanning, detection, and writing for one agent. Built-in plugins are auto-discovered from `cutctx.learn.plugins.*`. External plugins register via the `cutctx.learn_plugin` entry point.

**The Analyzer** is shared — it uses an LLM (Sonnet, GPT-4o, or Gemini Flash) to find patterns. Same analysis for any agent.

### Adding Support for a New Agent

1. Create `cutctx/learn/plugins/myagent.py`
2. Implement `LearnPlugin` + `ConversationScanner` (scanner + writer + detection)
3. Add `plugin = MyAgentPlugin()` at module scope
4. Done — `cutctx learn --agent myagent` works automatically

Or install an external plugin: `pip install cutctx-learn-cursor` (registers via entry point).
...
`cutctx learn` needs an LLM to analyze your sessions. It picks one automatically using this priority:

| Priority | Source | Example |
|----------|--------|---------|
| 1 | `--model` flag | `cutctx learn --model gpt-4o` |
| 2 | API key env var | `ANTHROPIC_API_KEY`, `OPENAI_API_KEY`, `GEMINI_API_KEY` |
| 3 | `CUTCTX_LEARN_CLI` env var | `export CUTCTX_LEARN_CLI=gemini` |
| 4 | Auto-detect installed CLIs | Checks PATH for `claude`, `gemini`, `codex` |

### Using without an API key

If you use Claude Code, Gemini CLI, or Codex via subscription (no raw API key), `cutctx learn` can call them directly:
...
cutctx learn --model gemini-cli

# Pin a CLI via environment variable
export CUTCTX_LEARN_CLI=codex
cutctx learn
```

Valid values for `CUTCTX_LEARN_CLI`: `claude`, `gemini`, `codex`.
