# Cutctx SDK: A Complete Explanation
...
flowchart TB
    subgraph Entry["Entry Points"]
        ...
        SDK["SDK Mode<br/><i>CutctxClient</i>"]
...
## What Problem Does Cutctx Solve?
...
**Cutctx creates "cutctx"** - it intelligently compresses your input tokens so you have more room (and budget) for what matters.

---

## How Cutctx Works: The Big Picture
...
### 1. CutctxClient (`client.py`) - The Wrapper
...
# After (with Cutctx)
base = OpenAI(api_key="...")
client = CutctxClient(original_client=base, provider=OpenAIProvider())
response = client.chat.completions.create(model="gpt-4o", messages=[...])
```
...
**Why this matters:** Token counting is model-specific. GPT-4 uses different tokenization than Claude. Cutctx needs accurate counts to know how much to compress.

---

### 3. Parser (`parser.py`) - Understanding Your Messages

Before optimizing, Cutctx needs to understand what's in your messages:
...
This is where the real work happens. Cutctx has 4 transforms that run in sequence:
...
# Smart compression:
{
    "__cutctx_constants": {"host": "prod-1"},  # Factor out
    "__cutctx_summary": "items 0-44: cpu stable at ~45",  # Summarize boring part
    "data": [
...
CREATE TABLE requests (
    ...
    tokens_input_before INTEGER,  -- Before Cutctx
    tokens_input_after INTEGER,   -- After Cutctx
...
### Step 2: CutctxClient intercepts
```python
# In client.py:
...
def apply(self, messages, ...):
    ...
    # Transform 3: Kompress ML compressor (opt-in via cutctx-ai[ml])
...
**Location:** `cutctx/cache/compression_store.py`
...
When compression happens, Cutctx injects retrieval instructions into the LLM context.
...
When running as MCP server, Cutctx exposes retrieval as a tool:
...
```json
{
    "__cutctx_compressed": true,
    "__cutctx_hash": "abc123def456",
    "__cutctx_stats": {
        "original_items": 1000,
        "kept_items": 20,
        "errors_preserved": 5
...
**Location:** `cutctx/cache/compression_feedback.py`
...
**Location:** `cutctx/ccr/response_handler.py`

**The Problem:** When the proxy injects the `cutctx_retrieve` tool, the LLM might call it. But who handles that tool call? Without response handling, the tool call would go back to the client unhandled.
...
│     └─ Contains: tool_use(cutctx_retrieve, hash=abc123)       │
│                                                                   │
...
**Location:** `cutctx/ccr/context_tracker.py`
...
Vision models charge by the token, and images are expensive (765-2900 tokens for a typical image). Cutctx's image compression uses a **trained ML router** to automatically select the optimal compression technique.
...
├── client.py            # CutctxClient - the main wrapper
...
**Cutctx is a Context Budget Controller that:**
