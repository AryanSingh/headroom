...
This guide covers deploying the cutctx proxy server as a background service on macOS using LaunchAgent. The service will start automatically on login and restart on crash.
...
- cutctx-ai installed with proxy support
- Anthropic API key configured

### Installing Cutctx with Proxy Support
...
```bash
# Clone or navigate to cutctx repository
...
1. Detect your cutctx installation
...
Edit `~/Library/LaunchAgents/com.cutctx.proxy.plist`:

1. Replace `__CUTCTX_PATH__` with your cutctx path:

   ```bash
   command -v cutctx
   # Example output: /usr/local/bin/cutctx
   ```
...
3. Update shell integration: `export CUTCTX_PORT=9000`

### Log Location

Logs are written to standard macOS locations:

- **Standard output**: `~/Library/Logs/cutctx/proxy.log`
- **Error output**: `~/Library/Logs/cutctx/proxy-error.log`
...
source /path/to/cutctx/examples/deployment/macos-launchagent/shell-integration.sh
```
...
launchctl kickstart -k gui/$(id -u)/com.cutctx.proxy

# Manual stop/start
launchctl bootout gui/$(id -u)/com.cutctx.proxy
launchctl bootstrap gui/$(id -u) ~/Library/LaunchAgents/com.cutctx.proxy.plist
```
...
launchctl disable gui/$(id -u)/com.cutctx.proxy

# Re-enable
launchctl enable gui/$(id -u)/com.cutctx.proxy
```
...
Should show cutctx listening on port 8787.
...
```bash
tail -n 20 ~/Library/Logs/cutctx/proxy-error.log
```
...
| `ModuleNotFoundError: No module named 'cutctx'` | Install: `pip install cutctx-ai[proxy]` |
| `command not found: cutctx` | Update plist with correct path: `command -v cutctx` |
...
- Missing dependencies: `pip install cutctx-ai[proxy]`
...
```bash
# Should be set to 1
echo $CUTCTX_SHELL_INTEGRATION_LOADED
```
...
```bash
grep -A1 RunAtLoad ~/Library/LaunchAgents/com.cutctx.proxy.plist
```
...
```bash
chmod 644 ~/Library/LaunchAgents/com.cutctx.proxy.plist
```

**Verify ownership:**

```bash
ls -l ~/Library/LaunchAgents/com.cutctx.proxy.plist
```
...
# Remove plist
rm ~/Library/LaunchAgents/com.cutctx.proxy.plist

# Remove logs (optional)
rm -rf ~/Library/Logs/cutctx
```
...
- [Architecture](ARCHITECTURE.md) - How Cutctx works internally
...
cp com.cutctx.proxy.plist.template ~/Library/LaunchAgents/com.cutctx.proxy-2.plist
# Edit: Change Label to com.cutctx.proxy-2, port to 8788
launchctl bootstrap gui/$(id -u) ~/Library/LaunchAgents/com.cutctx.proxy-2.plist
```
...
```bash
pip install 'cutctx-ai[pytorch-mps]'   # also works as [pytorch_mps]
export CUTCTX_EMBEDDER_RUNTIME=pytorch_mps
```
...
```xml
<key>CUTCTX_EMBEDDER_RUNTIME</key>
...
**Q: Why LaunchAgent instead of running `cutctx proxy` manually?**
...
A: Yes, fully compatible. ML compression (Kompress, opt-in via `cutctx-ai[ml]`) auto-detects MPS on Apple Silicon.
