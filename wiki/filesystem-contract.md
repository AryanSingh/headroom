...
Cutctx writes configuration, runtime state, logs, and caches to a small
...
|---|---|---|---|
| `CUTCTX_CONFIG_DIR` | `~/.cutctx/config` | User/admin-authored configuration (model catalogs, plugin settings, etc.) | Read-mostly |
| `CUTCTX_WORKSPACE_DIR` | `~/.cutctx` | Runtime state written by the proxy and CLI (savings, logs, memory DB, telemetry, caches) | Read-write |
...
`CUTCTX_SUBSCRIPTION_STATE_PATH`, `CUTCTX_MODEL_LIMITS`, ...)
...
explicit argument
    ...
    ▼
per-resource env var (e.g. CUTCTX_SAVINGS_PATH)
    │ falls through when unset/blank
    ▼
derived from canonical root
    │ e.g. ${CUTCTX_WORKSPACE_DIR}/proxy_savings.json
    ▼
default (e.g. ~/.cutctx/proxy_savings.json)
```

Examples:

- `CUTCTX_WORKSPACE_DIR=/mnt/state` → savings land at
  `/mnt/state/proxy_savings.json` unless `CUTCTX_SAVINGS_PATH` overrides.
- `CUTCTX_SAVINGS_PATH=/custom/savings.json` always wins, even when
  `CUTCTX_WORKSPACE_DIR` is set.
- Unset both and the default is `~/.cutctx/proxy_savings.json`.

## Bucket assignments

### Workspace bucket (`CUTCTX_WORKSPACE_DIR`)

| Resource | Default path | Legacy env var |
|---|---|---|
| Proxy savings ledger | `${WORKSPACE_DIR}/proxy_savings.json` | `CUTCTX_SAVINGS_PATH` |
| TOIN telemetry JSON | `${WORKSPACE_DIR}/toin.json` | `CUTCTX_TOIN_PATH` |
| Subscription tracker state | `${WORKSPACE_DIR}/subscription_state.json` | `CUTCTX_SUBSCRIPTION_STATE_PATH` |
...
### Config bucket (`CUTCTX_CONFIG_DIR`)

| Resource | Default path | Legacy env var |
|---|---|---|
| Models catalog | `${CONFIG_DIR}/models.json` | `CUTCTX_MODEL_LIMITS` (content override) |
| Plugin settings | `${CONFIG_DIR}/plugins/<name>/...` | — |

### Backward compatibility — models.json

`models.json` historically lived at `~/.cutctx/models.json` (i.e. in the
workspace root, not in `config/`). For a seamless migration the Python
providers check **both** locations in this order:

1. `${CUTCTX_CONFIG_DIR}/models.json` (new canonical location)
2. `${CUTCTX_WORKSPACE_DIR}/models.json` (legacy fallback)
...
# → ~/.cutctx/config/plugins/my-plugin

state_dir = paths.plugin_workspace_dir("my-plugin")
# → ~/.cutctx/plugins/my-plugin
...
```typescript
import { pluginConfigDir, pluginWorkspaceDir } from "@cutctx/sdk";
...
## Docker naming overlap: `CUTCTX_WORKSPACE` vs `CUTCTX_WORKSPACE_DIR`
...
|---|---|---|
| `CUTCTX_WORKSPACE` | Host-side (Docker) | Directory to bind-mount into the container as `/workspace` (equivalent to CWD in native runs). Used by `docker-compose.native.yml`. |
| `CUTCTX_WORKSPACE_DIR` | Inside-the-container | Canonical Cutctx state root. Resolves to `/tmp/cutctx-home/.cutctx` inside the official container image, which in turn bind-mounts to `${HOME}/.cutctx` on the host. |

The official Docker bootstrap (compose file, `scripts/install.sh`, and the
Python `install` command) sets `CUTCTX_WORKSPACE_DIR` and
`CUTCTX_CONFIG_DIR` inside the container so the proxy resolves state to
the bind-mounted path without any user action.

## Project-scoped `.cutctx/` directories

A few code paths deliberately use **project-local** `.cutctx/` paths
resolved relative to the current working directory rather than the
canonical workspace root:

- `cutctx/proxy/server.py` — project-scoped memory DB default
- `cutctx/memory/mcp_server.py` — project-scoped memory DB default
- `cutctx/cli/wrap.py` — project-scoped memory and hook artifacts

These **do not obey** `CUTCTX_WORKSPACE_DIR`. This is intentional: it
...
- `CUTCTX_SAVINGS_PATH`
- `CUTCTX_TOIN_PATH`
- `CUTCTX_SUBSCRIPTION_STATE_PATH`
- `CUTCTX_MODEL_LIMITS` (content override — JSON string or file path)
