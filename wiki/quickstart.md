...
Get Cutctx running in 5 minutes with these copy-paste examples.
...
See [Persistent Installs](persistent-installs.md) if you want Cutctx to stay up in the background and be reused by `wrap`.
...
messages = [
    ...
    },
    {
        "role": "tool",
        "tool_call_id": "call_1",
        # This is where Cutctx shines - compressing large outputs
        "content": json.dumps({
...
# Cutctx compresses the 500 results to ~20, keeping the most relevant
response = client.chat.completions.create(
    model="gpt-4o",
    messages=messages,
)
...
# INFO:cutctx.transforms.smart_crusher:SmartCrusher: keeping 15 of 500 items
```
...
```python
from cutctx import CutctxClient, OpenAIProvider, CutctxConfig
...
response = client.chat.completions.create(
    model="gpt-4o",
    messages=messages,
    cutctx_mode="optimize",  # Enable for this request only
)
```
...
| Content Type | What Cutctx Does | Typical Savings |
|--------------|-------------------|-----------------|
