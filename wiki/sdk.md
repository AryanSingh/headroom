...
The Cutctx SDK wraps your existing LLM client to add compression and optimization transparently.

## Installation

```bash
pip install cutctx-ai openai
```
...
Real savings happen with tool outputs. Here's where Cutctx shines:
...
# Cutctx compresses 500 results to ~15, keeping highest-scoring items
response = client.chat.completions.create(
    model="gpt-4o-mini",
    messages=messages
)
...
```python
from cutctx import CutctxClient, GoogleProvider
...
# INFO:cutctx.transforms.smart_crusher:SmartCrusher: kept 15 of 1000 items
```
