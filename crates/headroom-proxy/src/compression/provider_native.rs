...
//! Headroom as complementary to provider-native compaction by detecting
...
//! |------------------|---------------------------------------------------|
//! | HeadroomNative   | Default — Headroom compresses, no provider header  |
//! | ProviderNative   | Client sends compact header, hybrid_mode=false     |
//! | Hybrid            | Client sends compact header, hybrid_mode=true      |
//!
//! # Hybrid mode
//!
//! When both Headroom and provider-native compaction are applied:
//! 1. Headroom compresses first (deletion-based, streaming-aware).
...
pub enum CompactionStrategy {
    /// Use Headroom's own compression (default).
    HeadroomNative,
    /// Pass through to Anthropic's native compaction.
    ProviderNative,
    /// Apply both: Headroom first, then provider native for remaining.
    Hybrid,
}
...
impl Default for StrategyConfig {
    fn default() -> Self {
        Self {
            default_strategy: CompactionStrategy::HeadroomNative,
...
pub fn select_strategy(config: &StrategyConfig, headers: &HeaderMap) -> CompactionStrategy {
    ...
    // Default to Headroom's own compression
    config.default_strategy
}

// ── Response enhancement ────────────────────────────────────────────────────

/// Metadata about Headroom's compression for response enhancement.
#[derive(Debug, Clone)]
pub struct CompressionMetadata {
    /// Number of tokens saved by Headroom compression.
    ...
    if let Some(metadata) = headroom_metadata {
        ...
        if let Some(meta) = response.get_mut("metadata") {
            if let Some(obj) = meta.as_object_mut() {
                obj.insert("headroom_stats".to_string(), stats);
            }
        } else {
            let mut metadata_obj = serde_json::Map::new();
            metadata_obj.insert("headroom_stats".to_string(), stats);
...
/// For `HeadroomNative`: removes the compact header (Headroom handles compression).
...
pub fn prepare_outbound_headers(
    ...
    match strategy {
        ...
        CompactionStrategy::HeadroomNative => {
            // Remove compact header — Headroom will handle compression
...
mod tests {
    ...
    #[test]
    fn prepare_headers_headroom_native_removes_compact() {
        let incoming = make_header("anthropic-beta", "compact-2026-01-12");
        let outgoing = prepare_outbound_headers(&incoming, CompactionStrategy::HeadroomNative);
        assert!(!outgoing.contains_key("anthropic-beta"));
    }

    #[test]
    fn prepare_headers_headroom_native_keeps_other_betas() {
    ...
    fn enhance_response_with_metadata() {
        ...
        assert!(json["metadata"]["headroom_stats"].is_object());
    }
