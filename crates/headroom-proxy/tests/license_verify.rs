...
use headroom_proxy::config::LicenseTier;
use serde_json::json;
...
fn test_ed25519_license_verification() {
    ...
    std::env::set_var("HEADROOM_LICENSE_PUBLIC_KEYS", format!("testkid:{}", hex_pub));
    ...
    let tier = headroom_proxy::license::verify_license_token(&token);
    ...
    assert_eq!(LicenseTier::from_license_key(&tampered_token), LicenseTier::OpenSource);
    
    // Test unknown kid
    let unknown_kid = token.replace("testkid", "unknown");
    assert_eq!(LicenseTier::from_license_key(&unknown_kid), LicenseTier::OpenSource);
}
