//! PyO3 bindings for cutctx-core. Exposed to Python as `cutctx._core`.
//!
...
//! `cutctx.transforms._rust_diff_compressor.RustBackedDiffCompressor`,
...
use cutctx_core::transforms::tag_protector::{
...
///
/// Call once at startup from `cutctx_ee.__init__` before any EE code runs.
...
/// Mirror of `cutctx.transforms.diff_compressor.DiffCompressorConfig`.
...
/// Mirror of `cutctx.transforms.diff_compressor.DiffCompressionResult`.
...
/// Mirror of `cutctx.transforms.smart_crusher.SmartCrusherConfig`.
...
/// Mirror of `cutctx.transforms.smart_crusher.SmartCrusher`.
///
...
impl PySmartCrusher {
    ...
    fn new(config: Option<&PySmartCrusherConfig>, ccr_db_path: Option<String>) -> Self {
        ...
        if let Some(path) = ccr_db_path {
            if let Ok(store) = cutctx_core::ccr::backends::SqliteCcrStore::open(&path, 300) {
    ...
    fn without_compaction(
        ...
        if let Some(path) = ccr_db_path {
            match cutctx_core::ccr::backends::SqliteCcrStore::open(&path, 300) {
    ...
    fn with_compaction_format(
        ...
        match RustSmartCrusher::with_compaction_format(cfg, format_name) {
            ...
            None => Err(pyo3::exceptions::PyValueError::new_err(format!(
                ...
                cutctx_core::transforms::smart_crusher::compaction::CompactionStage::SUPPORTED_FORMAT_NAMES.join(", ")
            ))),
        }
    }
...
/// Mirror of `cutctx.transforms.content_detector.DetectionResult`.
///
...
/// `cutctx/transforms/content_router.py`).
///
...
/// Score a line against the default Cutctx keyword detector.
///
...
//
// Mirrors `cutctx.transforms.search_compressor.SearchCompressor` so the
...
//
// Mirrors `cutctx.transforms.log_compressor.LogCompressor`. Same CCR
...
//
// Mirrors `cutctx.transforms.tag_protector.{protect_tags,restore_tags,
...
/// `crates/cutctx-proxy` binary would sit in front of Python; that
/// binary is not deployed by the CLI (`cutctx proxy`,
/// `cutctx wrap codex`). This binding lets the Python proxy call
...
///   to `cutctx_core::transforms::live_zone::DEFAULT_MODEL`.
///
/// # Returns
/// `(body, modified)`:
...
fn compress_openai_responses_live_zone(
    ...
    let model_str = if model.is_empty() {
        cutctx_core::transforms::live_zone::DEFAULT_MODEL
    } else {
        model
    };
