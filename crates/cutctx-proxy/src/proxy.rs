...
pub struct AppState {
    ...
    /// `cutctx_retrieve(hash)`. Default: `None` (no persistence).
    pub ccr_store: Option<Arc<dyn cutctx_core::ccr::CcrStore>>,
...
impl AppState {
    ...
    /// the live-zone dispatcher stashes original block content keyed
    /// by its BLAKE3 hash so downstream agents can retrieve it via
    /// `cutctx_retrieve(hash)`.
    pub fn with_ccr_store(mut self, store: Arc<dyn cutctx_core::ccr::CcrStore>) -> Self {
...
pub(crate) async fn forward_http(
    ...
    let workspace_id = req
        .headers()
        .get("x-cutctx-workspace")
        .and_then(|v| v.to_str().ok().map(String::from));
    let project_id = req
        .headers()
        .get("x-cutctx-project")
        .and_then(|v| v.to_str().ok().map(String::from));
    let agent_id = req
        .headers()
        .get("x-cutctx-agent")
    ...
    if strip_internal && pre_strip_internal_count > 0 {
        tracing::info!(
            ...
            request_id = %request_id,
            "stripped internal x-cutctx-* headers from upstream-bound request"
        );
        ...
        tracing::warn!(
            ...
            request_id = %request_id,
            "CUTCTX_PROXY_STRIP_INTERNAL_HEADERS=disabled; \
             internal x-cutctx-* headers forwarded to upstream"
        );
    }
    ...
    // `cutctx-request-id` header so callers can correlate proxy
    ...
    let mut response = Response::builder().status(status);
    {
        ...
        if let Some(uid) = upstream_request_id.as_deref() {
            if let Ok(v) = http::HeaderValue::from_str(uid) {
                h.insert(HeaderName::from_static("cutctx-upstream-request-id"), v);
            }
        }
    }
