//! cutctx-proxy: transparent reverse proxy binary.
//!
...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    // downstream agents can retrieve it via `cutctx_retrieve(hash)`.
    if let Some(ccr_store) = init_ccr_store(&config) {
        ...
        let store: Arc<dyn cutctx_core::ccr::CcrStore> = Arc::from(ccr_store);
    ...
    if let Some(key) = config.license_key.as_deref() {
        let api_url = std::env::var("CUTCTX_LICENSE_API_URL")
            .unwrap_or_else(|_| "https://api.cutctx.dev".to_string());

        // Compute install fingerprint for machine binding
        let fingerprint = cutctx_proxy::license::fingerprint::compute_fingerprint();
        let instance_id = uuid::Uuid::new_v4().to_string();

        // SP-1.4: Clock rollback detection at startup
        if let Some(clock_state) = cutctx_proxy::license::fingerprint::load_clock_state() {
            ...
            if clock_state.detect_rollback(now) {
                tracing::warn!(
                    ...
                    cutctx_proxy::license::fingerprint::ClockState::MAX_BACKWARD_DRIFT_SECS
                );
            }
        }
        ...
        cutctx_proxy::license::client::start_crl_refresh_task(
            api_url.clone(),
            ...
            loop {
                tokio::select! {
                    _ = heartbeat_interval.tick() => {
                        ...
                        if !cutctx_proxy::license::client::checkout_seat(
                            &api_url_clone,
                            &key_clone,
                            &user_id,
                        )
                    ...
                    _ = license_check_interval.tick() => {
                        ...
                        let tier = cutctx_proxy::license::verify_license_token(&key_clone);
                        if tier == cutctx_proxy::config::LicenseTier::OpenSource {
                            tracing::warn!(
...
fn init_ccr_store(config: &Config) -> Option<Box<dyn cutctx_core::ccr::CcrStore>> {
    ...
    let backend_config = match backend_kind {
        CcrBackendKind::InMemory => CcrBackendConfig::InMemory {
            capacity: cutctx_core::ccr::DEFAULT_CAPACITY,
            ttl_seconds: ttl,
        },
        CcrBackendKind::Sqlite => {
            let path = config.ccr_path.as_deref().unwrap_or("~/.cutctx/ccr.db");
    ...
    match cutctx_core::ccr::backends::from_config(&backend_config) {
        Ok(store) => {
            tracing::info!(
        ...
        Err(e) => {
            tracing::error!(
                ...
                "CCR store initialization failed; compression will work but \
                 ...
                 cutctx_retrieve(hash) will get cache misses."
            );
            None
        }
    }
