//! OpenTelemetry integration for cutctx-proxy.
//!
...
//! to point to your OTel collector. The service name is `cutctx-proxy`.
//!
...
pub mod metric_names {
    ...
    pub const REQUESTS_TOTAL: &str = "cutctx.requests.total";
    /// Request duration in seconds.
    pub const REQUEST_DURATION: &str = "cutctx.request.duration";
    /// Input tokens per request (before compression).
    pub const REQUEST_TOKENS_INPUT: &str = "cutctx.request.tokens.input";
    /// Output tokens per request.
    pub const REQUEST_TOKENS_OUTPUT: &str = "cutctx.request.tokens.output";
    /// Compression ratio per request.
    pub const COMPRESSION_RATIO: &str = "cutctx.compression.ratio";
    /// Total tokens saved by compression.
    pub const COMPRESSION_TOKENS_SAVED: &str = "cutctx.compression.tokens.saved";
    /// Total bytes saved by compression.
    pub const COMPRESSION_BYTES_SAVED: &str = "cutctx.compression.bytes.saved";
    /// Cache hit count.
    pub const CACHE_HITS: &str = "cutctx.cache.hits";
    /// Cache miss count.
    pub const CACHE_MISSES: &str = "cutctx.cache.misses";
    /// Current cache entry count.
    pub const CACHE_ENTRIES: &str = "cutctx.cache.entries";
    /// Estimated cost savings in USD.
    pub const COST_ESTIMATED: &str = "cutctx.cost.estimated";
}
...
impl OtelMetrics {
    ...
    pub fn new(meter: &opentelemetry::metrics::Meter) -> Self {
        let requests_total = meter
            ...
            .with_description("Total requests processed by cutctx-proxy")
            .build();
...
pub fn create_otel_layer(
    ...
    let tracer = provider.tracer("cutctx-proxy");
...
pub fn add_span_attributes(
    ...
    span.record("cutctx.model", model);
    span.record("cutctx.auth_mode", auth_mode);
    span.record("cutctx.endpoint", endpoint);
    if let Some(saved) = tokens_saved {
        span.record("cutctx.tokens_saved", saved as i64);
    }
}
...
mod tests {
    ...
    fn metric_names_are_unique() {
        ...
        // Check all names start with "cutctx."
        for name in &names {
            assert!(
                name.starts_with("cutctx."),
                "Metric name {} should start with cutctx.",
                name
            );
        }
