...
///
/// Response-side `X-Cutctx-*` injection (e.g. `x-cutctx-tokens-saved`)
...
pub const INTERNAL_HEADER_PREFIX: &str = "x-cutctx-";
...
/// Returns true when `name` matches the internal `x-cutctx-*` prefix
...
///   - When `strip_internal == true`, `x-cutctx-*` headers stripped
///     (PR-A5, fixes P5-49). Operators can disable via
///     `CUTCTX_PROXY_STRIP_INTERNAL_HEADERS=disabled` for diagnostic
///     shadow tracing.
...
mod tests {
    ...
    fn internal_header_detected_case_insensitive() {
        ...
        assert!(is_internal_header(
            &HeaderName::from_bytes(b"X-Cutctx-Mode").unwrap()
        ));
        assert!(is_internal_header(
            &HeaderName::from_bytes(b"X-CUTCTX-FOO").unwrap()
        ));
        assert!(!is_internal_header(&HeaderName::from_static(
            "x-request-id"
        )));
    ...
    fn strip_internal_headers_removes_only_internal_prefix() {
        ...
        h.insert("x-cutctx-bypass", HeaderValue::from_static("true"));
        h.insert("x-cutctx-mode", HeaderValue::from_static("passthrough"));
        h.insert("x-request-id", HeaderValue::from_static("req-1"));
        let removed = strip_internal_headers(&mut h);
        assert_eq!(removed, 2);
        assert!(h.get("x-cutctx-bypass").is_none());
        assert!(h.get("x-cutctx-mode").is_none());
    ...
    fn build_forward_strips_internal_when_enabled() {
        ...
        incoming.insert("x-cutctx-bypass", HeaderValue::from_static("true"));
        ...
        assert!(out.get("x-cutctx-bypass").is_none());
    }
    ...
    fn build_forward_keeps_internal_when_disabled() {
        ...
        assert!(out.get("authorization").is_some());
        assert!(out.get("x-cutctx-bypass").is_some());
    }
}
