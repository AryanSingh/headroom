...
/// `x-cutctx-` is dropped before the upstream call. Stops fingerprinting
/// of the proxy via subscription-revocation flags (`x-cutctx-bypass`,
/// `x-cutctx-mode`, etc.) and prevents leakage of internal user-id /
...
///
/// Source priority: CLI flag → `CUTCTX_PROXY_STRIP_INTERNAL_HEADERS`
/// env var → default (`enabled`).
#[derive(Debug, Clone, Copy, PartialEq, Eq, ValueEnum)]
#[clap(rename_all = "snake_case")]
pub enum StripInternalHeaders {
    /// Strip every `x-cutctx-*` header from upstream-bound requests.
    /// Default. Operationally safe.
    Enabled,
    /// Forward `x-cutctx-*` to upstream verbatim. Diagnostic-only;
...
///
/// Source priority: CLI flag → `CUTCTX_PROXY_CACHE_CONTROL_AUTO_FROZEN`
/// env var → default (`enabled`).
#[derive(Debug, Clone, Copy, PartialEq, Eq, ValueEnum)]
#[clap(rename_all = "snake_case")]
...
/// `CUTCTX_PROXY_AUTH_MODE_POLICY_ENFORCEMENT` env var →
/// default (`disabled`).
#[derive(Debug, Clone, Copy, PartialEq, Eq, ValueEnum)]
#[clap(rename_all = "snake_case")]
...
#[command(
    name = "cutctx-proxy",
    version,
    about = "Cutctx transparent reverse proxy"
)]
pub struct CliArgs {
    /// Address the proxy listens on (e.g. 0.0.0.0:8787).
    #[arg(long, env = "CUTCTX_PROXY_LISTEN", default_value = "0.0.0.0:8787")]
    ...
    #[arg(long, env = "CUTCTX_PROXY_UPSTREAM")]
    pub upstream: Url,
    ...
    /// Enable Cutctx compression on LLM-shaped requests
    ...
    /// `--compression` (or set `CUTCTX_PROXY_COMPRESSION=1`).
    #[arg(
        long = "compression",
        env = "CUTCTX_PROXY_COMPRESSION",
    ...
    ///
    /// Source priority: CLI flag → `CUTCTX_PROXY_COMPRESSION_MODE`
    /// env var → default (`off`).
    #[arg(
        long = "compression-mode",
        env = "CUTCTX_PROXY_COMPRESSION_MODE",
        value_enum,
    ...
    #[arg(
        ...
        env = "CUTCTX_PROXY_CACHE_CONTROL_AUTO_FROZEN",
        value_enum,
    ...
    #[arg(
        ...
        env = "CUTCTX_PROXY_STRIP_INTERNAL_HEADERS",
        value_enum,
    ...
    ///
    /// Source priority: CLI flag → `CUTCTX_PROXY_ENABLE_RESPONSES_STREAMING`
    /// env var → default (`true`).
    #[arg(
        long = "enable-responses-streaming",
        env = "CUTCTX_PROXY_ENABLE_RESPONSES_STREAMING",
    ...
    ///
    /// Source priority: CLI flag → `CUTCTX_PROXY_ENABLE_CONVERSATIONS_PASSTHROUGH`
    /// env var → default (`true`).
    #[arg(
        long = "enable-conversations-passthrough",
        env = "CUTCTX_PROXY_ENABLE_CONVERSATIONS_PASSTHROUGH",
        default_value_t = true,
        action = clap::ArgAction::Set,
    )]
    ...
    ///
    /// Source priority: CLI flag → `CUTCTX_PROXY_ENABLE_BEDROCK_NATIVE`
    /// env var → default (`true`).
    #[arg(
        long = "enable-bedrock-native",
        env = "CUTCTX_PROXY_ENABLE_BEDROCK_NATIVE",
        default_value_t = true,
        action = clap::ArgAction::Set,
    )]
    ...
    ///
    /// Source priority: CLI flag → `CUTCTX_PROXY_BEDROCK_REGION`
    /// env var → `AWS_REGION` env var → default (`us-east-1`).
    #[arg(
        long = "bedrock-region",
        env = "CUTCTX_PROXY_BEDROCK_REGION",
    ...
    ///
    /// Source priority: CLI flag → `CUTCTX_PROXY_BEDROCK_ENDPOINT`
    /// env var → derived-from-region.
    #[arg(long = "bedrock-endpoint", env = "CUTCTX_PROXY_BEDROCK_ENDPOINT")]
    ...
    ///
    /// Source priority: CLI flag → `CUTCTX_PROXY_AWS_PROFILE`
    /// env var → `AWS_PROFILE` env var → default chain.
    #[arg(long = "aws-profile", env = "CUTCTX_PROXY_AWS_PROFILE")]
    ...
    ///
    /// Source priority: CLI flag → `CUTCTX_PROXY_BEDROCK_VALIDATE_EVENTSTREAM_CRC`
    /// env var → default (`true`).
    #[arg(
        long = "bedrock-validate-eventstream-crc",
        env = "CUTCTX_PROXY_BEDROCK_VALIDATE_EVENTSTREAM_CRC",
        default_value_t = true,
        action = clap::ArgAction::Set,
    )]
    ...
    ///
    /// Source priority: CLI flag → `CUTCTX_PROXY_VERTEX_REGION`
    /// env var → default (`us-central1`).
    #[arg(
        long = "vertex-region",
        env = "CUTCTX_PROXY_VERTEX_REGION",
    ...
    ///
    /// Source priority: CLI flag → `CUTCTX_PROXY_VERTEX_ADC_SCOPE`
    /// env var → default (`cloud-platform`).
    #[arg(
        long = "vertex-adc-scope",
        env = "CUTCTX_PROXY_VERTEX_ADC_SCOPE",
        default_value = "https://www.googleapis.com/auth/cloud-platform"
    )]
    pub vertex_adc_scope: String,

    /// Cutctx license key for commercial features. When set, the
    ...
    ///
    /// Source priority: CLI flag → `CUTCTX_CCR_BACKEND` env var →
    /// auto-detect from `--ccr-path`.
    #[arg(long = "ccr-backend", env = "CUTCTX_CCR_BACKEND", value_enum)]
    ...
    ///
    /// Source priority: CLI flag → `CUTCTX_CCR_PATH` env var →
    /// default (`~/.cutctx/ccr.db` when `--ccr-backend` is `sqlite`).
    #[arg(long = "ccr-path", env = "CUTCTX_CCR_PATH")]
    ...
    #[arg(
        ...
        env = "CUTCTX_LICENSE_CHECK_INTERVAL",
        default_value = "300"
    )]
...
impl LicenseTier {
    ...
    ///
    /// When `CUTCTX_LICENSE_HMAC_SECRET` is set, the key format must be
    ...
    pub fn from_license_key(key: &str) -> Self {
        ...
        let hmac_secret = std::env::var("CUTCTX_LICENSE_HMAC_SECRET").ok();
        if let Some(secret) = hmac_secret.filter(|s| !s.is_empty()) {
            return Self::from_license_key_hmac(key, &secret);
        }

        tracing::warn!("License key rejected: insecure prefix matching is disabled. Provide an hrk1 token or set CUTCTX_LICENSE_HMAC_SECRET.");
...
pub struct Config {
    ...
    pub auth_mode_policy_enforcement: AuthModePolicyEnforcement,
    /// Whether to strip internal `x-cutctx-*` headers from
    ...
    pub vertex_adc_scope: String,
    /// Cutctx license key. `None` in open-source mode.
    pub license_key: Option<String>,
...
impl Config {
    ...
    pub fn for_test(upstream: Url) -> Self {
        Self {
            ...
            // Production default: strip internal `x-cutctx-*` headers
