...
fn apply_replacements(body: &[u8], mut replacements: Vec<Replacement>) -> Option<Vec<u8>> {
...
pub(crate) fn apply_selective_clear(
    ...
    let messages = parsed.get(array_name)?.as_array()?;
    ...
    let raw_view: RawBodyView<'_> = serde_json::from_slice(body).ok()?;
    let raw_messages = raw_view.items(array_name)?;
    let mut markers_inserted = Vec::new();
    let mut replacements = Vec::new();
    for index in candidates.into_iter().take(clear_cap) {
        let original = raw_messages.get(index)?.get();
        ...
        let mut rewritten_message = messages.get(index)?.clone();
        if replace_message_text(&mut rewritten_message, &replacement) {
            store.put(&key, original);
            markers_inserted.push(marker);
            replacements.push(Replacement {
                range: raw_range(body, raw_messages.get(index)?)?,
                replacement: serde_json::to_vec(&rewritten_message).ok()?,
            });
        }
    }
...
pub(crate) fn apply_snapshot_resume(
    ...
    let messages = parsed.get(array_name)?.as_array()?;
    let snapshot_end = messages.len().checked_sub(config.keep_tail_turns)?;
        ...
        .filter(|key| cached_snapshot_matches(store, key, session_key, &snapshotted_messages))
    {
        ...
        let payload = serde_json::to_string(&document).ok()?;
    ...
    let raw_view: RawBodyView<'_> = serde_json::from_slice(body).ok()?;
    let raw_messages = raw_view.items(array_name)?;
    let first_range = raw_range(body, raw_messages.get(frozen_count)?)?;
    let last_range = raw_range(body, raw_messages.get(snapshot_end - 1)?)?;
    let rewritten_body = apply_replacements(
        body,
        vec![Replacement {
            range: (first_range.0, last_range.1),
            replacement: serde_json::to_vec(&summary_message).ok()?,
        }],
    )?;
...
pub(crate) fn apply_selected_structural_strategy(
    ...
    match decision.strategy {
        ...
        ContextStrategy::RollingWindow | ContextStrategy::SmartCompact => None,
    }
}
...
mod tests {
  ...
  "messages" : [
    ...
    {"role":"user","content":"fix websocket authentication"}
        ...
        let mutation =
            apply_selective_clear(body, 1, &policy, &StrategyConfig::default(), &store).unwrap();
