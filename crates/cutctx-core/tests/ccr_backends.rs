...
fn sqlite_ttl_purge() {
    ...
    assert_eq!(
        store.len(),
        1,
        "expired entry must be physically deleted, leaving only the dummy entry"
    );
}
...
fn from_config_redis_unsupported_when_feature_off() {
    use cutctx_core::ccr::backends::CcrBackendInitError;
...
mod redis_tests {
    use super::*;
    use cutctx_core::ccr::backends::RedisCcrStore;

    /// Reads `CUTCTX_TEST_REDIS_URL` from the environment — when the
    /// feature is on but no URL is configured we silently no-op. CI
    /// runs the redis test in a docker-compose'd matrix.
    fn redis_url() -> Option<String> {
        std::env::var("CUTCTX_TEST_REDIS_URL").ok()
    }

    #[test]
    fn redis_round_trip() {
        let Some(url) = redis_url() else {
            eprintln!("skipping redis_round_trip: CUTCTX_TEST_REDIS_URL not set");
            return;
        };
    ...
    fn redis_round_trip_via_from_config() {
        let Some(url) = redis_url() else {
            eprintln!("skipping redis_round_trip_via_from_config: CUTCTX_TEST_REDIS_URL not set");
            return;
        };
        let cfg = CcrBackendConfig::Redis {
            url,
