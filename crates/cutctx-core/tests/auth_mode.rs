//! Integration tests for `cutctx_core::auth_mode::classify`.
//!
...
use cutctx_core::auth_mode::{classify, AuthMode};
...
fn subscription_takes_precedence_over_oauth_token() {
    ...
    // request count, never identify Cutctx). UA wins.
    let h = headers(&[
...
/// `crates/cutctx-core/benches/auth_mode.rs`. This in-test loop
...
fn classify_under_10us_per_call() {
    ...
    // 10us = 10_000 ns. Asserting 10x cutctx guards against perf
