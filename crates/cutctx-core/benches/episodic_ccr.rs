...
//!
//! Run with:
//!     cargo bench -p cutctx-core --bench episodic_ccr

use std::sync::Arc;

use criterion::{black_box, criterion_group, criterion_main, Criterion, Throughput};
use cutctx_core::ccr::backends::SqliteCcrStore;
use cutctx_core::ccr::{CcrStore, InMemoryCcrStore};
use cutctx_core::transforms::smart_crusher::compaction::ir::OpaqueKind;
use cutctx_core::transforms::smart_crusher::compaction::walker::emit_opaque_ccr_marker;
...
fn bench_sqlite_ccr(c: &mut Criterion) {
    ...
    let tmp = std::env::temp_dir().join("cutctx_bench_episodic");
