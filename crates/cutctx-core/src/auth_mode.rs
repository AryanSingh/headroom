...
//!   `User-Agent`, never inject `X-Cutctx-*`, never strip
//!   `accept-encoding`).
//!
...
pub enum AuthMode {
    ...
    /// `X-Cutctx-*`; never mutate `User-Agent`.
    Subscription,
}
...
/// `crates/cutctx-core/benches/auth_mode.rs` asserts <10us / call.
...
mod inline_tests {
    ...
    //! cutctx-core --lib` exercises the helper without pulling in
    //! the integration-test binary. The exhaustive test matrix lives
    //! in `crates/cutctx-core/tests/auth_mode.rs`.
